...
- 자연어 기반 metadata 등록과 prepare/approve/execute
...
Prompt 원문은 `prompts/runtime/common/`, `prompts/runtime/specialized/`, `prompts/authoring/common/`과 승인된 경우에만 생성되는 `prompts/authoring/overlays/` 아래 versioned UTF-8 파일로 분리한다. `flow_inventory.json`의 native Prompt Template node가 `prompt_source`로 읽어 Flow canvas에 표시한다.

- Runtime Intent/Answer는 공통 Prompt Template과 도메인 특화 Prompt Template이 필수 pair다.
- Domain/Dataset/Main Filter authoring은 작업별 공통 Prompt Template 한 개가 기본이다.
- Authoring 특화 Prompt는 같은 normalized failure shape의 승인 case 3개 이상·서로 다른 natural-language source 2개 이상·root-cause·관리자 approval pin이 있는 경우에만 optional overlay로 추가한다.
...
- purpose별 필수/optional prompt fragment의 named authority/purpose/revision/SHA-256/placeholder/byte budget과 segment cardinality 검증
- route 또는 mode에 따른 provider 조건부 호출
- closed JSON/schema/candidate/fact-claim 검증
- hash와 call telemetry 기록

Runtime pair나 Authoring common이 누락되거나 pin이 다르면 component 내부 default로 대체하지 않고 provider 호출 전에 fail-closed한다. Authoring optional overlay는 없는 것이 기본이며, 연결된 경우 approval/purpose/domain/revision/hash가 모두 맞아야 한다. 상세 계약은 `harness/contracts/PROMPTS.md`를 따른다.
...
6. 데이터셋 등록 공통 Prompt Template
7. 주요 필터 등록 공통 Prompt Template
8. 승인 profile에서만 해당 작업의 optional 특화 Overlay Prompt Template

Runtime 공통과 특화를 한 Prompt Template 안의 section이나 placeholder로 합치지 않는다. Runtime Prompt Bundle Composer가 `common_prompt_message`, `specialized_prompt_message`, `runtime_context`를 별도 named input으로 받고 공통=`system`, 특화=`domain_policy`, context=`untrusted_data` authority를 검증한다. Authoring Composer는 작업별 common과 runtime context를 필수로 받고 승인된 경우에만 `approved_authoring_overlay` authority의 optional overlay와 approval pin을 받는다. question/candidate/facts/schema/source payload는 Prompt Template에 복제하지 않고 runtime context로 정확히 한 번만 전달한다.

공유 Data Analysis Flow의 Runtime 특화 Prompt Template에 제조 문구를 고정하지 않는다. 안정된 `{domain_prompt_text}` 포트 하나로 활성 Domain Package의 hash-pinned prompt text를 주입한다. 신규 Domain bootstrap은 작업별 공통 authoring Prompt와 자유형 source bundle을 사용하되 LLM 출력은 Domain 표시명/설명 annotation, compact Dataset IR, `target_type` 필수 Main Filter IR로 제한한다. 입력 TXT가 optional overlay나 그 approval을 만들 수 없다.
...
제조 v5 planner, literal parser와 dummy fixture는 명시적으로 선택하는 compatibility/validation package로 이동한다. 공유 Flow의 `domain_id`와 source mode는 미설정 상태에서 fail-closed하며 `manufacturing`/`dummy`를 기본값으로 두지 않는다.
...
- Domain/Dataset/Main Filter별 작업 공통 Prompt Template 한 개
- 승인된 반복 실패 profile에서만 해당 작업의 optional 특화 Overlay Prompt Template
- Prompt Bundle Composer
...
Domain Policy는 Runtime Context Builder, Prompt Composer와 LLM Invoker를 우회하는 것이 아니라 애초에 해당 node를 실행 경로에 포함하지 않는 deterministic 전용 Flow다. Main Filter도 optional `source_grounding_mode=explicit_inventory` proof가 유효할 때만 같은 zero-LLM 경로를 사용한다. Optional authoring overlay가 없는 기본 Flow에서 empty specialized Message를 만들어 pair처럼 가장하지 않는다.
...
Data Analysis Flow에는 입력/최초 설정, Runtime 공통·특화 prompt와 LLM route, active domain policy, data source mode, Typed IR/`registered_call` 실행, Message/API/GaiA·multi-turn 출력 설명 Note를 배치한다. Domain/Dataset/Main Filter Flow에는 자연어 입력, 작업별 공통 prompt와 optional overlay gate, 수정 범위, 승인·저장·출력 Note를 배치한다. Domain Policy Flow에는 explicit 관리자 입력, LLM 0회, registered function card는 pre-registered hash만 참조한다는 Note를 배치한다.
...
- Runtime Intent/Answer의 공통/특화 Prompt Template만 별도 필수 pair로 만들고, exact Langflow 1.9.2 processor로 `{domain_prompt_text}` 고정 dynamic port를 hydrate
- Domain/Dataset/Main Filter authoring은 작업별 공통 Prompt Template 한 개를 기본으로 하며, `authoring_prompt_overlay_approval.v1`의 반복 실패 gate를 통과한 경우에만 별도 optional overlay node를 연결
...
- 공개 `/run` tweak로 prompt, overlay, pin과 model policy를 바꾸지 못하도록 차단
...
- `prompts/runtime/common/`, `prompts/runtime/specialized/`, `prompts/authoring/common/`, `prompts/authoring/overlays/`로 purpose별 원문과 승인 overlay를 분리
- Data Analysis에 Intent 공통·특화, Answer 공통·특화 Prompt Template 4개를 추가하고 Domain/Dataset/Main Filter에는 작업별 공통 Prompt Template 한 개씩만 기본 연결
- 승인된 authoring overlay가 있으면 공통 Prompt와 다른 node ID/source/hash/edge로 연결하고 approval purpose/domain/revision/hash를 함께 pin한다. 승인되지 않았거나 비어 있는 specialized Message를 기본 Flow에 만들지 않는다.
...
- `flow_inventory.json`에 purpose별 segment cardinality, source path, prompt ID/revision/hash/authority, optional overlay approval과 입력 edge를 추가
- `flow_builder_support.py`의 기존 Prompt Template/`prompt_source` 기능에 exact 1.9.2 dynamic port hydration, purpose별 segment cardinality, authority와 allowed-variable 검증 추가
...
- 공유 Flow 기본값을 unconfigured/fail-closed로 변경
...
- Mongo collection, timeout, TTL, approval 입력은 advanced로 이동
...
- Domain/Dataset/Main Filter authoring LLM 호출은 작업별 공통 Prompt 한 개를 기본으로 사용하며, 승인된 반복 실패 overlay가 있을 때만 별도 optional segment를 포함
...
- Runtime 공통 prompt 변경 시 특화/component source hash가, Runtime 특화 prompt 변경 시 공통/component source hash가 불변. Authoring common/approved overlay 변경도 서로와 component source hash를 독립적으로 유지
- source row/full catalog/secret/query가 LLM edge와 prompt/state/trace에 없음
- public HTTP tweak allowlist가 Runtime 공통·특화 Prompt Template, domain prompt text, authoring common/overlay와 approval pin, 모든 prompt pin과 model policy 변경을 거부
...
- Authoring 기본 export에 승인 pin 없는 specialized overlay가 연결됨
