...
제조 v5 planner, literal parser와 dummy fixture는 compatibility/validation package로 격리한다. 공유 Flow의 `01 사용 가능 메타데이터 불러오기`는 domain/source mode 선택 input을 두지 않고 노드에 지정된 3컬렉션의 항목 문서를 결합해 실행용 metadata를 메모리에서 컴파일한다.
...
```text
분석 질문 입력
→ 사용 가능 메타데이터 불러오기 (Mongo URI/DB/세 컬렉션명/timeout, 항목 문서 결합·런타임 컴파일)
...
- Metadata loader의 UI에 MongoDB URI·database·세 컬렉션명·timeout을 제공하고 입력 3컬렉션의 항목 문서 결합 및 런타임 컴파일을 fail-closed로 구현
