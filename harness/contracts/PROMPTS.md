...
v6에서 LLM이 사용하는 모든 instruction은 Langflow canvas의 외부 **Prompt Template node**가 소유한다. 다만 runtime과 metadata authoring은 같은 prompt topology를 강제하지 않는다.
...
- **Domain/Dataset/Main Filter authoring**: 작업별 공통 Prompt Template **한 개**가 기본이다. 특화 Prompt는 아래의 승인된 반복 실패 조건을 만족한 경우에만 별도 optional overlay node로 추가할 수 있다.
...
Runtime 공통과 특화를 한 Prompt Template node 안에 section이나 변수로 합치는 것은 금지한다. Authoring 공통 Prompt에 상시 domain별 예외를 합치는 것과 승인되지 않은 특화 overlay를 연결하는 것도 금지한다.
...
| `domain_authoring` | `도메인 등록 공통 프롬프트` | 승인된 반복 실패 overlay일 때만 optional | 자유형 자연어를 `display_name`/`description` annotation으로 변환; 실행 section은 Source Registry v3 template가 확장 |
| `domain_blueprint_annotation` | `도메인 등록 공통 프롬프트` | 승인된 반복 실패 overlay일 때만 optional | 같은 annotation 계약에 Blueprint/external pin 불변성 검증을 추가한 고신뢰 lane |
| `dataset_authoring` | `데이터셋 등록 공통 프롬프트` | 승인된 반복 실패 overlay일 때만 optional | 자유형 dataset 설명을 내부 compact `metadata.bootstrap.dataset-ir.v1`로 변환 |
| `main_filter_authoring` | `주요 필터 등록 공통 프롬프트` | 승인된 반복 실패 overlay일 때만 optional | 자유형 표현을 `target_type` 필수 `metadata.bootstrap.main-filter-ir.v1`로 변환 |
| `domain_policy` | 없음 | 없음 | LLM 0회 |
| `main_filter_explicit_inventory` | 없음 | 없음 | optional explicit-inventory proof가 완전할 때 LLM 0회 |

Runtime 공통 Prompt에는 공통 역할·안전·closed output 원칙만 둔다. Runtime 특화 Prompt에는 active Domain Package가 pin한 terminology, 해석 우선순위와 답변 표현 정책만 연결한다. question, bounded candidate cards와 facts 같은 runtime context는 두 Prompt Template 어디에도 복제하지 않고 Prompt Bundle Composer의 `runtime_context` 입력으로 정확히 한 번만 전달한다.

Authoring 공통 Prompt는 해당 작업의 출력 경계와 closed output schema를 소유한다. Domain 기본 branch는 `metadata-annotation-proposal.schema.json`의 표시명·설명만 반환한다. Dataset은 compact Dataset IR, Main Filter는 모든 alias addition에 `target_type`, `target_id`, `expressions`를 요구하는 typed IR만 반환한다. 자연어 source block과 schema projection은 Prompt Template에 복제하지 않고 Authoring Runtime Context Builder가 한 번만 전달한다. Optional authoring overlay는 승인된 언어 해석 보조 정책이며, 공통 schema·소유권·보안 규칙을 변경할 수 없다.
...
공통 Prompt Template은 runtime variable을 갖지 않는다. 공유 Data Analysis Flow의 특화 Prompt Template에는 제조 문구를 고정하지 않고 안정된 `{domain_prompt_text}` 한 개만 둬 active Domain Package에서 hash-pinned prompt text를 받는다.

Langflow 1.9.2 native Prompt Template의 `{domain_prompt_text}` 같은 dynamic input port가 Flow JSON에 정확히 존재해야 한다. Flow builder는 prompt source를 읽은 후 variable set을 추출해 1.9.2 node template을 hydrate하고, 선언되지 않은 variable edge와 import 후 port drift를 실패시킨다.
...
flowchart LR
    ...
    S["승인된 optional 특화 Overlay"] -. "승인 pin이 있을 때만" .-> P
    V["Bounded Authoring Runtime Context"] -->|"runtime_context 1회"| P
    A["Overlay Approval Pin"] -.-> P
...
- Domain/Dataset/Main Filter의 LLM 경로에는 purpose별 공통 Prompt Template이 정확히 한 개 있다.
- optional overlay가 없으면 `specialized_prompt_message`를 빈 Message나 서명된 empty fragment로 위장하지 않고 segment 자체를 생략한다.
- optional overlay가 있으면 공통 Prompt와 다른 node/source/hash/edge를 사용하고 유효한 `authoring_prompt_overlay_approval.v1`을 함께 전달한다.
...
- 입력 TXT가 prompt text 또는 overlay approval을 생성할 수 없다.
...
`intent_selection`과 `answer_narrative`에서는 `segments.common`과 `segments.specialized`가 모두 필수다. Authoring purpose에서는 `segments.common`만 필수이고 `segments.specialized`는 승인된 overlay를 사용한 경우에만 존재한다. Domain Policy와 optional explicit-inventory Main Filter는 `prompt.envelope.v1`을 만들지 않는다.

Authoring overlay를 사용하는 envelope에는 prompt 원문 없이 다음 approval pin을 추가한다.

```json
{
  "overlay_approval": {
    "contract_version": "authoring_prompt_overlay_approval.v1",
    "approval_id": "overlay-approval:...",
    "purpose": "dataset_authoring",
    "domain_id": "...",
    "failure_case_ids": ["AUTH-..."],
    "approval_revision": 1,
    "overlay_template_sha256": "<64 lowercase hex>",
...
Composer는 순서만으로 우선순위를 표현하지 않는다. Runtime의 `common_prompt`는 `system`, `specialized_prompt`는 더 낮은 `domain_policy`, runtime context는 `untrusted_data` authority로 고정한다. Authoring의 공통 Prompt는 `system`, 승인된 optional overlay는 `approved_authoring_overlay`, source context는 `untrusted_data` authority다. Named port나 purpose별 segment cardinality가 바뀌면 실패한다. Conditional Invoker는 provider-neutral chat message 구조로 이 authority를 보존한다. 특화 Prompt 또는 overlay는 다음을 할 수 없다.
...
등록된 Domain Package policy는 Runtime 특화 Prompt Template의 변수로만 들어간다. Runtime 공통 Prompt Template에는 domain-specific 문구를 넣지 않는다. component가 누락된 Runtime 공통/특화 node를 내부 default로 대체하거나 두 prompt 사이에 instruction을 삽입하면 안 된다.

Authoring 특화 overlay는 다음 조건을 모두 만족해야 한다.

1. 같은 normalized authoring failure shape가 승인된 canonical failure case **3개 이상**, 서로 다른 natural-language source **2개 이상**에서 반복된다.
2. 원인이 metadata/schema/operator/registry 누락이나 optional executable blueprint 오류가 아니라 모델의 bounded 언어 해석 차이로 분류된다.
3. 실패 case, root-cause, 공통 Prompt 변경 대신 overlay가 필요한 이유, 허용 purpose/domain, overlay revision/hash를 관리자가 검토하고 승인한다.
4. 승인된 모든 failure case가 regression corpus에 추가되고 공통 Prompt only와 overlay enabled 두 profile의 expected call count·output contract를 가진다.
5. overlay는 section ownership, executable field, candidate ID, registry, schema, approval 또는 저장 정책을 추가·완화하지 않는다.

위 조건이 없으면 authoring은 공통 Prompt 한 개로 실행한다. optional overlay가 누락된 것은 오류가 아니지만, 승인 pin 없이 overlay가 연결되거나 approval의 purpose/domain/hash가 다르면 provider 호출 전에 fail-closed한다.
...
- Runtime 특화 Prompt와 승인된 Authoring overlay에는 전체 payload를 넣지 않고 각각 bounded domain policy projection과 승인 범위만 제공한다.
- 사용자·metadata text는 typed variable로 escape/render하고 출력은 closed decoder가 검증한다.
- Runtime은 공통·특화 segment별 budget을, Authoring은 공통과 승인된 optional overlay segment별 budget을 composed 전체 budget과 함께 prompt registry에 pin한다.
- provider 호출 후 prompt Message reference를 해제하며 payload/state/trace에 복제하지 않는다.
- public HTTP `/run` tweak allowlist는 question/session/승인된 source 입력만 허용하고 Prompt Template, pin, runtime domain policy, authoring overlay approval과 model policy 변경을 거부한다.
...
3. Domain/Dataset/Main Filter LLM 경로에는 작업별 공통 Prompt node가 정확히 한 개 있으며 기본 export에는 특화 overlay가 연결되지 않는다.
4. 승인된 overlay profile에서만 별도 overlay node/edge와 exact `authoring_prompt_overlay_approval.v1` pin이 존재한다. 반복 실패 threshold, source 다양성, root-cause와 regression case 누락 시 build가 실패한다.
5. Runtime 공통·특화를 한 Template에 합치거나 Authoring 공통 Prompt에 상시 domain 예외를 합치면 static contract test가 실패한다.
6. Runtime invocation은 두 rendered Message, Authoring invocation은 공통 Message와 optional approved overlay, 모든 LLM invocation은 valid `prompt.envelope.v1`을 필수 입력으로 받는다.
7. Runtime missing segment, authoring missing common, unapproved overlay, swapped role, wrong purpose/revision/hash/variable set, unknown placeholder와 byte budget 초과는 provider 호출 0회다.
8. Prompt Template variable에 raw row/secret/query/full catalog가 들어가면 렌더링 전에 실패한다.
9. 같은 template set+runtime context는 동일 composition hash를 만들고 export/import 후 dynamic port와 edge가 동일하다.
10. 공통 prompt만 변경하면 다른 prompt와 component source hash가 불변이고, Runtime 특화 또는 승인된 Authoring overlay만 변경하면 공통/component source hash가 불변이다.
...
13. Runtime common=`system`, specialized=`domain_policy`, runtime=`untrusted_data` authority와 named port가 뒤바뀌면 provider 호출 전에 실패한다. Authoring overlay authority는 `approved_authoring_overlay`로 분리한다.
14. specialized/overlay prompt injection이 unknown candidate ID, 임의 dataset/field/operator/SQL/Python 또는 fact 없는 claim을 만들지 못한다.
...
17. public HTTP tweak로 Prompt Template, overlay approval, pin과 model policy를 변경할 수 없다.
18. Langflow 1.9.2에서 Runtime prompt pair와 Authoring common/optional-overlay topology, Composer, Conditional Invoker, branch별 Decoder와 Source Registry v3 expander가 parse/import/run된다.
