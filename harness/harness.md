...
| Authoring Prompt Source | Domain/Dataset/Main Filter별 공통 Prompt를 분리해 제공; 자유형 자연어를 Domain annotation, compact Dataset IR, `target_type` 필수 Main Filter IR로 변환; 승인된 반복 실패에서만 별도 optional overlay 제공 | 작업자에게 inventory/IR 문법 강제, Domain LLM에 실행 metadata 생성 허용, 기본 Flow에 상시 특화 Prompt 연결, Domain Policy에서 prompt 또는 LLM 호출 |
...
| Presentation adapter | node input의 표시 profile로 Message section 선택·Markdown 렌더링 | canonical result/API/state 수정 |
| API/GaiA terminal | `response.v1`과 GaiA answer/metadata 출력 | full source row 재직렬화, Message를 구조화 data로 역파싱 |
...
3. runtime은 compiled metadata만 사용한다.
...
19. Runtime Intent와 Answer는 Flow canvas에 **공통 Prompt Template node와 특화 Prompt Template node를 물리적으로 따로 둔다.** Domain/Dataset/Main Filter authoring은 작업별 공통 Prompt 한 개가 기본이며, 특화 Prompt는 `PROMPTS.md`의 승인된 반복 실패 gate를 통과한 optional overlay로만 분리한다. Domain Policy는 Prompt/LLM 0회다. Main Filter의 zero-LLM compile은 `source_grounding_mode=explicit_inventory`에서 완전한 binding proof가 있을 때만 선택적으로 허용한다.
20. Runtime custom component는 `common_prompt_message`, `specialized_prompt_message`, `runtime_context`를 서로 다른 named input으로 받고 authority/hash/size/purpose 검증·조건부 호출·closed decoder만 수행한다. Authoring component는 공통 Message와 runtime context를 필수로 받고 승인된 경우에만 optional overlay Message와 approval pin을 받는다.
21. Runtime 공통/특화 prompt는 서로 다른 node ID, prompt ID, revision, source SHA, rendered SHA와 edge를 유지한다. 공통은 `system`, 특화는 `domain_policy`, runtime context는 `untrusted_data` authority다. Authoring overlay는 `approved_authoring_overlay` authority와 별도 approval hash를 사용한다. component가 임의 문구를 삽입하거나 내부 default로 대체하지 않는다.
22. 공통 production prompt는 Flow build manifest가, Runtime 특화 prompt text는 active Domain Package가 독립적으로 pin한다. Authoring overlay는 최소 3개 승인 failure case와 2개 source의 반복 실패·root-cause·관리자 승인을 pin한다. 어느 prompt를 수정해도 해당 revision/hash와 composition hash를 갱신하고 Flow bundle·manifest·검증 증적을 재생성해야 한다.
...
26. 사용자에게 보이는 Flow·node·input·output의 이름과 설명은 한국어를 기본으로 하고 내부 ID/port/schema key는 유지한다. 각 input/output에는 기능, 필수 여부, 계약 type과 연결 대상을 알 수 있는 설명을 제공한다.
...
28. 기본 metadata authoring은 `자유형 TXT → 외부 공통 Prompt → branch별 closed annotation/IR → Source Registry v3 결정론적 확장 → compile → prepare/approve/execute`다. 최초 Domain bootstrap은 Domain annotation, compact Dataset IR, `target_type` 필수 Main Filter IR을 각각 최대 1회씩, 총 3회 생성하고 한 번에 merge/compile한다. 후속 Dataset/Main Filter 수정은 최대 1회다. `source_grounding_mode=explicit_inventory`와 Blueprint/external pin은 선택적 고신뢰 lane이며 일반 작업자 입력의 전제 조건이 아니다.
29. Deterministic compiler는 schema·dependency·security 일관성을 보장하지만 자연어의 모호한 업무 의도를 임의 확정하지 않는다. Prepare가 assumptions/missing information/typed diff를 표시하고, 외부 승인자는 exact candidate hash를 검토한다.
30. 최초 Domain bootstrap의 Prompt 경계는 다음과 같이 고정한다.
   ...
   - Domain·Dataset·Main Filter의 **공통 Prompt Template node 세 개**는 서로 다른 node/source/hash/edge로 유지한다. 한 Prompt Template 안의 section으로 합치지 않는다. 각 분기는 별도의 Context Builder·Composer·Conditional Invoker를 가지며 하나의 승인된 Language Model node를 공유할 수 있다.
   ...
   - 특화 authoring 지시는 공통 Prompt나 custom component에 하드코딩하지 않는다. 반복 실패 gate와 관리자 pin을 통과한 경우에만 해당 분기의 **별도 optional 특화 Overlay Prompt node**로 연결한다. 현재 기본 Flow는 공통 Prompt 세 개만 포함하며 빈 특화 node를 가장하지 않는다.
...
1. Request capsule과 원문 evidence 기반 typed literal/metadata candidate 생성
2. 이전 turn state/result contract 로드
...
11. owner-bound entity value resolve, immutable job bundle validation, trusted config/query resolution, retrieval
...
21. display profile을 적용한 Message, structured API, GaiA output fan-out
...
- Domain/Dataset/Main Filter 기본 Flow마다 상시 특화 authoring Prompt를 붙이거나 empty specialized fragment로 필수 pair를 가장하기
- 반복 실패 case·root-cause·관리자 approval pin 없이 authoring overlay를 활성화하기
...
- Runtime Intent/Answer 공통·특화 Prompt pair와 Authoring 작업별 공통/승인 optional overlay topology의 named authority/runtime-context edge 완전성 통과
