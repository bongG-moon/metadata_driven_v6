...
2. natural-language metadata authoring을 유지한다.
   ...
   - import-ready 등록 Flow는 별도 `source-registry.v3` 입력을 받지 않는다. 도메인·테이블 카탈로그·메인 필터 항목을 자기 컬렉션에 저장하고, 결정론적 compiler가 세 컬렉션을 결합해 실행 계약·참조 무결성·원천 설정을 검증한다. 과거 registry-backed bootstrap은 내부 마이그레이션·검증 도구에서만 유지한다.
...
25-1. Data Analysis Flow의 업무 특화 함수는 `08B 특화 함수 계약 입력` Text Input에서 일반 metadata·prompt와 분리해 선언한다. 선언은 `specialized-function-input.v1` JSON이며 function ID/version, typed literal trigger와 canonical field binding만 포함한다. Text Input의 임의 Python을 실행하지 않으며 로컬 standalone allowlist에 없는 function, version, field binding은 계획 생성 전에 fail-closed한다.
25-2. 제조 기본 특화 함수는 `manufacturing.filter_ordered_range@1`과 `manufacturing.match_product_tokens@1`이다. 전자는 `ordered_range` literal과 등록된 공정 ordering을 받아 inclusive row selection을 수행하고, 후자는 실제 `product_token` literal만 canonical 제품 필드에 AND로 적용한다. 일반 제품 그룹·공정 그룹·filter·aggregate·join·rank는 이 특화 함수로 우회하지 않고 기존 typed primitive를 유지한다.
...
28. 기본 metadata authoring은 `자유형 TXT → 외부 공통·특화 Prompt pair → branch별 작은 item IR 또는 결정론적 원문 투영 → compile → 3컬렉션 항목 저장`이다. Domain·Dataset·Main Filter는 각자 독립 Flow에서 최대 1회 호출하며, SQL·filter mapping·필수 변수는 원문에서 직접 구조화한다. v5식 입력에 실행 필수 사실이 모두 명시되면 모델 JSON framing과 무관하게 결정론적 투영을 사용할 수 있지만, 누락·충돌 값을 추측하거나 잘린 JSON을 임의 복구하지 않는다. `source_grounding_mode=explicit_inventory`와 Blueprint/external pin은 선택적 고신뢰 lane이며 일반 작업자 입력의 전제 조건이 아니다.
...
31. 세 등록 Flow의 등록 유형은 Flow별 컴포넌트 소스에 고정한다. 작업자에게 `등록 항목`, `domain_id`, `environment`, `dry_run`, `registry_json` 입력을 노출하지 않는다. 프로필이 없으면 실행 번들 결합 시 내부 `default` 도메인과 서울 시간대를 적용하고, 명시적 프로필은 최대 하나만 허용한다. environment는 `production`으로 고정한다.
...
30. 최초 Domain bootstrap의 Prompt 경계는 다음과 같이 고정한다.
   ...
   - 세 import-ready Context Builder는 자연어 원문, 등록 유형별 닫힌 출력 스키마, 공통·특화 Prompt만 LLM에 제공한다. 전체 MongoDB 문서, SQL 실행 결과, credential, 다른 컬렉션 payload는 LLM에 전달하지 않는다. 기존 컬렉션의 업무 어휘와 참조 관계는 저장 단계의 결정론적 compiler가 결합·검증한다.
...
9. metadata resolver와 공통 plan compiler로 `analysis.plan.v1` 생성. `08B 특화 함수 계약 입력`에서 검증되고 질문에 대응 typed literal이 있는 registered function만 exact registry pin을 가진 `registered_call`로 확장
