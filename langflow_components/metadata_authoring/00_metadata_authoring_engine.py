...
def _metadata_collection_names(domain_collection: Any, table_collection: Any, main_filter_collection: Any) -> dict[str, str]:
    ...
    if invalid or len(set(actual.values())) != 3:
        _metadata_collection_fail('메타데이터 컬렉션 이름은 안전한 서로 다른 세 이름이어야 합니다.', {'invalid_roles': invalid, 'actual': actual})
    return actual
...
def load_available_domain_package_from_three_collections(database: Any, *, domain_collection: str=DOMAIN_METADATA_COLLECTION, table_collection: str=TABLE_CATALOG_COLLECTION, main_filter_collection: str=MAIN_FILTER_COLLECTION, session: Any=None) -> dict[str, Any]:
    ...
    return load_domain_package_from_three_collections(database, domain_id, environment, domain_collection=actual['domain'], table_collection=actual['table_catalog'], main_filter_collection=actual['main_filter'], session=session)
...
def replace_metadata_release(database: Any, documents: Mapping[str, Mapping[str, Any]], *, session: Any=None, domain_collection: str=DOMAIN_METADATA_COLLECTION, table_collection: str=TABLE_CATALOG_COLLECTION, main_filter_collection: str=MAIN_FILTER_COLLECTION) -> None:
    collections = _metadata_collection_names(domain_collection, table_collection, main_filter_collection)
