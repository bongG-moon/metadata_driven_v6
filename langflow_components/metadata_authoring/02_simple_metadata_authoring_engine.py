# -*- coding: utf-8 -*-
"""GENERATED standalone component: SimpleMetadataAuthoringEngine.
...
_DOMAIN_ALIAS_TARGET_TYPES = {'metrics': 'metric', 'entity_groups': 'entity_group', 'grains': 'grain', 'relations': 'relation', 'predicates': 'predicate', 'recipes': 'recipe'}
...
_NUMERIC_METRIC_TYPES = {'number', 'integer', 'decimal', 'float', 'quantity', 'currency', 'rate', 'percent', 'percentage'}
...
def _natural_field_aliases(source_text: str, field_id: str, physical_column: str) -> list[str]:
    ...
    names = {str(field_id or '').strip().casefold(), str(physical_column or '').strip().casefold()}
    names.discard('')
    result: list[str] = []
    for name in sorted(names, key=len, reverse=True):
        pattern = re.compile(f'(?<![A-Za-z0-9_.-]){re.escape(name)}(?![A-Za-z0-9_.-])\\s*컬럼.{{0,120}}?(?:이\\s*값|해당\\s*값|그\\s*값)\\s*(?:은|는)\\s*(?P<label>[가-힣A-Za-z][가-힣A-Za-z0-9 _./-]{{0,39}}?)(?=\\s*(?:이야|야|입니다|이다|을\\s*의미|를\\s*의미))', re.IGNORECASE | re.DOTALL)
...
def _enrich_dataset_item_payload(payload: Mapping[str, Any], natural_text: str) -> dict[str, Any]:
    ...
    for field_id, raw_binding in raw_fields.items():
        ...
        declared_aliases = _natural_field_aliases(natural_text, str(field_id), str(binding.get('physical_column') or ''))
    ...
    for name, raw_spec in list(parameters.items()):
        ...
        semantic_type = str(binding.get('semantic_type') or '').casefold() if isinstance(binding, Mapping) else ''
        if semantic_type in {'localdate', 'date'}:
            spec['type'] = 'LocalDate'
            compact_pattern = re.compile(f'(?<![A-Za-z0-9_]){re.escape(str(name))}(?![A-Za-z0-9_]).{{0,80}}?YYYYMMDD\\s*형식', re.IGNORECASE | re.DOTALL)
            dashed_pattern = re.compile(f'(?<![A-Za-z0-9_]){re.escape(str(name))}(?![A-Za-z0-9_]).{{0,80}}?YYYY-MM-DD\\s*형식', re.IGNORECASE | re.DOTALL)
    ...
    if not isinstance(dataset.get('date_policy'), Mapping):
        for name, spec in parameters.items():
            ...
            if semantic_type in {'localdate', 'date'} and str(spec.get('type') or '').casefold() in {'localdate', 'date'}:
                dataset['date_policy'] = {'field': str(name), 'inclusive_start': True, 'inclusive_end': True, 'timezone': 'Asia/Seoul'}
                break
    return dataset
...
def _synthesize_dataset_metrics(draft: dict[str, Any]) -> None:
    ...
    for field_id, owners in sorted(candidates.items()):
        ...
        aliases: list[str] = []
        units = {str(binding.get('unit') or '').strip() for binding in bindings if str(binding.get('unit') or '').strip()}
        ...
        metric = {'aliases': aliases, 'source_binding': {'dataset_family': family, 'field': field_id}, 'additivity': {'default': 'additive', 'allowed_rollups': ['sum']}, 'null_policy': 'exclude_from_sum', 'zero_policy': 'preserve_zero', 'value_type': 'number'}
        if len(units) == 1:
...
def make_partial_metadata_item_documents(authoring_kind: str, patch: Mapping[str, Any], source_text: str, *, updated_at: str | None=None) -> dict[str, list[dict[str, Any]]]:
    ...
    if kind == 'domain':
        ...
        if section_keys:
                ...
                for key in sorted(entries):
                    ...
                    if section == 'entity_groups':
                        ...
                        if set(payload) != required:
                            _fail('공정 그룹에는 group_id, display_name, target_field, members, aliases만 필요합니다.', {'key': str(key), 'missing': sorted(required - set(payload)), 'unknown': sorted(set(payload) - required)})
                        ...
                        safe_id = '[A-Za-z][A-Za-z0-9_.-]{0,127}'
                        if re.fullmatch(safe_id, str(key)) is None or group_id != str(key) or re.fullmatch(safe_id, group_id) is None or (re.fullmatch(safe_id, target_field) is None) or (not display_name) or (not members) or (not aliases) or any((not value for value in members + aliases)) or (len(members) != len(set(members))) or (len(aliases) != len(set(aliases))):
                            _fail('공정 그룹 식별자, 대상 필드, 구성원 또는 별칭이 올바르지 않습니다.', {'key': str(key)})
                        payload = {'group_id': group_id, 'display_name': display_name, 'target_field': target_field, 'expansion': 'closed_set', 'members': members, 'selection': {'operator': 'in', 'value': members}, 'aliases': aliases, 'alias_policy': {'match': 'bounded_longest', 'conflict': 'fail_ambiguous'}, 'legacy_identity': f'process_group.{group_id}'}
                    documents['domain'].append(_item_document('domain', section, str(key), payload, source_text=str(source_text or ''), legacy_natural_texts=None, updated_at=timestamp))
                    alias_target_type = _DOMAIN_ALIAS_TARGET_TYPES.get(section)
                    alias_values = payload.get('aliases') if isinstance(payload.get('aliases'), list) else []
                    alias_values = list(dict.fromkeys((str(value).strip() for value in alias_values if str(value).strip())))
                    if alias_target_type and alias_values:
                        alias_key = f'{alias_target_type}:{key}'
                        alias_payload = {'target_type': alias_target_type, 'target_key': str(key), 'values': [{'text': value, 'priority': 100} for value in alias_values], 'normalization': ['unicode_nfkc', 'trim', 'collapse_space', 'latin_casefold'], 'match': 'bounded_longest', 'conflict': 'fail_ambiguous', 'provenance_source': 'domain'}
                        documents['domain'].append(_item_document('domain', 'aliases', alias_key, alias_payload, source_text=str(source_text or ''), legacy_natural_texts=None, updated_at=timestamp))
        else:
            ...
            if re.fullmatch('[A-Za-z][A-Za-z0-9_.-]{0,127}', domain_id) is None:
                _fail('자동 생성된 도메인 식별자가 올바르지 않습니다.')
            payload = {'display_name': str(patch.get('display_name') or domain_id).strip(), 'description': str(patch.get('description') or '').strip(), 'locale': str(patch.get('locale') or 'ko-KR').strip(), 'timezone': str(patch.get('timezone') or 'Asia/Seoul').strip()}
            documents['domain'].append(_item_document('domain', 'profile', domain_id, payload, source_text=str(source_text or ''), legacy_natural_texts=None, updated_at=timestamp))
    elif kind == 'dataset':
        ...
        for key in sorted(patch['datasets']):
            ...
            documents['table_catalog'].append(_item_document('table_catalog', 'datasets', str(key), payload, source_text=str(source_text or ''), legacy_natural_texts=None, updated_at=timestamp))
    else:
        ...
        for key in sorted(patch['aliases']):
            ...
            if not expected_key or _duplicate_text(key) != _duplicate_text(expected_key) or (not expressions):
                _fail('메인 필터 alias의 key, target 또는 표현식이 완전하지 않습니다.', {'key': str(key)})
            payload['provenance_source'] = 'main_filters'
            documents['main_filter'].append(_item_document('main_filter', 'aliases', str(key), payload, source_text=str(source_text or ''), legacy_natural_texts=None, updated_at=timestamp))
    return documents
...
def assemble_domain_package_from_items(documents: Mapping[str, Sequence[Mapping[str, Any]]], *, alias_activation_out: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    profile = profiles[0] if profiles else {'key': 'default', 'payload': {'display_name': 'Default Domain', 'description': '', 'locale': 'ko-KR', 'timezone': 'Asia/Seoul'}}
    ...
    registered_fields = {_duplicate_text(field_id) for dataset in draft['datasets'].values() if isinstance(dataset, Mapping) for field_id in dataset.get('fields') or {}}
    registries = {'dataset': {_duplicate_text(value) for value in draft['datasets']}, 'field': registered_fields, 'metric': {_duplicate_text(value) for value in draft['metrics']}, 'relation': {_duplicate_text(value) for value in draft['relations']}, 'grain': {_duplicate_text(value) for value in draft['grains']}, 'predicate': {_duplicate_text(value) for value in draft['predicates']}, 'recipe': {_duplicate_text(value) for value in draft['recipes']}, 'entity_group': {_duplicate_text(value) for value in draft['entity_groups']}}
    ...
    for alias_id, payload in draft['aliases'].items():
        ...
        if registered is not None and target_key not in registered:
            deferred_aliases.append({'alias_id': str(alias_id), 'target_type': str(payload.get('target_type') or '').strip(), 'target_key': str(payload.get('target_key') or '').strip()})
            continue
    ...
    if isinstance(alias_activation_out, dict):
        ...
        alias_activation_out.update({'contract_version': 'metadata.alias-activation.v1', 'active_count': len(active_aliases), 'deferred_count': len(deferred_aliases), 'deferred': deferred_aliases})
...
def upsert_partial_metadata_items(database: Any, documents: Mapping[str, Sequence[Mapping[str, Any]]], *, session: Any=None, domain_collection: str=DOMAIN_METADATA_COLLECTION, table_collection: str=TABLE_CATALOG_COLLECTION, main_filter_collection: str=MAIN_FILTER_COLLECTION) -> None:
    ...
    for kind, collection_name in actual.items():
        ...
        for document in prepared[kind]:
            ...
            if isinstance(current, Mapping) and _validated_item(current, kind) == document:
                continue
            collection.replace_one({'_id': document['_id']}, deepcopy(document), upsert=True, session=session)
...
__all__ = ['DOMAIN_METADATA_COLLECTION', 'MAIN_FILTER_COLLECTION', 'METADATA_COLLECTIONS', 'METADATA_RELEASE_VERSION', 'METADATA_SECTION_VERSION', 'TABLE_CATALOG_COLLECTION', 'assemble_domain_package_from_items', 'assemble_domain_package_from_sections', 'load_available_domain_package_from_three_collections', 'load_domain_package_from_three_collections', 'metadata_item_set_projection', 'merge_metadata_items_for_write', 'make_partial_metadata_item_documents', 'make_metadata_item_documents', 'make_metadata_section_documents', 'replace_metadata_items', 'replace_metadata_release', 'upsert_partial_metadata_items']
...
def normalize_authoring_draft_shorthand(manifest: Mapping[str, Any], draft: Mapping[str, Any], target_context: Mapping[str, Any] | None=None) -> dict[str, Any]:
...
def normalize_authoring_section_patch_shorthand(manifest: Mapping[str, Any], patch: Mapping[str, Any], authoring_kind: str, base_draft: Mapping[str, Any] | None=None) -> dict[str, Any]:
    ...
    if kind == 'main_filter':
        ...
        result = normalize_authoring_draft_shorthand(sealed_manifest, result, target_context=base_snapshot)
...
def normalize_draft_alias_shorthand(manifest: Mapping[str, Any], draft: Mapping[str, Any]) -> dict[str, Any]:
...
def merge_blueprint_annotations(blueprint: Mapping[str, Any], annotations: Mapping[str, Any] | None, *, expected_blueprint_sha256: str, expected_domain_id: str, expected_environment: str, source_manifest: Mapping[str, Any]) -> dict[str, Any]:
...
_v2a_IDENTITY_FIELDS = {'metrics': 'metric_id', 'entity_groups': 'group_id', 'grains': 'grain_id', 'relations': 'relation_id', 'orderings': 'ordering_id', 'predicates': 'predicate_id', 'recipes': 'recipe_id'}

_v2a_PATCH_SECTIONS = {'dataset': frozenset({'datasets'}), 'main_filter': frozenset({'aliases', 'entity_groups', 'grains', 'orderings', 'predicates', 'recipes'}), 'domain_policy': frozenset({'prompt_extensions', 'specialized_functions', 'output_profile'})}

_v2a_DELETE_KEYS = frozenset({'$delete', '_delete', 'delete', 'delete_keys', 'deleted', 'remove', 'remove_keys', 'removed'})

def _v2a_fail(message: str, details: Mapping[str, Any] | None=None) -> None:
    raise ContractError('metadata_schema_error', 'metadata_section_patch', message, dict(details or {}))

def _v2a_without_identity_cards(values: Mapping[str, Any], identity_field: str) -> dict[str, dict[str, Any]]:
    result: dict[str, dict[str, Any]] = {}
    for key in sorted(values):
        raw = values[key]
        if not isinstance(raw, Mapping):
            _v2a_fail('runtime catalog identity card must be an object.', {'section_key': key})
        item = deepcopy(dict(raw))
        if str(item.pop(identity_field, key)) != str(key):
            _v2a_fail('runtime catalog identity card does not match its map key.', {'section_key': key, 'identity_field': identity_field})
        result[str(key)] = item
    return result
...
def runtime_catalog_v2_to_authoring_draft(catalog: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if validated.get('contract_version') != RUNTIME_CATALOG_V2:
        _v2a_fail('metadata.runtime.catalog.v2 is required.')
    datasets: dict[str, dict[str, Any]] = {}
    for dataset_key in sorted(validated['datasets']):
        raw = validated['datasets'][dataset_key]
        if not isinstance(raw, Mapping):
            _v2a_fail('runtime dataset must be an object.', {'dataset_key': dataset_key})
        dataset = deepcopy(dict(raw))
        if str(dataset.pop('key', dataset_key)) != str(dataset_key):
            _v2a_fail('runtime dataset key does not match its map key.', {'dataset_key': dataset_key})
        datasets[str(dataset_key)] = dataset
    draft: dict[str, Any] = {'contract_version': AUTHORING_DRAFT_VERSION, 'display_name': str(validated.get('display_name') or validated['domain_id']), 'description': str(validated.get('description') or ''), 'locale': str(validated.get('locale') or 'ko-KR'), 'timezone': str(validated.get('timezone') or 'Asia/Seoul'), 'datasets': datasets, 'metrics': _v2a_without_identity_cards(validated.get('metrics') or {}, 'metric_id'), 'entity_groups': _v2a_without_identity_cards(validated.get('entity_groups') or {}, 'group_id'), 'grains': _v2a_without_identity_cards(validated.get('grains') or {}, 'grain_id'), 'relations': _v2a_without_identity_cards(validated.get('relations') or {}, 'relation_id'), 'orderings': _v2a_without_identity_cards(validated.get('orderings') or {}, 'ordering_id'), 'predicates': _v2a_without_identity_cards(validated.get('predicates') or {}, 'predicate_id'), 'recipes': _v2a_without_identity_cards(validated.get('recipes') or {}, 'recipe_id'), 'aliases': deepcopy(dict(validated.get('aliases') or {})), 'prompt_extensions': deepcopy(dict(validated.get('prompt_extensions') or {})), 'specialized_functions': deepcopy(list(validated.get('specialized_functions') or [])), 'output_profile': deepcopy(dict(validated.get('output_profile') or {})), 'source_provenance': {'source_type': 'compiled_runtime_catalog_v2', 'base_catalog_sha256': str(validated.get('catalog_sha256') or '')}}
    return validate_contract(draft, 'metadata-authoring-draft.schema.json', stage='metadata_section_patch', error_code='metadata_schema_error')

def _v2a_assert_no_delete_directive(value: Any, path: tuple[str, ...]=()) -> None:
    if isinstance(value, Mapping):
        for raw_key, child in value.items():
            key = str(raw_key)
            if key.casefold() in _v2a_DELETE_KEYS:
                _v2a_fail('authoring section patches are upsert-only.', {'path': '.'.join((*path, key))})
            _v2a_assert_no_delete_directive(child, (*path, key))
    elif isinstance(value, list):
        for index, child in enumerate(value):
            _v2a_assert_no_delete_directive(child, (*path, str(index)))

def _v2a_deep_upsert(base: Mapping[str, Any], patch: Mapping[str, Any]) -> dict[str, Any]:
    merged = deepcopy(dict(base))
    for key in sorted(patch):
        value = patch[key]
        if key in merged and isinstance(merged[key], Mapping) and isinstance(value, Mapping):
            merged[key] = _v2a_deep_upsert(merged[key], value)
        else:
            merged[key] = deepcopy(value)
    return merged

def _v2a_upsert_specialized_functions(base: Any, patch: Any) -> list[dict[str, Any]]:
    ...
    if not isinstance(base, list) or not isinstance(patch, list) or (not patch):
        _v2a_fail('specialized_functions policy patch must be a non-empty array.')
    merged: dict[tuple[str, int], dict[str, Any]] = {}
    for source, label in ((base, 'base'), (patch, 'patch')):
        for index, raw in enumerate(source):
            if not isinstance(raw, Mapping):
                _v2a_fail('specialized function policy card must be an object.', {'source': label, 'index': index})
            item = deepcopy(dict(raw))
            ...
            if not function_id or version < 1:
                _v2a_fail('specialized function policy identity is incomplete.', {'source': label, 'index': index})
            merged[function_id, version] = item
    return [merged[key] for key in sorted(merged)]

def apply_authoring_section_patch(base_draft: Mapping[str, Any], patch: Mapping[str, Any], authoring_kind: str) -> dict[str, Any]:
    ...
    """
    base = validate_contract(deepcopy(dict(base_draft)), 'metadata-authoring-draft.schema.json', stage='metadata_section_patch', error_code='metadata_schema_error')
    if not isinstance(patch, Mapping):
        _v2a_fail('authoring section patch must be an object.')
    incoming = deepcopy(dict(patch))
    _v2a_assert_no_delete_directive(incoming)
    kind = str(authoring_kind or '').strip().casefold()
    if kind == 'domain':
        if incoming.get('contract_version') != AUTHORING_DRAFT_VERSION:
            _v2a_fail('domain authoring requires a complete metadata.authoring.draft.v1 object.')
        return validate_contract(incoming, 'metadata-authoring-draft.schema.json', stage='metadata_section_patch', error_code='metadata_schema_error')
    allowed = _v2a_PATCH_SECTIONS.get(kind)
    if allowed is None:
        _v2a_fail('unsupported authoring section kind.', {'authoring_kind': authoring_kind})
    unknown = sorted(set(incoming) - set(allowed))
    if unknown:
        _v2a_fail('authoring patch contains a section owned by another Flow.', {'authoring_kind': kind, 'unknown_sections': unknown})
    if not incoming:
        _v2a_fail('authoring section patch is empty.', {'authoring_kind': kind})
    result = deepcopy(base)
    for section in sorted(incoming):
        value = incoming[section]
        if section == 'specialized_functions':
            result[section] = _v2a_upsert_specialized_functions(result.get(section), value)
            continue
        if not isinstance(value, Mapping) or not value:
            _v2a_fail('authoring section patch must contain at least one object entry.', {'section': section})
        before = result.get(section)
        if not isinstance(before, Mapping):
            _v2a_fail('base authoring section is not an object.', {'section': section})
        result[section] = _v2a_deep_upsert(before, value)
    return validate_contract(result, 'metadata-authoring-draft.schema.json', stage='metadata_section_patch', error_code='metadata_schema_error')

__all__ = ['apply_authoring_section_patch', 'runtime_catalog_v2_to_authoring_draft']

_SEMANTIC_VOCABULARY_SECTIONS = ('datasets', 'fields', 'metrics', 'relations', 'grains', 'orderings', 'predicates', 'recipes', 'entity_groups')

_SEMANTIC_VOCABULARY_LIMITS = {'datasets': (1, 128), 'fields': (1, 4096), 'metrics': (0, 1024), 'relations': (0, 256), 'grains': (0, 256), 'orderings': (0, 128), 'predicates': (0, 256), 'recipes': (0, 256), 'entity_groups': (0, 512)}

def _validated_semantic_vocabulary(value, *, expected_dataset_families=None, expected_field_families=None):
    ...
    id_pattern = re.compile('[A-Za-z][A-Za-z0-9_.-]{0,127}')
    ...
    for section in _SEMANTIC_VOCABULARY_SECTIONS:
        ...
        expected_keys = {'id', 'family', 'labels'} if section == 'datasets' else {'id', 'families', 'labels'} if section == 'fields' else {'id', 'labels'}
        seen_ids = set()
        label_owners = {}
        normalized_cards = []
        for card in cards:
            ...
            for label in labels:
                ...
                normalized_label = re.sub('\\s+', ' ', label.strip())
                folded = normalized_label.casefold()
                owner = label_owners.setdefault(folded, item_id)
                if not normalized_label or len(normalized_label.encode('utf-8')) > 512 or any((ord(character) < 32 for character in normalized_label)) or any((fragment in folded for fragment in forbidden)) or (normalized_label != label) or (folded in folded_labels) or (owner != item_id):
            ...
            if section == 'datasets':
                ...
                if not isinstance(families, list) or not 1 <= len(families) <= 128 or families != sorted(set(families)) or any((not isinstance(family, str) or not id_pattern.fullmatch(family) for family in families)):
    ...
    if expected_dataset_families is not None:
        expected_datasets = {str(dataset_id): str(family) for dataset_id, family in expected_dataset_families.items()}
    ...
    if expected_field_families is not None:
        expected_fields = {str(field_id): sorted({str(family) for family in families}) for field_id, families in expected_field_families.items()}
...
def _dataset_field_allowlists_from_vocabulary(semantic_vocabulary, dataset_descriptors=None):
    dataset_families = {str(card['id']): str(card['family']) for card in (semantic_vocabulary or {}).get('datasets') or [] if isinstance(card, dict)}
    field_families = {str(card['id']): set(card.get('families') or []) for card in (semantic_vocabulary or {}).get('fields') or [] if isinstance(card, dict)}
    if isinstance(dataset_descriptors, dict):
        if set(dataset_descriptors) != set(dataset_families):
            raise ValueError('Approved dataset descriptors do not match the semantic vocabulary.')
        allowlists = {}
        for dataset_id, family in sorted(dataset_families.items()):
            ...
            if not isinstance(fields, dict) or not fields:
                raise ValueError('Approved dataset descriptor has no exact field allowlist.')
            field_ids = sorted((str(field_id) for field_id in fields))
            if any((family not in field_families.get(field_id, set()) for field_id in field_ids)):
                raise ValueError('Approved dataset descriptor field does not belong to its family.')
            allowlists[dataset_id] = field_ids
    else:
        allowlists = {dataset_id: sorted((field_id for field_id, families in field_families.items() if family in families)) for dataset_id, family in sorted(dataset_families.items())}
...
_MAIN_FILTER_TARGET_SECTIONS = {'dataset': 'datasets', 'field': 'fields', 'metric': 'metrics', 'relation': 'relations', 'grain': 'grains', 'predicate': 'predicates', 'recipe': 'recipes', 'entity_group': 'entity_groups'}

def _main_filter_target_allowlists(semantic_vocabulary):
    allowlists = {}
    for target_type, section in _MAIN_FILTER_TARGET_SECTIONS.items():
        ids = sorted({str(card['id']) for card in (semantic_vocabulary or {}).get(section) or [] if isinstance(card, dict) and isinstance(card.get('id'), str)})
        if ids:
...
def _apply_main_filter_ir_allowlists(schema, semantic_vocabulary):
    ...
    branches = []
    for target_type, target_ids in sorted(_main_filter_target_allowlists(semantic_vocabulary).items()):
        branch = deepcopy(base_item)
        branch['properties']['target_type'] = {'type': 'string', 'enum': [target_type]}
        branch['properties']['target_id'] = {'type': 'string', 'enum': target_ids}
        branches.append(branch)
    additions['items'] = {'oneOf': branches}
    return projected

_SEMANTIC_TEMPLATE_SECTIONS = ('metrics', 'relations', 'entity_groups', 'grains', 'orderings', 'predicates', 'recipes')

_SEMANTIC_TEMPLATE_SPECS = {'metrics': ('metric_id', {'additivity', 'aggregation', 'aliases', 'dependencies', 'formula', 'metric_id', 'null_policy', 'source_binding', 'source_field', 'temporal_contract', 'unit', 'value_type', 'zero_policy'}, 1024), 'relations': ('relation_id', {'aliases', 'cardinality', 'join_type', 'key_mappings', 'left_dataset', 'left_keys', 'multi_match_policy', 'null_key_policy', 'relation_id', 'right_dataset', 'right_keys'}, 2048), 'entity_groups': ('group_id', {'alias_policy', 'aliases', 'display_name', 'entity', 'expansion', 'group_id', 'legacy_identity', 'members', 'selection', 'target_field'}, 2048), 'grains': ('grain_id', {'display_fields', 'grain_id', 'keys'}, 2048), 'orderings': ('ordering_id', {'items', 'keys', 'ordering_id'}, 2048), 'predicates': ('predicate_id', {'aliases', 'allowed_operators', 'grain_id', 'group_id', 'predicate', 'predicate_id'}, 2048), 'recipes': ('recipe_id', {'aliases', 'datasets', 'default_operation_template', 'derived_metrics', 'grain', 'metrics', 'recipe_id', 'required_fields', 'required_slots'}, 2048)}

_SEMANTIC_TEMPLATE_LEGACY_ALIAS_TYPES = {'dataset', 'field', 'metric', 'process_group', 'process', 'product_group', 'recipe', 'status'}

_SEMANTIC_TEMPLATE_GENERIC_ALIAS_TYPES = {'dataset', 'field', 'metric', 'relation', 'grain', 'predicate', 'recipe', 'entity_group'}

_SEMANTIC_TEMPLATE_LEGACY_RECIPE_OPS = {'filter', 'ordered_range', 'product_token_match', 'project', 'derive', 'aggregate', 'compare_fields', 'compare_group_attributes', 'find_duplicate_groups', 'join', 'presence_filter', 'sort', 'rank', 'concat_segments', 'transform_previous_result'}

_SEMANTIC_TEMPLATE_GENERIC_RECIPE_OPS = {'filter', 'project', 'aggregate', 'join', 'derive', 'compare_fields', 'sort', 'rank', 'transform_previous_result'}

_DATASET_TEMPLATE_KEYS = {'date_filter_contract', 'date_policy', 'default_detail_fields', 'display_name', 'fixture_only', 'parameters', 'read_policy', 'time_scope', 'upstream_bindings'}

_DATASET_TEMPLATE_REQUIRED_KEYS = {'date_policy', 'default_detail_fields', 'display_name', 'parameters', 'read_policy', 'time_scope'}

def _semantic_sha256_json(value):
    return hashlib.sha256(json.dumps(value, ensure_ascii=False, sort_keys=True, separators=(',', ':'), allow_nan=False).encode('utf-8')).hexdigest()

def _validated_dataset_template(dataset_id, value, declared_sha256, approved_field_ids):
    if not isinstance(value, dict) or not _DATASET_TEMPLATE_REQUIRED_KEYS <= set(value) <= _DATASET_TEMPLATE_KEYS or (not re.fullmatch('[0-9a-f]{64}', str(declared_sha256 or ''))) or (_semantic_sha256_json(value) != declared_sha256):
    ...
    forbidden_keys = {'code', 'source', 'source_code', 'module', 'module_path', 'import_path', 'python', 'sql', 'url', 'uri', 'credential', 'secret', 'password', 'token', 'api_key', 'config_ref', 'query_ref', 'source_type', 'source_adapter', 'fields', 'family'}
    forbidden_values = ('http://', 'https://', 'mongodb://', 'mongodb+srv://', 'select ', 'insert ', 'update ', 'delete ', 'import ', 'e' + 'xec(', 'e' + 'val(', 'password', 'secret', 'token=', 'api_key')
    ...
    def visit(current, depth=0):
        ...
        if isinstance(current, dict):
            ...
            for raw_key, child in current.items():
                ...
                if not key or len(key) > 256 or key.casefold() in forbidden_keys:
        ...
        if isinstance(current, str):
            ...
            if not current.strip() or len(current) > 4096 or any((marker in folded for marker in forbidden_values)):
    ...
    if not isinstance(detail_fields, list) or len(detail_fields) > 128 or len(detail_fields) != len(set(detail_fields)) or any((item not in approved_fields for item in detail_fields)) or (not isinstance(value.get('display_name'), str)) or (not 1 <= len(value['display_name']) <= 200) or (not isinstance(value.get('time_scope'), str)) or (not 1 <= len(value['time_scope']) <= 64) or (not isinstance(value.get('parameters'), dict)) or (len(value['parameters']) > 128) or (not isinstance(value.get('date_policy'), dict)) or (len(value['date_policy']) > 32) or (not isinstance(read_policy, dict)) or (set(read_policy) - {'read_only', 'timeout_seconds', 'max_rows'}) or (read_policy.get('read_only') is not True) or isinstance(read_policy.get('timeout_seconds'), bool) or (not isinstance(read_policy.get('timeout_seconds'), int)) or (not 1 <= read_policy['timeout_seconds'] <= 120) or isinstance(read_policy.get('max_rows'), bool) or (not isinstance(read_policy.get('max_rows'), int)) or (not 1 <= read_policy['max_rows'] <= 1000000):
        raise ValueError('Approved dataset template policies are invalid.')
    if 'date_filter_contract' in value and (not isinstance(value['date_filter_contract'], dict) or len(value['date_filter_contract']) > 32):
    ...
    if 'upstream_bindings' in value and (not isinstance(value['upstream_bindings'], list) or len(value['upstream_bindings']) > 64 or any((not isinstance(card, dict) for card in value['upstream_bindings']))):
...
def _validated_semantic_templates(value, semantic_vocabulary):
    expected_root = {'contract_version', 'locale', 'timezone', 'planner_policy', *_SEMANTIC_TEMPLATE_SECTIONS, 'aliases'}
    if not isinstance(value, dict) or set(value) != expected_root:
    ...
    if not isinstance(locale, str) or not 1 <= len(locale) <= 64 or (not isinstance(timezone, str)) or (not 1 <= len(timezone) <= 128):
    ...
    if planner_profile == 'generic_v2':
        ...
        if set(planner_policy) != {'planner_profile', 'legacy_catalog_sha256'} or not re.fullmatch('[0-9a-f]{64}', str(planner_policy.get('legacy_catalog_sha256') or '')):
    ...
    forbidden_keys = {'source_type', 'source_adapter', 'config_ref', 'query_ref', 'physical_column', 'physical_aliases', 'coercion', 'credential', 'credentials', 'password', 'secret', 'api_key', 'token', 'code', 'python', 'script', 'sql'}
    forbidden_values = ('http://', 'https://', 'mongodb://', 'mongodb+srv://', 'select ', 'insert ', 'update ', 'delete ', 'import pandas', 'def ', 'lambda ', 'api_key', 'password', 'secret', 'token=')
    forbidden_value_patterns = (re.compile('(?:https?|mongodb(?:\\+srv)?|jdbc)://', re.IGNORECASE), re.compile('\\b(?:select\\s+.+\\s+from|insert\\s+into|delete\\s+from|update\\s+.+\\s+set)\\b', re.IGNORECASE), re.compile('\\b(?:import\\s+[A-Za-z_]|from\\s+[A-Za-z_][A-Za-z0-9_.]*\\s+import|def\\s+[A-Za-z_]\\w*\\s*\\(|lambda\\s+)'))

    def validate_safe_json(current, path=()):
        if len(path) > 24:
            raise ValueError('Approved semantic templates exceed the nesting depth limit.')
        if isinstance(current, dict):
            if len(current) > 8192 or any((str(key).casefold() in forbidden_keys for key in current)):
        ...
        if isinstance(current, str):
            folded = current.casefold()
            if not current or len(current) > 4096 or any((fragment in folded for fragment in forbidden_values)) or any((pattern.search(current) for pattern in forbidden_value_patterns)):
    ...
    vocabulary_ids = {section: {str(card['id']) for card in (semantic_vocabulary or {}).get(section) or [] if isinstance(card, dict) and isinstance(card.get('id'), str)} for section in _SEMANTIC_TEMPLATE_SECTIONS}
    normalized = {'contract_version': 'metadata.authoring.semantic-templates.v1', 'locale': locale, 'timezone': timezone, 'planner_policy': deepcopy(planner_policy)}
    for section in _SEMANTIC_TEMPLATE_SECTIONS:
        cards = value.get(section)
        identity_key, allowed_keys, section_limit = _SEMANTIC_TEMPLATE_SPECS[section]
        if not isinstance(cards, dict) or len(cards) > min(_SEMANTIC_VOCABULARY_LIMITS[section][1], section_limit) or set(cards) != vocabulary_ids[section]:
            raise ValueError(f'Approved semantic template {section} IDs do not match the vocabulary.')
        for item_id, card in cards.items():
            if not re.fullmatch('[A-Za-z][A-Za-z0-9_.-]{0,127}', str(item_id)) or not isinstance(card, dict) or identity_key in card or (not set(card).issubset(allowed_keys - {identity_key})):
                raise ValueError(f'Approved semantic template {section} card is open or invalid.')
        validate_safe_json(cards, (section,))
        normalized[section] = {str(item_id): cards[item_id] for item_id in sorted(cards)}
    registered_families = {str(card.get('family') or '') for card in (semantic_vocabulary or {}).get('datasets') or [] if isinstance(card, dict)}
    field_families = {str(card.get('id') or ''): set(card.get('families') or []) for card in (semantic_vocabulary or {}).get('fields') or [] if isinstance(card, dict)}
    for metric in normalized['metrics'].values():
        binding = metric.get('source_binding')
        if binding is None:
            continue
        if not isinstance(binding, dict) or not {'dataset_family', 'field'}.issubset(binding) or (not set(binding).issubset({'dataset_family', 'field', 'fixed_filters'})):
        ...
        for fixed_filter in fixed_filters:
            if not isinstance(fixed_filter, dict) or set(fixed_filter) != {'field', 'operator', 'semantic_type', 'value'} or family not in field_families.get(str(fixed_filter.get('field') or ''), set()):
    ...
    def recipe_operations(current):
        ...
        if isinstance(current, dict):
            ...
            for child in current:
                operations.update(recipe_operations(child))
        return operations
    allowed_recipe_ops = _SEMANTIC_TEMPLATE_LEGACY_RECIPE_OPS if planner_profile == 'legacy_v1_compat' else _SEMANTIC_TEMPLATE_GENERIC_RECIPE_OPS
    for recipe in normalized['recipes'].values():
        if not recipe_operations(recipe.get('default_operation_template')).issubset(allowed_recipe_ops):
    ...
    target_ids = {'dataset': {str(card.get('id') or '') for card in (semantic_vocabulary or {}).get('datasets') or [] if isinstance(card, dict)}, 'field': set(field_families), 'metric': set(normalized['metrics']), 'relation': set(normalized['relations']), 'grain': set(normalized['grains']), 'predicate': set(normalized['predicates']), 'recipe': set(normalized['recipes']), 'entity_group': set(normalized['entity_groups']), 'process_group': set(normalized['entity_groups']), 'product_group': set(normalized['predicates'])}
    process_targets = set()
    ...
    allowed_alias_types = _SEMANTIC_TEMPLATE_LEGACY_ALIAS_TYPES if planner_profile == 'legacy_v1_compat' else _SEMANTIC_TEMPLATE_GENERIC_ALIAS_TYPES
    for alias_id, card in aliases.items():
        if not isinstance(alias_id, str) or not alias_id or len(alias_id) > 256 or (not isinstance(card, dict)) or (set(card) != {'target_type', 'target_key', 'values', 'normalization', 'match', 'conflict', 'provenance_source'}):
        ...
        values = card.get('values')
        if alias_id != f'{target_type}:{target_key}' or target_type not in allowed_alias_types or (target_type != 'status' and target_key not in target_ids.get(target_type, set())) or (not isinstance(normalization, list)) or (not 1 <= len(normalization) <= 16) or (len(normalization) != len(set(normalization))) or any((not isinstance(item, str) or not re.fullmatch('[a-z][a-z0-9_]{0,63}', item) for item in normalization)) or any((not isinstance(card.get(key), str) or not card.get(key) or len(card.get(key)) > 128 for key in ('match', 'conflict', 'provenance_source'))) or (not isinstance(values, list)) or (not 1 <= len(values) <= 128):
            raise ValueError('Approved semantic template alias policy is invalid.')
        for alias_value in values:
            if not isinstance(alias_value, dict) or set(alias_value) != {'text', 'priority'} or (not isinstance(alias_value.get('text'), str)) or (not alias_value.get('text')) or (len(alias_value.get('text')) > 256) or isinstance(alias_value.get('priority'), bool) or (not isinstance(alias_value.get('priority'), int)) or (not 0 <= alias_value.get('priority') <= 1000000):
                raise ValueError('Approved semantic template alias value is invalid.')
    validate_safe_json(aliases, ('aliases',))
    normalized['aliases'] = {str(alias_id): aliases[alias_id] for alias_id in sorted(aliases)}
    return normalized
...
def _json_object(text):
    ...
    if isinstance(text, dict):
        value = deepcopy(text)
        response_bytes = len(json.dumps(value, ensure_ascii=False, separators=(',', ':'), allow_nan=False).encode('utf-8'))
        if response_bytes > MAX_AUTHORING_MODEL_RESPONSE_BYTES:
            raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft exceeds the bounded response size.', {'response_bytes': response_bytes, 'max_bytes': MAX_AUTHORING_MODEL_RESPONSE_BYTES})
        return value
    raw = str(text or '').strip()
    response_bytes = len(raw.encode('utf-8'))
    if not raw or response_bytes > MAX_AUTHORING_MODEL_RESPONSE_BYTES:
        raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft response size is invalid.', {'response_bytes': response_bytes, 'max_bytes': MAX_AUTHORING_MODEL_RESPONSE_BYTES})
    spans = []
    start = None
    depth = 0
    in_string = False
    ...
    for index, character in enumerate(raw):
        if start is None:
            if character == '{':
                ...
                raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft JSON framing is unbalanced.', {'framing_reason': 'unmatched_closing_brace'})
            continue
        if in_string:
            if escaped:
                escaped = False
    ...
    if start is not None:
        raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft JSON framing is incomplete.', {'framing_reason': 'unclosed_object'})
    if len(spans) != 1:
        raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft must contain exactly one JSON object.', {'object_count': len(spans)})
    prefix = raw[:spans[0][0]]
    suffix = raw[spans[0][1]:]
    if any((character in '[]' for character in prefix + suffix)):
        raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft JSON framing is invalid.', {'framing_reason': 'array_wrapper'})
    candidate = raw[spans[0][0]:spans[0][1]]
    try:
        value = json.loads(candidate)
    except json.JSONDecodeError as exc:
        raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft JSON is invalid.', {'line': exc.lineno, 'column': exc.colno}) from exc
    if not isinstance(value, dict):
        raise ContractError('metadata_schema_error', 'metadata_draft', 'LLM draft must be an object.')
    return value
...
def _strict_json_value(text, *, label, expected_type):
    ...
    try:
        ...
        raise ContractError('metadata_schema_error', 'metadata_policy_input', f'{label} must be valid JSON.', {'line': exc.lineno}) from exc
...
BOOTSTRAP_FRAGMENT_SECTIONS = {'domain': ('display_name', 'description', 'locale', 'timezone', 'metrics', 'relations', 'aliases', 'entity_groups', 'grains', 'orderings', 'predicates', 'recipes', 'output_profile'), 'dataset': ('datasets',), 'main_filter': ('aliases',)}

BOOTSTRAP_FRAGMENT_REQUIRED = {'domain': ('display_name',), 'dataset': ('dataset_cards',), 'main_filter': ('aliases',)}

BOOTSTRAP_BRANCH_ORDER = ('domain', 'dataset', 'main_filter')

COMPACT_DATASET_CARD_OPTIONAL = ('display_name',)
...
def _bootstrap_reachable_defs(properties, available_defs):
    ...
    while pending:
        ...
        if not isinstance(definition, dict):
            raise ContractError('metadata_schema_error', 'metadata_authoring', '분할 초기 등록 schema definition 참조가 유효하지 않습니다.', {'definition': name})
...
def _bootstrap_partial_schema(value):
    ...
    if not isinstance(value, dict):
        return deepcopy(value)
    result = {key: _bootstrap_partial_schema(child) for key, child in value.items() if key != 'required'}
...
def _bootstrap_fragment_draft_schema(kind):
    ...
    if sections is None:
        raise ContractError('metadata_schema_error', 'metadata_authoring', '분할 초기 등록 제안 종류가 유효하지 않습니다.', {'authoring_branch': str(kind or '')})
    if kind == 'domain':
    ...
    return {'$schema': full_schema.get('$schema'), 'title': f'metadata.authoring.{kind}.section-patch.v1', 'type': 'object', 'additionalProperties': False, 'minProperties': 1, 'maxProperties': len(sections), 'properties': deepcopy(owned), '$defs': _bootstrap_reachable_defs(owned, full_schema.get('$defs') or {}), 'required': list(BOOTSTRAP_FRAGMENT_REQUIRED[kind])}

def _bootstrap_output_schema(kind, source_sha256, approved_dataset_ids=(), approved_dataset_field_ids=None, approved_semantic_vocabulary=None):
    ...
    if kind == 'main_filter' and approved_semantic_vocabulary is not None:
        draft_schema = _apply_main_filter_ir_allowlists(draft_schema, approved_semantic_vocabulary)
    approved_ids = sorted({str(item).strip() for item in approved_dataset_ids or () if str(item).strip()})
    if kind == 'dataset' and approved_ids:
        field_allowlists = {dataset_id: sorted({str(field_id).strip() for field_id in (approved_dataset_field_ids or {}).get(dataset_id) or [] if str(field_id).strip()}) for dataset_id in approved_ids}
        if any((not field_ids for field_ids in field_allowlists.values())):
            raise ContractError('metadata_dependency_error', 'metadata_source_context', '승인 dataset별 field allowlist가 비어 있습니다.')
        ...
        for dataset_id in approved_ids:
            ...
            dataset_card['properties']['dataset_id'] = {'type': 'string', 'enum': [dataset_id]}
            field_card = deepcopy(base_field_card)
            field_card['properties']['id'] = {'type': 'string', 'enum': allowed_fields}
            field_card['properties']['col'] = {'type': 'string', 'enum': allowed_fields}
            dataset_card['properties']['fields']['items'] = field_card
            dataset_branches.append(dataset_card)
        draft_schema['$defs']['datasetCard'] = {'oneOf': dataset_branches}
    draft_definitions = {}
    proposal_definitions = proposal_schema.setdefault('$defs', {})
    if set(draft_definitions) & set(proposal_definitions):
        raise ContractError('metadata_schema_error', 'metadata_prompt_contract', '등록 초안 schema definition 이름이 proposal 계약과 충돌합니다.')
    ...
    for branch in proposal_schema.get('oneOf') or []:
        ...
        if not isinstance(properties, dict) or not isinstance(required, list):
            continue
        properties['source_sha256'] = {'type': 'string', 'const': source_sha256}
...
def _authoring_section_output_schema(kind, *, source_sha256, grounding_mode, annotation_only=False, approved_dataset_ids=(), approved_dataset_field_ids=None, approved_semantic_vocabulary=None):
    if grounding_mode == 'freeform_llm' and approved_semantic_vocabulary is None and (not annotation_only):
        schema_name = {'domain': 'metadata-freeform-domain-ir.schema.json', 'dataset': 'metadata-freeform-dataset-ir.schema.json', 'main_filter': 'metadata-freeform-main-filter-ir.schema.json'}.get(kind)
        draft_schema = load_schema(schema_name) if schema_name else {}
    elif kind == 'domain':
        draft_schema = load_schema('metadata-annotation-proposal.schema.json' if annotation_only else 'metadata-authoring-draft.schema.json')
    elif kind == 'dataset':
    ...
    if kind == 'dataset' and approved_ids:
        ...
        for dataset_id in approved_ids:
            ...
            card['properties']['dataset_id'] = {'type': 'string', 'enum': [dataset_id]}
            field_card = deepcopy(base_field_card)
            field_card['properties']['id'] = {'type': 'string', 'enum': field_allowlists[dataset_id]}
            field_card['properties']['col'] = {'type': 'string', 'enum': field_allowlists[dataset_id]}
...
def _bootstrap_schema_material(value):
    ...
    if isinstance(value, dict):
        return {key: _bootstrap_schema_material(child) for key, child in value.items() if key != 'description'}
    return deepcopy(value)
...
def _compact_detail_field_ids(raw_values, raw_fields, dataset_id):
    ...
    for raw_field in raw_fields:
        ...
        for name in names:
            key = re.sub('\\s+', ' ', str(name or '').strip()).casefold()
            if key:
                lookup.setdefault(key, set()).add(field_id)
    resolved = []
    seen = set()
    for detail_index, raw_value in enumerate(raw_values or []):
        key = re.sub('\\s+', ' ', str(raw_value or '').strip()).casefold()
        candidates = sorted(lookup.get(key) or [])
        if len(candidates) != 1:
            raise ContractError('metadata_schema_error', 'metadata_authoring', '기본 상세 필드를 데이터셋 내부 canonical field 하나로 확정할 수 없습니다.', {'dataset_id': dataset_id, 'detail_index': detail_index, 'candidate_count': len(candidates), 'detail_value_sha256': hashlib.sha256(str(raw_value or '').encode('utf-8')).hexdigest()})
        field_id = candidates[0]
        if field_id in seen:
            raise ContractError('metadata_schema_error', 'metadata_authoring', '기본 상세 필드가 canonical field 기준으로 중복되었습니다.', {'dataset_id': dataset_id, 'detail_index': detail_index, 'field_id': field_id})
        seen.add(field_id)
...
def _validated_dataset_descriptor(dataset_id, raw_descriptor):
    ...
    id_pattern = re.compile('^[A-Za-z][A-Za-z0-9_.-]{0,127}$')
    semantic_type_pattern = re.compile('^[A-Za-z][A-Za-z0-9_]{0,63}$')
    allowed_descriptor_keys = {'physical_column', 'semantic_type', 'roles', 'physical_aliases', 'coercion', 'nullable', 'required_in_source', 'timezone'}
    if not isinstance(raw_descriptor, dict) or set(raw_descriptor) != {'family', 'fields', 'proposal_exclusions', 'dataset_template', 'dataset_template_sha256'}:
        raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 dataset descriptor 계약이 닫혀 있지 않습니다.', {'dataset_id': str(dataset_id)})
    ...
    if not id_pattern.fullmatch(family) or not isinstance(raw_fields, dict) or (not raw_fields) or (not isinstance(raw_exclusions, dict)) or (len(raw_exclusions) > 2048):
        raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 dataset family 또는 field descriptor가 유효하지 않습니다.', {'dataset_id': str(dataset_id)})
    fields = {}
    for field_id in sorted(raw_fields):
        raw_field = raw_fields.get(field_id)
        if not id_pattern.fullmatch(str(field_id)) or not isinstance(raw_field, dict) or (not {'physical_column', 'semantic_type', 'roles'} <= set(raw_field) <= allowed_descriptor_keys):
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 field descriptor의 ID 또는 key 계약이 유효하지 않습니다.', {'dataset_id': str(dataset_id), 'field_id': str(field_id)})
        ...
        if not physical_column or len(physical_column) > 256 or (not semantic_type_pattern.fullmatch(semantic_type)) or (not isinstance(roles, list)) or (not roles) or (len(roles) != len(set(roles))) or (not set(roles) <= set(AUTHORING_FIELD_ROLE_ORDER)) or (not isinstance(physical_aliases, list)) or (len(physical_aliases) != len(set(physical_aliases))) or any((not isinstance(value, str) or not value.strip() for value in physical_aliases)):
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 field descriptor의 물리 컬럼, 타입 또는 역할이 유효하지 않습니다.', {'dataset_id': str(dataset_id), 'field_id': str(field_id)})
        normalized = deepcopy(raw_field)
        normalized['physical_column'] = physical_column
        normalized['semantic_type'] = semantic_type
        normalized['roles'] = [role for role in AUTHORING_FIELD_ROLE_ORDER if role in roles]
        if physical_aliases:
            normalized['physical_aliases'] = list(physical_aliases)
        fields[str(field_id)] = normalized
    try:
        dataset_template = _validated_dataset_template(dataset_id, raw_descriptor.get('dataset_template'), raw_descriptor.get('dataset_template_sha256'), fields)
    except ValueError as exc:
        raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 dataset template 또는 hash가 유효하지 않습니다.', {'dataset_id': str(dataset_id)}) from exc
    approved_names = {_normalized_descriptor_name(name) for field_id, descriptor in fields.items() for name in (field_id, descriptor.get('physical_column'), *(descriptor.get('physical_aliases') or [])) if _normalized_descriptor_name(name)}
    exclusion_reasons = {'source_projection_not_registered', 'source_expression_before_alias', 'legacy_source_name'}
    proposal_exclusions = {}
    ...
    for raw_name in sorted(raw_exclusions):
        ...
        if not isinstance(policy, dict):
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 proposal exclusion policy는 object여야 합니다.', {'dataset_id': str(dataset_id)})
        reason = policy.get('reason_code')
        expected_policy_keys = {'reason_code', 'target_field_id'} if reason in {'source_expression_before_alias', 'legacy_source_name'} else {'reason_code'}
        if not isinstance(raw_name, str) or not raw_name.strip() or len(raw_name) > 256 or any((ord(character) < 32 for character in raw_name)) or (reason not in exclusion_reasons) or (set(policy) != expected_policy_keys) or (folded in folded_exclusions) or (folded in approved_names):
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 proposal exclusion 이름 또는 사유가 유효하지 않습니다.', {'dataset_id': str(dataset_id)})
        target_field_id = policy.get('target_field_id')
        if target_field_id is not None and target_field_id not in fields:
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '승인 proposal exclusion target field가 존재하지 않습니다.', {'dataset_id': str(dataset_id)})
        folded_exclusions.add(folded)
        proposal_exclusions[raw_name] = {key: policy[key] for key in sorted(policy)}
    return {'family': family, 'fields': fields, 'proposal_exclusions': proposal_exclusions, 'dataset_template': dataset_template, 'dataset_template_sha256': str(raw_descriptor.get('dataset_template_sha256') or '')}
...
def _resolve_compact_field_card(raw_field, dataset_descriptor, dataset_id, field_index):
    ...
    exclusion_lookup = {_normalized_descriptor_name(name): policy for name, policy in dataset_descriptor['proposal_exclusions'].items()}
    ...
    def fail_resolution(code, stage, message, candidate_count):
        raise ContractError(code, stage, message, {'dataset_id': str(dataset_id), 'field_index': field_index, 'candidate_count': candidate_count, 'field_id_sha256': hashlib.sha256(str(raw_field.get('id') or '').encode('utf-8')).hexdigest(), 'physical_column_sha256': hashlib.sha256(str(raw_field.get('col') or '').encode('utf-8')).hexdigest(), 'operator_action': 'review_source_registry_descriptor_or_exclusion'})
    if len(col_candidates) > 1 or len(id_candidates) > 1:
        fail_resolution('metadata_dependency_error', 'metadata_source_descriptors', 'LLM field card를 동일 dataset의 승인 descriptor 하나로 확정할 수 없습니다.', max(len(col_candidates), len(id_candidates)))
    ...
    if col_candidates:
        ...
        if id_candidates and field_id not in id_candidates:
            fail_resolution('metadata_dependency_error', 'metadata_source_descriptors', 'LLM field id와 물리 col이 서로 다른 승인 descriptor를 가리킵니다.', 0)
        binding_corrected = not id_candidates
    else:
        col_policy = exclusion_lookup.get(col_name)
        if col_policy is None:
            fail_resolution('metadata_registry_drift', 'metadata_source_registry', 'LLM이 제안한 물리 col이 승인 descriptor나 명시적 exclusion에 없습니다. 작업자가 아니라 운영자가 Source Registry를 검토해야 합니다.', 0)
        reason_code = str(col_policy.get('reason_code') or '')
        target_field_id = col_policy.get('target_field_id')
        applied_exclusion = {'name_sha256': hashlib.sha256(str(raw_field.get('col') or '').encode('utf-8')).hexdigest(), 'reason_code': reason_code, 'target_field_id': str(target_field_id or '')}
        if target_field_id is None:
            if id_candidates:
                fail_resolution('metadata_dependency_error', 'metadata_source_descriptors', '노출 제외된 물리 col을 승인 field id에 결합할 수 없습니다.', 0)
            return (None, False, applied_exclusion)
        field_id = str(target_field_id)
        if id_candidates and field_id not in id_candidates:
            fail_resolution('metadata_dependency_error', 'metadata_source_descriptors', '원천 alias exclusion target과 LLM field id가 충돌합니다.', 0)
    ...
    if not proposed_roles <= approved_roles:
        raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'LLM field role이 승인 descriptor 역할 범위를 벗어났습니다.', {'dataset_id': str(dataset_id), 'field_id': field_id})
    proposed_aliases = set(raw_field.get('physical_aliases') or [])
    approved_names = {str(field_id), str(descriptor.get('physical_column') or ''), *[str(value) for value in descriptor.get('physical_aliases') or []]}
    if proposed_aliases and (not proposed_aliases <= approved_names):
        raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'LLM physical alias가 승인 descriptor 범위를 벗어났습니다.', {'dataset_id': str(dataset_id), 'field_id': field_id})
    for key in ('coercion', 'timezone'):
        if key in raw_field and raw_field.get(key) != descriptor.get(key):
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'LLM 기술 field 속성이 승인 descriptor와 충돌합니다.', {'dataset_id': str(dataset_id), 'field_id': field_id, 'attribute': key})
    if raw_field.get('required_in_source') is True and descriptor.get('required_in_source') is not True:
        raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'LLM required_in_source가 승인 descriptor와 충돌합니다.', {'dataset_id': str(dataset_id), 'field_id': field_id})
    if raw_field.get('nullable') is False and descriptor.get('nullable') is not False:
        raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'LLM nullable이 승인 descriptor와 충돌합니다.', {'dataset_id': str(dataset_id), 'field_id': field_id})
...
def _merge_equivalent_compact_field_cards(existing, incoming, dataset_id, field_id):
    ...
    for key in sorted(additive_list_keys):
        ...
        if not isinstance(left, list) or not isinstance(right, list):
            raise ContractError('metadata_schema_error', 'metadata_authoring', '동일 승인 field의 선택 목록 속성을 병합할 수 없습니다.', {'dataset_id': str(dataset_id), 'field_id': str(field_id), 'attribute': key})
        if left or right:
            merged[key] = sorted({str(value) for value in [*left, *right] if str(value)})
    for key in sorted(bounded_list_keys):
        if key not in incoming:
            continue
        right = incoming.get(key)
        if not isinstance(right, list):
            raise ContractError('metadata_schema_error', 'metadata_authoring', '동일 승인 field의 허용 목록 속성을 병합할 수 없습니다.', {'dataset_id': str(dataset_id), 'field_id': str(field_id), 'attribute': key})
        if key in merged:
            left = merged.get(key)
            if not isinstance(left, list) or set(left) != set(right):
                raise ContractError('metadata_schema_error', 'metadata_authoring', '동일 승인 field의 허용 목록이 서로 충돌합니다.', {'dataset_id': str(dataset_id), 'field_id': str(field_id), 'attribute': key})
        merged[key] = sorted({str(value) for value in right if str(value)})
    for key in sorted(scalar_keys):
        if key not in incoming:
            continue
        if key in merged and merged.get(key) != incoming.get(key):
            raise ContractError('metadata_schema_error', 'metadata_authoring', '동일 승인 field의 선택 속성이 서로 충돌합니다.', {'dataset_id': str(dataset_id), 'field_id': str(field_id), 'attribute': key})
        merged[key] = deepcopy(incoming[key])
    return merged

_FREEFORM_FIELD_ROLES = {'filter', 'group', 'join', 'compare', 'aggregate', 'derive', 'project', 'sort', 'rank', 'metric', 'output'}

def _freeform_csv_values(value):
    return [item.strip(" \t\r\n.。'") for item in re.split('\\s*,\\s*', str(value or '')) if item.strip(" \t\r\n.。'")]

def _freeform_filter_mappings(source_text):
    """Read canonical -> physical mappings directly from worker text."""
    text = str(source_text or '').replace('\r\n', '\n').replace('\r', '\n')
    marker = re.search('(?i)(?<![A-Za-z0-9_])filter_mappings(?![A-Za-z0-9_])', text)
    if marker is None:
        return {}
    tail = text[marker.end():]
    end = re.search('(?im)^\\s*(?:standard_column_aliases|default_detail_columns|metric_semantics|selection_criteria|required_params|required_param_mappings|query_template)\\s*(?:[:=]|(?:은|는)\\b)', tail)
    if end:
        tail = tail[:end.start()]
    mappings = {}
    for match in re.finditer('(?<![A-Za-z0-9_.-])([A-Za-z][A-Za-z0-9_.-]{0,127})\\s*[-=]>\\s*([A-Za-z][A-Za-z0-9_$#.-]{0,255})', tail):
        canonical, physical = (match.group(1), match.group(2))
        if canonical in mappings and mappings[canonical] != physical:
            raise ContractError('metadata_schema_error', 'metadata_dataset_mapping', 'filter_mappings에 같은 표준 필드의 물리 컬럼이 두 개 이상 지정되었습니다.', {'field_id': canonical})
...
def _freeform_query_columns(source_text):
    ...
    neutral = re.sub('/\\*.*?\\*/', ' ', query, flags=re.DOTALL)
    neutral = re.sub('(?m)--[^\\n]*$', ' ', neutral)
    select_match = re.search('(?is)\\bSELECT\\b(.*?)\\bFROM\\b', neutral)
    ...
    for expression in select_match.group(1).split(','):
        ...
        if not value:
            continue
        alias = re.search('(?i)\\bAS\\s+([A-Za-z][A-Za-z0-9_$#.-]{0,255})\\s*$', value)
        if alias:
            name = alias.group(1)
        else:
            token = value.split()[-1]
            name = token.rsplit('.', 1)[-1].strip('"')
        if re.fullmatch('[A-Za-z][A-Za-z0-9_$#.-]{0,255}', name) and name not in columns:
            columns.append(name)
    return columns
...
def _freeform_selection_criteria(source_text):
    text = str(source_text or '').replace('\r\n', '\n').replace('\r', '\n')
    result = {}
    scope = re.search('(?i)selection_criteria(?:의|\\.)?\\s*time_scope\\s*(?:은|는|:|=)\\s*([A-Za-z][A-Za-z0-9_.-]{0,63})', text)
    if scope:
    ...
    use_when = re.search('(?is)selection_criteria(?:의|\\.)?\\s*use_when\\s*(?:은|는|:|=)\\s*(.*?)\\s*(?:이고|이며|,?\\s*exclude_when\\s*(?:은|는|:|=))', text)
    exclude_when = re.search('(?is)exclude_when\\s*(?:은|는|:|=)\\s*(.*?)(?:이야|입니다|해\\.?|\\n\\s*조회|\\n\\s*filter_mappings|$)', text)
    if use_when:
...
def _freeform_dataset_family(source_text, dataset_id):
    text = str(source_text or '')
    patterns = (f'(?i)(?<![A-Za-z0-9_]){re.escape(dataset_id)}(?![A-Za-z0-9_]).{{0,80}}?([A-Za-z][A-Za-z0-9_.-]{{0,127}})\\s*계열', '(?i)(?:dataset_)?family\\s*(?:은|는|:|=)\\s*([A-Za-z][A-Za-z0-9_.-]{0,127})')
...
def _freeform_source_type(source_text):
    ...
    markers = (('oracle', 'oracle'), ('datalake', 'datalake'), ('goodocs', 'goodocs'), ('mongodb', 'mongodb'), ('h-api', 'http'), ('http', 'http'))
...
def _freeform_field_semantic_type(field_id, physical_column, source_text, proposed=''):
    ...
    identifier_tokens = set(re.findall('[A-Z0-9]+', marker))
    text = str(source_text or '')
    if identifier_tokens & {'DATE', 'DT', 'DATETIME', 'TIMESTAMP', 'TIME', 'AT'}:
        return 'date'
    if identifier_tokens & {'QTY', 'QUANTITY', 'COUNT', 'CNT', 'AMOUNT', 'PRODUCTION', 'WIP', 'PRESS', 'SEQ', 'TAT', 'UPH', 'RATE', 'YIELD'}:
        return 'number'
    identifiers = [str(field_id or '').strip(), str(physical_column or '').strip()]
    for sentence in re.split('[\\r\\n.!?]+', text):
        if not any((identifier and re.search(f'(?<![A-Za-z0-9_]){re.escape(identifier)}(?![A-Za-z0-9_])', sentence, re.IGNORECASE) for identifier in identifiers)):
            continue
        if re.search('YYYY[- ]?MM[- ]?DD|날짜|기준일|일자|시각|시간', sentence, re.IGNORECASE):
            return 'date'
        if re.search('수량|생산량|건수|합계|개수|대수|평균|비율', sentence):
            return 'number'
    return 'string'
...
def _freeform_field_roles(field_id, physical_column, explicit_mappings, semantic_type, proposed=None):
    ...
    if semantic_type in {'number', 'integer', 'decimal', 'float'} and re.search('(?:QTY|QUANTITY|COUNT|CNT|AMOUNT|PRODUCTION|WIP|UPH|TAT|RATE|YIELD)', f'{field_id} {physical_column}', re.IGNORECASE):
...
def _freeform_dataset_source_segment(source_text, dataset_id):
    ...
    """
    text = str(source_text or '')
    marker = re.compile('(?im)^[^\\r\\n]*?(?<![A-Za-z0-9_.-])([A-Za-z][A-Za-z0-9_.-]{0,127})\\s*(?:으)?로\\s*등록[^\\r\\n]*$')
...
def _expand_freeform_dataset_fragment(fragment, source_text, reconciliation_out=None):
    ...
    if not isinstance(fragment, dict) or set(fragment) != {'dataset_cards'}:
        raise ContractError('metadata_schema_error', 'metadata_authoring', '테이블 카탈로그 등록 결과는 dataset_cards 목록이어야 합니다.')
    cards = fragment.get('dataset_cards')
    if not isinstance(cards, list) or not cards:
        raise ContractError('metadata_schema_error', 'metadata_authoring', '등록할 데이터셋 항목이 없습니다.')
    ...
    for card_index, raw_card in enumerate(cards):
        ...
        dataset_id = explicit_dataset_id if len(cards) == 1 and explicit_dataset_id else proposed_dataset_id
        if proposed_dataset_id and proposed_dataset_id != dataset_id:
            dataset_identity_overrides.append({'proposed': proposed_dataset_id, 'source_authority': dataset_id})
        if not re.fullmatch('[A-Za-z][A-Za-z0-9_.-]{0,127}', dataset_id) or dataset_id in datasets:
            raise ContractError('metadata_schema_error', 'metadata_authoring', 'dataset_id가 없거나 중복되었습니다.', {'dataset_index': card_index, 'dataset_id': dataset_id})
        ...
        reverse_mapping = {physical.casefold(): canonical for canonical, physical in explicit_mappings.items()}
        total_filter_mappings += len(explicit_mappings)
        total_query_columns += len(query_columns)
        selection_criteria_from_text = selection_criteria_from_text or bool(selection_from_text)
        family = str(_freeform_dataset_family(dataset_source_text, dataset_id) or dataset_id).strip()
        source_type = str(_freeform_source_type(dataset_source_text) or raw_card.get('source_type') or '').strip().casefold()
        if not family or source_type not in {'oracle', 'sql', 'mongodb', 'http', 'datalake', 'goodocs', 'file', 'dummy', 'previous_result'}:
            raise ContractError('metadata_clarification_required', 'metadata_clarification', '데이터 계열 또는 원천 종류를 자연어 설명에서 확인할 수 없습니다.', {'questions': ['데이터셋의 계열 이름과 원천 종류를 알려주세요.'], 'missing_fields': ['데이터 계열 또는 원천 종류']})
        model_fields = {}
        for field_index, raw_field in enumerate(raw_card.get('fields') or []):
            ...
            if not re.fullmatch('[A-Za-z][A-Za-z0-9_.-]{0,127}', field_id) or not physical:
                raise ContractError('metadata_schema_error', 'metadata_authoring', 'field card의 id 또는 col이 올바르지 않습니다.', {'dataset_id': dataset_id, 'field_index': field_index})
            explicitly_grounded = field_id in explicit_mappings or physical.casefold() in query_column_set or re.search(f'(?<![A-Za-z0-9_])(?:{re.escape(field_id)}|{re.escape(physical)})(?![A-Za-z0-9_])', dataset_source_text, re.IGNORECASE) is not None
            ...
            if field_id in model_fields and model_fields[field_id]['physical_column'] != physical:
                raise ContractError('metadata_schema_error', 'metadata_dataset_mapping', '같은 표준 필드가 서로 다른 물리 컬럼으로 제안되었습니다.', {'dataset_id': dataset_id, 'field_id': field_id})
            semantic_type = _freeform_field_semantic_type(field_id, physical, dataset_source_text, raw_field.get('semantic_type'))
            if str(raw_field.get('semantic_type') or '').strip() and str(raw_field.get('semantic_type') or '').strip().casefold() != semantic_type.casefold():
                discarded_model_semantic_types += 1
            binding = {'physical_column': physical, 'semantic_type': semantic_type, 'roles': _freeform_field_roles(field_id, physical, explicit_mappings, semantic_type, raw_field.get('roles'))}
        ...
        for canonical, physical in explicit_mappings.items():
            if canonical not in model_fields:
                semantic_type = _freeform_field_semantic_type(canonical, physical, dataset_source_text)
                model_fields[canonical] = {'physical_column': physical, 'semantic_type': semantic_type, 'roles': _freeform_field_roles(canonical, physical, explicit_mappings, semantic_type)}
        ...
        for physical in query_columns:
            ...
            if canonical not in model_fields:
                ...
                if aliases:
                    model_fields[canonical]['aliases'] = aliases
        if not model_fields:
            raise ContractError('metadata_clarification_required', 'metadata_clarification', '조회 결과 컬럼 또는 필터 매핑을 확인할 수 없습니다.', {'questions': ['조회 결과 컬럼이나 필터 매핑을 자연어로 알려주세요.'], 'missing_fields': ['조회 컬럼']})
        ...
        placeholders = list(dict.fromkeys((match.group(1).strip() for match in re.finditer('\\{([A-Za-z][A-Za-z0-9_]*)\\}', dataset_source_text))))
        ...
        for name in placeholders:
            ...
            if str(binding.get('semantic_type') or '').casefold() in {'localdate', 'date'}:
                spec['type'] = 'LocalDate'
                compact_pattern = re.compile(f'(?<![A-Za-z0-9_]){re.escape(name)}(?![A-Za-z0-9_]).{{0,80}}?YYYYMMDD\\s*형식', re.IGNORECASE | re.DOTALL)
                dashed_pattern = re.compile(f'(?<![A-Za-z0-9_]){re.escape(name)}(?![A-Za-z0-9_]).{{0,80}}?YYYY-MM-DD\\s*형식', re.IGNORECASE | re.DOTALL)
                if compact_pattern.search(dataset_source_text):
                    spec['format'] = 'YYYYMMDD'
                elif dashed_pattern.search(dataset_source_text):
                    spec['format'] = 'YYYY-MM-DD'
                if date_policy is None:
                    date_policy = {'field': name, 'inclusive_start': True, 'inclusive_end': True, 'timezone': 'Asia/Seoul'}
            else:
                spec.setdefault('type', 'string')
            spec['required'] = True
            parameters[name] = spec
        dataset = {'display_name': str(raw_card.get('display_name') or dataset_id), 'family': family, 'source_type': source_type, 'time_scope': time_scope, 'selection_criteria': selection, 'fields': {key: model_fields[key] for key in sorted(model_fields)}}
        if parameters:
            dataset['parameters'] = parameters
    ...
    evidence = {'contract_version': 'metadata.freeform-dataset-expansion.v1', 'dataset_count': len(datasets), 'field_count': sum((len(item['fields']) for item in datasets.values())), 'filter_mapping_count': total_filter_mappings, 'query_column_count': total_query_columns, 'selection_criteria_from_text': selection_criteria_from_text, 'dataset_identity_overrides': dataset_identity_overrides, 'discarded_model_semantic_type_count': discarded_model_semantic_types}
...
def _freeform_recipe_identity(key, payload, source_text=''):
    ...
    if not explicit and source_text:
        matches = re.findall('(?:recipe|레시피)\\s*(?:를|을)?\\s*(?:as\\s+)?([A-Za-z0-9_.-]+)\\s*(?:로|으로)?', str(source_text), flags=re.IGNORECASE)
...
def _freeform_recipe_keys(payload, source_text=''):
    ...
    if not candidates:
        candidates = (payload or {}).get('keys') or (payload or {}).get('grain_keys') or (payload or {}).get('product_keys') or []
    if not candidates and source_text:
        match = re.search('(?:grain\\s*)?(?:keys?|key|키)(?:\\s*목록)?\\s*(?:은|는|:|=|is)\\s*([^\\r\\n.]+)', str(source_text), flags=re.IGNORECASE)
        if match:
            candidates = re.split('\\s*(?:,|→|->|/|그리고)\\s*', match.group(1))
    return list(dict.fromkeys((str(value).strip().rstrip('이야입니다') for value in candidates if re.fullmatch('[A-Za-z][A-Za-z0-9_]*', str(value).strip().rstrip('이야입니다')))))
...
def _normalize_freeform_recipe(key, payload, source_text=''):
    ...
    if isinstance(raw_grain, str):
        ...
        grain_ref = str(raw_grain.get('entity_id') or raw_grain.get('grain_ref') or raw_grain.get('grain_id') or '').strip()
    if not grain_ref and source_text:
        match = re.search('grain\\s*(?:은|는|:|=|is)\\s*([A-Za-z][A-Za-z0-9_.-]*)', str(source_text), flags=re.IGNORECASE)
        if match:
            grain_ref = match.group(1)
    if not isinstance(template, dict) and keys:
        template = {'op': 'aggregate', 'group_by': keys, 'metrics': [{'as_ref': '$metric.id', 'field_ref': '$metric.field', 'function_ref': '$metric.rollup'}]}
    slot_aliases = {'dataset': 'dataset_refs', 'metric': 'metric_refs', 'grain': 'grain_refs', 'field': 'field_refs', 'relation': 'relation_refs', 'recipe': 'recipe_refs', 'entity_group': 'entity_group_refs', 'filter': 'filter_refs'}

    def _clean_alias(value):
        text = str(value or '').strip()
        return re.sub('(?:이야|예요|입니다|야)$', '', text).strip()
    recipe = {'recipe_id': recipe_id, 'aliases': list(dict.fromkeys((_clean_alias(value) for value in (payload or {}).get('aliases') or [] if _clean_alias(value)))), 'required_slots': list(dict.fromkeys((slot_aliases.get(str(value).strip(), str(value).strip()) for value in (payload or {}).get('required_slots') or ['dataset', 'metric'] if str(value).strip())))}
    if isinstance(template, dict):
        recipe['default_operation_template'] = template
    if keys:
        recipe['grain'] = {'entity_id': grain_ref or recipe_id.split('.', 1)[0], 'grain_id': recipe_id, 'keys': keys, 'null_match_policy': str((raw_grain or {}).get('null_match_policy') if isinstance(raw_grain, dict) else 'blank_equals_blank') or 'blank_equals_blank'}
...
def _expand_freeform_domain_fragment(fragment, reconciliation_out=None, source_text=''):
    ...
    has_business_item = any((isinstance(item, dict) and str(item.get('section') or '') != 'profile' for item in raw_items))
    ...
    for index, item in enumerate(raw_items):
        ...
        if section == 'entity_groups':
            ...
            group = {'group_id': key, 'display_name': str(payload.get('display_name') or key), 'target_field': target_field, 'members': [re.sub('(?:입니다|이에요|예요|이야|야)$', '', str(value).strip()).strip() for value in members or [] if str(value).strip()], 'aliases': [str(value).strip() for value in aliases if str(value).strip()]}
            if not group['target_field'] or not group['members'] or (not group['aliases']):
                raise ContractError('metadata_clarification_required', 'metadata_clarification', '공정 그룹의 적용 필드, 포함 값 또는 별칭이 부족합니다.', {'questions': ['공정 그룹의 적용 필드, 포함 값, 별칭을 알려주세요.'], 'missing_fields': ['공정 그룹 설명']})
            patch.setdefault(section, {})[key] = group
            continue
        if section == 'metrics':
            source_binding = deepcopy(payload.get('source_binding')) if isinstance(payload.get('source_binding'), dict) else {}
            dataset_family = str(source_binding.get('dataset_family') or payload.get('dataset_family') or '').strip()
            source_field = str(source_binding.get('field') or payload.get('source_field') or '').strip()
            if not dataset_family and source_text:
                match = re.search('([A-Za-z][A-Za-z0-9_.-]*)\\s*(?:계열|family)', str(source_text), flags=re.IGNORECASE)
                if match:
                    dataset_family = match.group(1)
            if not source_field and source_text:
                match = re.search('([A-Za-z][A-Za-z0-9_]*)\\s*(?:컬럼|column|field)', str(source_text), flags=re.IGNORECASE)
                if match:
            ...
            if dataset_family and source_field:
                source_binding = {'dataset_family': dataset_family, 'field': source_field}
            ...
            allowed_rollups = [str(value).strip().casefold() for value in allowed_rollups or [] if str(value).strip()]
            default_rollup = str((payload.get('additivity') or {}).get('default') if isinstance(payload.get('additivity'), dict) else payload.get('default_aggregation') or payload.get('aggregation') or '').strip().casefold()
            if isinstance(payload.get('additivity'), dict):
                ...
                default_kind = 'distinct' if default_rollup in {'count_distinct', 'nunique', 'list_unique'} else 'non_additive' if default_rollup in {'mean', 'avg', 'average', 'min', 'max'} else 'additive'
                additivity = {'default': default_kind, 'allowed_rollups': list(dict.fromkeys(allowed_rollups or ['sum']))}
            metric = {'metric_id': key, 'value_type': str(payload.get('value_type') or 'number'), 'unit': str(payload.get('unit') or 'count'), 'null_policy': str(payload.get('null_policy') or ('exclude_from_sum' if payload.get('exclude_nulls') is not False else 'preserve_null')), 'zero_policy': str(payload.get('zero_policy') or 'preserve_zero'), 'additivity': additivity}
        ...
        if section == 'grains':
            ...
            grain = {'grain_id': key, 'keys': [re.sub('(?:입니다|이에요|예요|이야|야)$', '', str(value).strip()).strip() for value in keys or [] if str(value).strip()]}
            display_fields = payload.get('display_fields')
            if isinstance(display_fields, list) and display_fields:
                grain['display_fields'] = [str(value).strip() for value in display_fields if str(value).strip()]
    ...
    if isinstance(reconciliation_out, dict):
        reconciliation_out.clear()
        reconciliation_out.update({'contract_version': 'metadata.freeform-domain-expansion.v1', 'item_count': len(seen) - discarded_profile_items, 'discarded_redundant_profile_count': discarded_profile_items})
    return patch
...
def _expand_freeform_main_filter_fragment(fragment):
    ...
    for index, item in enumerate(fragment.get('items') or []):
        ...
        if not filter_key or filter_key in seen or (not aliases):
            raise ContractError('metadata_schema_error', 'metadata_authoring', '메인 필터 key가 중복되었거나 별칭이 없습니다.', {'item_index': index, 'filter_key': filter_key})
        seen.add(filter_key)
...
def _expand_compact_dataset_fragment(fragment, dataset_descriptors=None, reconciliation_out=None):
    ...
    for dataset_index, raw_card in enumerate(fragment.get('dataset_cards') or []):
        ...
        if dataset_id in datasets:
            raise ContractError('metadata_schema_error', 'metadata_authoring', '압축 데이터셋 등록 IR에 중복 dataset_id가 있습니다.', {'dataset_id': dataset_id, 'dataset_index': dataset_index})
        if dataset_id not in dataset_descriptors:
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', '동일 dataset의 승인 source descriptor가 없습니다.', {'dataset_id': dataset_id})
        descriptor = _validated_dataset_descriptor(dataset_id, dataset_descriptors[dataset_id])
        applied_dataset_templates.append({'dataset_id': dataset_id, 'dataset_template_sha256': descriptor['dataset_template_sha256']})
        ...
        for field_index, raw_field in enumerate(raw_card['fields']):
            field_id, binding_corrected, applied_exclusion = _resolve_compact_field_card(raw_field, descriptor, dataset_id, field_index)
            if applied_exclusion is not None:
                applied_exclusion_rows.append({'dataset_id': dataset_id, **applied_exclusion})
            if field_id is None:
                model_field_count += 1
                continue
            if binding_corrected:
                corrected_binding_pairs.append({'dataset_id': dataset_id, 'field_id': field_id, 'proposed_id_sha256': hashlib.sha256(str(raw_field.get('id') or '').encode('utf-8')).hexdigest(), 'proposed_column_sha256': hashlib.sha256(str(raw_field.get('col') or '').encode('utf-8')).hexdigest()})
            if field_id in resolved_model_fields:
                resolved_model_fields[field_id] = _merge_equivalent_compact_field_cards(resolved_model_fields[field_id], raw_field, dataset_id, field_id)
            else:
        ...
        datasets[dataset_id] = dataset
    evidence = {'contract_version': 'metadata.dataset-registry-reconciliation.v1', 'approved_field_count': approved_field_count, 'model_field_count': model_field_count, 'completed_field_count': len(completed_pairs), 'applied_exclusion_count': len(applied_exclusion_rows), 'applied_exclusions_sha256': sha256_json(sorted(applied_exclusion_rows, key=lambda item: (item['dataset_id'], item['name_sha256'], item['reason_code'], item['target_field_id']))), 'corrected_binding_count': len(corrected_binding_pairs), 'corrected_bindings_sha256': sha256_json(sorted(corrected_binding_pairs, key=lambda item: (item['dataset_id'], item['field_id'], item['proposed_id_sha256'], item['proposed_column_sha256']))), 'compiler_owned_dataset_template_count': len(applied_dataset_templates), 'compiler_owned_dataset_templates_sha256': sha256_json(sorted(applied_dataset_templates, key=lambda item: item['dataset_id'])), 'completed_fields_sha256': sha256_json(sorted(completed_pairs, key=lambda item: (item['dataset_id'], item['field_id']))), 'normalized_fields_sha256': sha256_json(sorted(normalized_pairs, key=lambda item: (item['dataset_id'], item['field_id'])))}
    if isinstance(reconciliation_out, dict):
        reconciliation_out.clear()
        reconciliation_out.update(evidence)
    return {'datasets': {key: datasets[key] for key in sorted(datasets)}}

def _validate_bootstrap_fragment(fragment, kind, dataset_descriptors=None, semantic_vocabulary=None, reconciliation_out=None, main_filter_reconciliation_out=None, domain_already_expanded=False):
    if kind == 'domain' and domain_already_expanded:
        ...
        if not isinstance(fragment, dict) or set(fragment) != expected:
            raise ContractError('metadata_schema_error', 'metadata_semantic_templates', '승인 도메인 템플릿 확장 결과의 root 계약이 닫혀 있지 않습니다.')
        return deepcopy(fragment)
    schema = _bootstrap_fragment_draft_schema(kind)
    errors = sorted(Draft202012Validator(schema).iter_errors(fragment), key=lambda item: list(item.absolute_path))
    if errors:
        first = errors[0]
        path = '.'.join((str(part) for part in first.absolute_path)) or '$'
        raise ContractError('metadata_schema_error', 'metadata_authoring', '분할 초기 등록 제안 조각이 폐쇄형 소유권 계약과 일치하지 않습니다.', {'authoring_branch': kind, 'path': path, 'reason': first.message[:400]})
    if kind == 'dataset':
        return _expand_compact_dataset_fragment(fragment, dataset_descriptors=dataset_descriptors, reconciliation_out=reconciliation_out)
    if kind == 'main_filter':
        return _expand_compact_main_filter_fragment(fragment, approved_semantic_vocabulary=semantic_vocabulary, reconciliation_out=main_filter_reconciliation_out)
    return deepcopy(fragment)

def _merge_bootstrap_fragments(fragments, dataset_descriptors=None, semantic_vocabulary=None, reconciliation_out=None, main_filter_reconciliation_out=None, domain_already_expanded=False):
    merged = {'contract_version': 'metadata.authoring.draft.v1'}
    section_owners = {}
    merged_aliases = {}
    alias_owners = {}
    for kind in BOOTSTRAP_BRANCH_ORDER:
        fragment = _validate_bootstrap_fragment(fragments.get(kind), kind, dataset_descriptors=dataset_descriptors if kind == 'dataset' else None, semantic_vocabulary=semantic_vocabulary if kind == 'main_filter' else None, reconciliation_out=reconciliation_out if kind == 'dataset' else None, main_filter_reconciliation_out=main_filter_reconciliation_out if kind == 'main_filter' else None, domain_already_expanded=bool(domain_already_expanded) if kind == 'domain' else False)
            ...
            if section == 'aliases':
                for alias in sorted(value):
                    if alias in merged_aliases and merged_aliases[alias] != value[alias]:
                        merged_aliases[alias] = _merge_natural_alias_card(merged_aliases[alias], value[alias], alias)
                    if alias not in merged_aliases:
                        merged_aliases[alias] = deepcopy(value[alias])
                        alias_owners[alias] = kind
                continue
            if section in merged:
                raise ContractError('metadata_schema_error', 'metadata_authoring', '분할 초기 등록 root section 소유권이 충돌합니다.', {'section': section, 'first_branch': section_owners[section], 'second_branch': kind})
            merged[section] = deepcopy(value)
            section_owners[section] = kind
    if merged_aliases:
        baseline_aliases = fragments.get('domain', {}).get('aliases') if domain_already_expanded and isinstance(fragments.get('domain'), dict) else {}
        merged_aliases, alias_conflict_resolution = _resolve_natural_alias_conflicts(merged_aliases, baseline_aliases if isinstance(baseline_aliases, dict) else {})
        if isinstance(main_filter_reconciliation_out, dict):
            main_filter_reconciliation_out['alias_conflict_resolution'] = alias_conflict_resolution
        merged['aliases'] = {alias: merged_aliases[alias] for alias in sorted(merged_aliases)}
    return merged

def _expand_bootstrap_domain_annotation(annotation, *, semantic_templates, semantic_vocabulary):
    ...
    annotation_errors = sorted(Draft202012Validator(annotation_schema).iter_errors(annotation), key=lambda item: (list(item.absolute_path), item.message))
    if annotation_errors:
        exc = annotation_errors[0]
        raise ContractError('metadata_schema_error', 'metadata_authoring', '도메인 자연어 제안은 표시 이름과 설명만 포함해야 합니다.', {'path': list(exc.absolute_path)})
    try:
        templates = _validated_semantic_templates(semantic_templates, semantic_vocabulary)
    except ValueError as exc:
        raise ContractError('metadata_dependency_error', 'metadata_semantic_templates', '승인된 도메인 의미 템플릿이 유효하지 않습니다.') from exc
    fragment = {'display_name': str(annotation['display_name']), 'description': str(annotation['description']), 'locale': str(templates['locale']), 'timezone': str(templates['timezone']), **{section: deepcopy(templates[section]) for section in _SEMANTIC_TEMPLATE_SECTIONS}, 'aliases': deepcopy(templates['aliases']), 'output_profile': deepcopy(templates['planner_policy'])}
    evidence = {'contract_version': 'metadata.domain-template-expansion.v1', 'template_contract_version': templates['contract_version'], 'annotation_sha256': sha256_json(annotation), 'semantic_templates_sha256': sha256_json(templates), 'planner_policy_sha256': sha256_json(templates['planner_policy']), 'section_counts': {section: len(fragment[section]) for section in (*_SEMANTIC_TEMPLATE_SECTIONS, 'aliases')}}
...
def _deterministic_registry_bootstrap_fragments(registry_context):
    ...
    if not isinstance(descriptors, dict) or not isinstance(vocabulary, dict):
        raise ContractError('metadata_dependency_error', 'metadata_registry_bootstrap', '승인 레지스트리에서 초기 Dataset/Main Filter 조각을 만들 수 없습니다.')
    dataset_cards = []
    for dataset_id in sorted(descriptors):
        descriptor = _validated_dataset_descriptor(dataset_id, descriptors[dataset_id])
        dataset_cards.append({'dataset_id': dataset_id, 'fields': [{'id': field_id, 'col': str(descriptor['fields'][field_id]['physical_column'])} for field_id in sorted(descriptor['fields'])]})
    target_types = {'datasets': 'dataset', 'fields': 'field', 'metrics': 'metric', 'relations': 'relation', 'grains': 'grain', 'predicates': 'predicate', 'recipes': 'recipe', 'entity_groups': 'entity_group'}
    alias_additions = []
    for section, target_type in target_types.items():
        for card in vocabulary.get(section) or []:
            ...
            expressions = sorted({str(value).strip() for value in card.get('labels') or [] if str(value).strip()})
            if target_id and expressions:
                alias_additions.append({'target_type': target_type, 'target_id': target_id, 'expressions': expressions})
    if not dataset_cards or not alias_additions:
        raise ContractError('metadata_dependency_error', 'metadata_registry_bootstrap', '승인 레지스트리의 초기 Dataset 또는 Main Filter 목록이 비어 있습니다.')
    return ({'dataset_cards': dataset_cards}, {'alias_additions': alias_additions})
    ...
    if expected_purpose == 'metadata_main_filter_draft' and root_properties == {'items'}:
        ...
        if isinstance(additions, list) and set(draft) == {'alias_additions'}:
            ...
            for addition in additions:
                if not isinstance(addition, dict) or addition.get('target_type') != 'field' or (not str(addition.get('target_id') or '')) or (not isinstance(addition.get('expressions'), list)):
                    return current
                items.append({'filter_key': str(addition['target_id']), 'payload': {'aliases': deepcopy(addition['expressions'])}})
...
def _freeform_sentence_value(source_text, labels):
    ...
    match = re.search(f'(?im)(?:{label_pattern})\\s*(?:은|는|:|=)\\s*([^\\r\\n.]+)', str(source_text or ''))
    if match is None:
        return ''
    value = match.group(1).strip(' \t\r\n.\'"')
    value = re.split('\\s*(?:이고|이며)\\s+(?=유의어|별칭|설명|description)', value, maxsplit=1)[0]
    value = re.sub('\\s*(?:이면\\s*(?:돼|됩니다?)|이면\\s*돼요|이야|입니다?|로\\s*(?:해줘|저장해줘))\\s*$', '', value).strip()
    return value

def _freeform_dataset_id(source_text):
    matches = re.findall('(?im)(?<![A-Za-z0-9_.-])([A-Za-z][A-Za-z0-9_.-]{0,127})\\s*(?:으)?로\\s*등록(?:해줘|합니다?|하세요|할게)?', str(source_text or ''))
    values = list(dict.fromkeys(matches))
    return values[0] if len(values) == 1 else ''

def _freeform_domain_group_card(source_text):
    text = str(source_text or '')
    group_match = re.search('(?im)(?<![A-Za-z0-9_.-])([A-Za-z][A-Za-z0-9_.-]{0,127})\\s*공정\\s*그룹(?:을|를)?\\s*등록', text)
    field_match = re.search('(?im)(?<![A-Za-z0-9_])field(?![A-Za-z0-9_])\\s*(?:은|는|:|=)\\s*([A-Za-z][A-Za-z0-9_.-]{0,127})', text)
    aliases_match = re.search('(?im)유의어\\s*(?:은|는|:|=)\\s*([^\\r\\n.]+)', text)
    members_match = re.search('(?im)포함\\s*공정\\s*(?:은|는|:|=)\\s*(?:[A-Za-z][A-Za-z0-9_.-]{0,127}\\s*값\\s*)?([^\\r\\n.]+)', text)
    if not all((group_match, field_match, aliases_match, members_match)):
        return None
    group_id = group_match.group(1)
    aliases = _freeform_csv_values(re.sub('\\s*(?:이야|입니다?|로\\s*저장해줘)\\s*$', '', aliases_match.group(1)))
    members = _freeform_csv_values(re.sub('\\s*(?:이야|입니다?|로\\s*저장해줘)\\s*$', '', members_match.group(1)))
    if not aliases or not members:
        return None
    display_name = _freeform_sentence_value(text, ('display_name', '화면(?:에\\s*보일)?\\s*이름')) or group_id
    return {'section': 'entity_groups', 'key': group_id, 'payload': {'display_name': display_name, 'field': field_match.group(1), 'processes': members, 'aliases': aliases}}

def _freeform_domain_profile_card(source_text):
    text = str(source_text or '')
    match = re.search('(?im)(?<![A-Za-z0-9_.-])([A-Za-z][A-Za-z0-9_.-]{0,127})\\s*도메인(?:으)?로\\s*등록', text)
    if match is None:
        return None
    domain_id = match.group(1)
    display_name = _freeform_sentence_value(text, ('display_name', '화면(?:에\\s*보일)?\\s*이름')) or domain_id
    payload = {'display_name': display_name}
    locale = re.search('(?im)언어\\s*(?:은|는|:|=)\\s*([A-Za-z]{2}-[A-Za-z]{2})', text)
    timezone_match = re.search('(?im)시간대\\s*(?:은|는|:|=)\\s*([A-Za-z]+/[A-Za-z_]+)', text)
    if locale:
...
def _freeform_domain_grain_card(source_text):
    text = str(source_text or '')
    if re.search('(?:recipe|레시피)', text, flags=re.IGNORECASE):
        return None
    match = re.search('(?<![A-Za-z0-9_.-])([A-Za-z][A-Za-z0-9_.-]{0,127})\\s*grain', text, flags=re.IGNORECASE)
    keys = _freeform_recipe_keys({}, text)
    if match is None or not keys:
        return None
    return {'section': 'grains', 'key': match.group(1), 'payload': {'grain_keys': keys}}

def _freeform_domain_recipe_card(source_text):
    text = str(source_text or '')
    if not re.search('(?:recipe|레시피)', text, flags=re.IGNORECASE):
        return None
    recipe_id = _freeform_recipe_identity('', {}, text)
    keys = _freeform_recipe_keys({}, text)
    grain_match = re.search('grain\\s*(?:은|는|:|=|is)\\s*([A-Za-z][A-Za-z0-9_.-]*)', text, flags=re.IGNORECASE)
    if not recipe_id or not keys or grain_match is None:
        return None
    aliases_match = re.search('유의어\\s*(?:은|는|:|=)\\s*([^\\r\\n.]+)', text, flags=re.IGNORECASE)
    aliases = []
    if aliases_match:
        aliases = _freeform_csv_values(re.sub('\\s*(?:이야|입니다)\\s*$', '', aliases_match.group(1)))
    return {'section': 'recipes', 'key': recipe_id, 'payload': {'aliases': aliases, 'grain': grain_match.group(1), 'grain_keys': keys, 'required_slots': ['dataset', 'metric']}}

def _freeform_main_filter_cards(source_text):
    text = str(source_text or '')
    marker = re.compile('(?im)(?<![A-Za-z0-9_.-])([A-Za-z][A-Za-z0-9_.-]{0,127})\\s*(?:을|를|도)?\\s*(?:주요\\s*조회\\s*조건|main\\s*filter)(?:으)?로\\s*(?:등록|저장)')
    matches = list(marker.finditer(text))
    cards = []
    for index, match in enumerate(matches):
        ...
        if aliases_match is None:
            continue
        alias_text = re.sub('\\s*(?:이야|입니다?|로\\s*저장해줘)\\s*$', '', aliases_match.group(1))
        alias_text = re.sub('(?<=[A-Za-z0-9_])야\\s*$', '', alias_text)
        aliases = _freeform_csv_values(alias_text)
        if not aliases:
            continue
        payload = {'aliases': aliases}
        display_name = _freeform_sentence_value(segment, ('display_name', '화면(?:에\\s*보일)?\\s*이름'))
        if display_name:
            payload['display_name'] = display_name
...
def _deterministic_freeform_authoring_proposal(component, expected_purpose, output_schema):
    ...
    """
    root_properties = _freeform_draft_root_properties(output_schema)
    expected_root = {'metadata_domain_draft': {'items'}, 'metadata_dataset_draft': {'dataset_cards'}, 'metadata_main_filter_draft': {'items'}}.get(expected_purpose)
    ...
    if expected_purpose == 'metadata_dataset_draft':
        ...
        family = _freeform_dataset_family(source_text, dataset_id) or dataset_id if dataset_id else ''
        source_type = _freeform_source_type(source_text)
        if not dataset_id or not family or (not source_type) or (not (_freeform_filter_mappings(source_text) or _freeform_query_columns(source_text))):
            return None
        card = {'dataset_id': dataset_id, 'display_name': _freeform_sentence_value(source_text, ('display_name', '화면(?:에\\s*보일)?\\s*이름')) or dataset_id, 'family': family, 'source_type': source_type, 'fields': []}
        ...
        item = _freeform_domain_recipe_card(source_text) or _freeform_domain_grain_card(source_text) or _freeform_domain_group_card(source_text) or _freeform_domain_profile_card(source_text)
        if item is None:
            return None
        draft = {'items': [item]}
    else:
        ...
        if not items:
            return None
        draft = {'items': items}
    proposal = {'contract_version': 'metadata.authoring.proposal.v1', 'status': 'complete', 'source_sha256': hashlib.sha256(source_text.encode('utf-8')).hexdigest(), 'draft': draft}
    if output_schema is not None and any(Draft202012Validator(output_schema).iter_errors(proposal)):
        return None
    component._observed_authoring_source_projection = {'contract_version': 'metadata.deterministic-source-projection.v1', 'purpose': expected_purpose, 'status': 'used', 'source_sha256': proposal['source_sha256'], 'draft_sha256': sha256_json(draft)}
    return proposal

def _authoring_invocation_draft(component, *, input_name, expected_purpose, required, expected_output_schema=None, expected_runtime_context_sha256=''):
    ...
    if not required:
        if calls != 0:
            raise ContractError('metadata_policy_error', 'metadata_llm_boundary', '결정론적 등록 분기에서 LLM이 호출되었습니다.', {'input_name': input_name, 'expected_purpose': expected_purpose, 'llm_calls': calls})
        return None
    deterministic_proposal = _deterministic_freeform_authoring_proposal(component, expected_purpose, expected_output_schema)
    if deterministic_proposal is not None:
        return deterministic_proposal
    if invocation.get('contract_version') != 'llm.invocation.v1' or invocation.get('purpose') != expected_purpose or invocation.get('status') != 'ok' or (calls != 1):
        invocation_error = invocation.get('error') if isinstance(invocation.get('error'), dict) else {}
        provider_error_type = str(invocation_error.get('provider_error_type') or '')[:80]
        if not provider_error_type.isidentifier():
            provider_error_type = ''
        provider_error_status = str(invocation_error.get('provider_error_status') or '')[:80]
        if not re.fullmatch('[A-Za-z0-9._-]{1,80}', provider_error_status):
            provider_error_status = ''
        provider_error_code = str(invocation_error.get('provider_error_code') or '')[:80]
        if not re.fullmatch('[A-Za-z0-9._-]{1,80}', provider_error_code):
            provider_error_code = ''
        provider_binding = str(invocation.get('provider_schema_binding') or '')[:80]
        if not re.fullmatch('[a-z][a-z0-9_]{0,79}', provider_binding):
            provider_binding = ''
        raise ContractError('metadata_llm_unavailable', 'metadata_draft', '외부 Prompt/LLM 노드의 검증된 단일 호출 결과가 필요합니다.', {'input_name': input_name, 'expected_purpose': expected_purpose, 'llm_calls': calls, 'error_type': provider_error_type, 'reason': provider_error_status, 'provider_error_code': provider_error_code, 'provider_schema_binding': provider_binding, 'invocation_error_code': str(invocation_error.get('code') or '')[:80]})
    prompt_bundle_sha256 = str(invocation.get('prompt_bundle_sha256') or '')
    runtime_context_sha256 = str(invocation.get('runtime_context_sha256') or '')
    if not re.fullmatch('[0-9a-f]{64}', prompt_bundle_sha256):
        raise ContractError('metadata_dependency_error', 'metadata_llm_prompt_binding', 'LLM invocation prompt bundle evidence is invalid.', {'input_name': input_name, 'expected_purpose': expected_purpose})
    if expected_runtime_context_sha256:
        if not re.fullmatch('[0-9a-f]{64}', str(expected_runtime_context_sha256)) or runtime_context_sha256 != expected_runtime_context_sha256:
            raise ContractError('metadata_dependency_error', 'metadata_llm_prompt_binding', 'LLM invocation runtime context evidence does not match the authoritative context.', {'input_name': input_name, 'expected_purpose': expected_purpose})
    if expected_output_schema is not None:
        ...
        binding_status = str(evidence.get('binding_status') or '') if isinstance(evidence, dict) else ''
        provider_binding = str(invocation.get('provider_schema_binding') or '')
        provider_schema_sha256 = str(evidence.get('provider_schema_sha256') or '') if isinstance(evidence, dict) else ''
        projection = str(evidence.get('projection') or '') if isinstance(evidence, dict) else ''
        google_projection_valid = binding_status == 'google_native_json_schema' and projection == 'google_supported_json_schema_subset.v6' and bool(re.fullmatch('[0-9a-f]{64}', provider_schema_sha256))
        portable_projection_valid = binding_status == 'portable_prompt_and_compiler_validation' and projection == 'none' and (provider_schema_sha256 == '')
        if not isinstance(evidence, dict) or set(evidence) != {'contract_version', 'binding_status', 'projection', 'authoritative_schema_sha256', 'provider_schema_sha256'} or evidence.get('contract_version') != 'llm.schema-binding.evidence.v1' or (provider_binding != binding_status) or (evidence.get('authoritative_schema_sha256') != expected_schema_sha256) or (not (google_projection_valid or portable_projection_valid)):
            raise ContractError('metadata_dependency_error', 'metadata_llm_schema_binding', 'LLM 호출의 스키마 결합 증적이 authoritative runtime schema와 일치하지 않습니다.', {'input_name': input_name, 'expected_purpose': expected_purpose, 'expected_schema_sha256': expected_schema_sha256, 'binding_status': binding_status})
    response_text = str(invocation.get('response_text') or '')
    response_bytes = len(response_text.encode('utf-8'))
    observed_response_sha256 = hashlib.sha256(response_text.encode('utf-8')).hexdigest()
    if invocation.get('response_sha256') != observed_response_sha256:
        raise ContractError('metadata_dependency_error', 'metadata_llm_response_binding', 'LLM invocation response hash does not match the response body.', {'input_name': input_name, 'expected_purpose': expected_purpose, 'response_bytes': response_bytes})
    try:
        parsed = _json_object(response_text)
    except ContractError as exc:
        details = deepcopy(exc.details) if isinstance(exc.details, dict) else {}
        details.update({'input_name': input_name, 'expected_purpose': expected_purpose, 'response_bytes': response_bytes, 'response_sha256': hashlib.sha256(response_text.encode('utf-8')).hexdigest()})
        raise ContractError(exc.code, exc.stage, exc.public_message, details, exc.retryable) from exc
    parsed = _normalize_legacy_freeform_proposal_shape(parsed, expected_purpose, expected_output_schema)
    if expected_output_schema is not None:
        schema_errors = sorted(Draft202012Validator(expected_output_schema).iter_errors(parsed), key=lambda item: (list(item.absolute_path), item.validator or ''))
        if schema_errors:
            ...
            exc = max(candidates, key=lambda item: (len(list(item.absolute_path)), 1 if str(item.validator or '') in {'required', 'additionalProperties'} else 0, -len(str(item.message or ''))))
            raise ContractError('metadata_schema_error', 'metadata_llm_schema_validation', 'LLM output does not satisfy the authoritative metadata schema.', {'input_name': input_name, 'expected_purpose': expected_purpose, 'path': list(exc.absolute_path), 'validator': str(exc.validator or '')[:80], 'reason': str(exc.message or '')[:400], 'root_validator': str(root_error.validator or '')[:80], 'response_bytes': response_bytes, 'response_sha256': observed_response_sha256})
        observed_bindings = deepcopy(getattr(component, '_observed_authoring_schema_bindings', None) or {})
        if expected_purpose in observed_bindings:
            raise ContractError('metadata_dependency_error', 'metadata_llm_schema_binding', 'Duplicate LLM schema-binding evidence was observed for one authoring purpose.', {'expected_purpose': expected_purpose})
        observed_bindings[expected_purpose] = {'contract_version': 'metadata.llm-schema-binding-summary.v1', 'purpose': expected_purpose, 'invocation_count': 1, 'provider_schema_binding': provider_binding, 'binding_status': binding_status, 'projection': projection, 'authoritative_schema_sha256': expected_schema_sha256, 'provider_schema_sha256': provider_schema_sha256, 'runtime_output_schema_sha256': expected_schema_sha256, 'raw_prompt_persisted': False, 'raw_response_persisted': False}
        component._observed_authoring_schema_bindings = observed_bindings
    return parsed

_CLARIFICATION_TECHNICAL_PATTERN = re.compile('(?:canonical|dataset[ _-]?id|field[ _-]?id|physical(?:[ _-]?column)?|semantic(?:[ _-]?type)?|json|dsl|config_ref|query_ref|schema|source[ _-]?registry|registry|물리\\s*컬럼|시맨틱\\s*타입|기술\\s*문법|내부\\s*ID|레지스트리|스키마)', re.IGNORECASE)

_GENERIC_BUSINESS_CLARIFICATION = '어떤 업무 데이터나 수치를 뜻하는지 자연어로 설명해 주세요.'

def _worker_safe_clarification(questions, missing_fields):
    safe_questions = []
    for raw_question in questions if isinstance(questions, list) else []:
        question = re.sub('\\s+', ' ', str(raw_question or '').strip())
        if not question:
            continue
...
def _unwrap_freeform_authoring_proposal(raw_proposal, *, source_sha256):
    ...
    if proposal.get('contract_version') != 'metadata.authoring.proposal.v1' or not re.fullmatch('[0-9a-f]{64}', expected_source_sha256) or proposal.get('source_sha256') != expected_source_sha256:
        raise ContractError('metadata_schema_error', 'metadata_authoring_proposal', '자유형 자연어 등록 제안이 원문 SHA-256과 결합되지 않았습니다.', {'expected_source_sha256': expected_source_sha256})
    proposal_sha256 = sha256_json(proposal)
    if status == 'complete':
        if set(proposal) != common_keys | {'draft'} or not isinstance(proposal.get('draft'), dict):
            raise ContractError('metadata_schema_error', 'metadata_authoring_proposal', '완료 제안은 폐쇄형 draft 하나만 포함해야 합니다.')
        draft = deepcopy(proposal['draft'])
        evidence = {'contract_version': 'metadata.authoring.proposal.validation.v1', 'proposal_contract_version': 'metadata.authoring.proposal.v1', 'status': 'complete', 'source_sha256': expected_source_sha256, 'proposal_sha256': proposal_sha256, 'draft_sha256': sha256_json(draft)}
        return (draft, evidence)
    if status == 'needs_clarification':
        if set(proposal) != common_keys | {'clarification'}:
            raise ContractError('metadata_schema_error', 'metadata_authoring_proposal', '확인 질문 제안에는 draft 또는 저장 후보 필드를 포함할 수 없습니다.')
        ...
        if not isinstance(clarification, dict) or set(clarification) != {'questions', 'missing_fields'}:
            raise ContractError('metadata_schema_error', 'metadata_authoring_proposal', '확인 질문 payload가 폐쇄형 계약과 일치하지 않습니다.')
        questions = clarification.get('questions')
        missing_fields = clarification.get('missing_fields')
        if not isinstance(questions, list) or not 1 <= len(questions) <= 3 or len(set(questions)) != len(questions) or any((not isinstance(item, str) or not item.strip() or len(item) > 400 for item in questions)) or (not isinstance(missing_fields, list)) or (len(missing_fields) > 32) or (len(set(missing_fields)) != len(missing_fields)) or any((not isinstance(item, str) or not item.strip() or len(item) > 128 for item in missing_fields)):
            raise ContractError('metadata_schema_error', 'metadata_authoring_proposal', '확인 질문의 개수 또는 길이가 허용 범위를 벗어났습니다.')
        raise ContractError('metadata_clarification_required', 'metadata_clarification', '메타데이터를 추측하지 않고 작업자 확인이 필요한 항목을 반환했습니다.', {'contract_version': 'metadata.authoring.clarification.v1', 'questions': [item.strip() for item in questions], 'missing_fields': [item.strip() for item in missing_fields], 'source_sha256': expected_source_sha256, 'proposal_sha256': proposal_sha256})
    raise ContractError('metadata_schema_error', 'metadata_authoring_proposal', '자유형 자연어 등록 제안 status가 유효하지 않습니다.', {'status': status})

def _unwrap_bootstrap_authoring_proposal(raw_proposal, *, kind, source_sha256, composite_source_sha256):
    try:
        return _unwrap_freeform_authoring_proposal(raw_proposal, source_sha256=source_sha256)
    except ContractError as exc:
        ...
        branch_label = {'domain': '업무 정의', 'dataset': '업무 데이터', 'main_filter': '조회 조건'}.get(kind, '업무 설명')
        details['questions'] = [f'[{branch_label} {index}] {question}'[:400] for index, question in enumerate(questions, start=1)]
        details['missing_fields'] = [f'{branch_label}: {field}'[:128] for field in list(details.get('missing_fields') or [])]
        original_proposal_sha256 = str(details.get('proposal_sha256') or '')
        details['source_sha256'] = composite_source_sha256
        details['proposal_sha256'] = sha256_json({'authoring_branch': kind, 'proposal_sha256': original_proposal_sha256})
        raise ContractError(exc.code, exc.stage, exc.public_message, details, exc.retryable) from exc

def _enforce_domain_policy_boundary(draft, *, source_sha256, proposal_sha256='', grounding_mode='freeform_llm', approved_planner_policy=None):
    ...
    sealed_planner_policy = deepcopy(approved_planner_policy) if isinstance(approved_planner_policy, dict) else {}
    non_empty_prompt = isinstance(prompt_extensions, dict) and any((str(prompt_extensions.get(key) or '').strip() for key in ('intent', 'answer')))
    if non_empty_prompt or specialized_functions not in (None, []) or (output_profile not in (None, {}) and output_profile != sealed_planner_policy):
        raise ContractError('metadata_policy_error', 'metadata_domain_policy_boundary', '도메인 자연어 LLM은 특화 프롬프트, 등록 함수 또는 출력 정책을 작성할 수 없습니다. Domain Policy Flow를 사용하세요.')
    current['prompt_extensions'] = {'intent': '', 'answer': ''}
    current['specialized_functions'] = []
    current['output_profile'] = sealed_planner_policy
    current['source_provenance'] = {'source_sha256': str(source_sha256 or ''), 'grounding_mode': str(grounding_mode or 'freeform_llm'), 'proposal_sha256': str(proposal_sha256 or '')}
    return current
...
def _complete_unambiguous_metric_bindings(draft, approved_semantic_vocabulary=None):
    ...
    vocabulary = approved_semantic_vocabulary if isinstance(approved_semantic_vocabulary, dict) else {}
    completed = []
    preserved = []
    if not isinstance(datasets, dict) or not isinstance(metrics, dict):
        metrics = {}
    vocabulary_metric_ids = [card.get('id') for card in vocabulary.get('metrics') or [] if isinstance(card, dict)]
    vocabulary_fields = [card for card in vocabulary.get('fields') or [] if isinstance(card, dict)]
    draft_candidates = {(str(card.get('family') or ''), str(field_id)) for card in (datasets or {}).values() if isinstance(card, dict) for field_id in (card.get('fields') or {} if isinstance(card.get('fields'), dict) else {})}
    for metric_id, raw_metric in metrics.items():
        ...
        candidates = sorted({candidate for candidate in semantic_candidates if candidate in draft_candidates})
        if len(candidates) != 1:
            raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', '지표 원천 참조가 승인된 의미 필드 하나로 정확히 결정되지 않습니다.', {'metric_id_sha256': sha256_json(str(metric_id)), 'source_binding_sha256': sha256_json(existing_binding), 'candidate_count': len(candidates)})
        family, field_id = candidates[0]
        raw_metric['source_binding'] = {'dataset_family': family, 'field': field_id}
        completed.append(str(metric_id))
    completed = sorted(completed)
    preserved = sorted(preserved)
    return (current, {'contract_version': 'metadata.metric-binding-reconciliation.v1', 'resolution_mode': 'approved_semantic_vocabulary_exact', 'completed_count': len(completed), 'completed_metric_ids_sha256': sha256_json(completed), 'corrected_count': 0, 'corrected_metric_ids_sha256': sha256_json([]), 'preserved_full_card_count': len(preserved), 'preserved_full_card_ids_sha256': sha256_json(preserved)})

_SEMANTIC_ALIAS_TARGET_SECTIONS = (('datasets', 'dataset'), ('fields', 'field'), ('metrics', 'metric'), ('relations', 'relation'), ('grains', 'grain'), ('predicates', 'predicate'), ('recipes', 'recipe'), ('entity_groups', 'entity_group'))

_NATURAL_ALIAS_POLICY = {'normalization': ['unicode_nfkc', 'trim', 'collapse_space', 'latin_casefold'], 'match': 'bounded_longest', 'conflict': 'fail_ambiguous', 'provenance_source': 'natural_authoring'}
...
def _semantic_alias_target_candidates(draft, vocabulary, target_id):
    ...
    for section, target_type in _SEMANTIC_ALIAS_TARGET_SECTIONS:
        ...
        matches = [card for card in cards or [] if isinstance(card, dict) and card.get('id') == target_id]
        if not matches:
            continue
        if section == 'datasets':
            registered = isinstance(datasets, dict) and target_id in datasets
        elif section == 'fields':
            registered = any((isinstance(dataset, dict) and isinstance(dataset.get('fields'), dict) and (target_id in dataset['fields']) for dataset in (datasets or {}).values()))
        else:
            registered = isinstance(draft.get(section), dict) and target_id in draft[section]
        if registered:
...
def _normalized_natural_alias_expressions(expressions, target_id):
    if not isinstance(expressions, list) or not 1 <= len(expressions) <= 64:
        raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '자연어 별칭 표현은 bounded non-empty list여야 합니다.', {'target_id_sha256': sha256_json(str(target_id)), 'expression_count': len(expressions) if isinstance(expressions, list) else 0})
    forbidden = ('http://', 'https://', 'mongodb://', 'mongodb+srv://', 'select ', 'insert ', 'update ', 'delete ', 'password', 'secret', 'token=', 'api_key')
    result = {}
    for raw_expression in expressions:
        if not isinstance(raw_expression, str):
            raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '자연어 별칭 표현은 text여야 합니다.', {'target_id_sha256': sha256_json(str(target_id))})
        expression = re.sub('\\s+', ' ', unicodedata.normalize('NFKC', raw_expression).strip())
        folded = expression.casefold()
        if not expression or len(expression.encode('utf-8')) > 512 or any((ord(character) < 32 for character in expression)) or any((fragment in folded for fragment in forbidden)):
            raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '자연어 별칭 표현이 비어 있거나 허용 범위를 벗어났습니다.', {'target_id_sha256': sha256_json(str(target_id))})
...
def _merge_natural_alias_card(existing, generated, canonical_key):
    ...
    if not isinstance(existing, dict) or any((existing.get(key) != generated.get(key) for key in identity_keys)) or any((key in existing and existing.get(key) != generated.get(key) for key in policy_keys)) or (not isinstance(existing.get('values'), list)):
        raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', 'canonical alias card가 다른 대상 또는 정책과 충돌합니다.', {'alias_key_sha256': sha256_json(canonical_key)})
    merged = deepcopy(existing)
    seen = set()
    for value in merged['values']:
        text = _alias_value_text(value)
        if text is None:
            raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '기존 canonical alias value가 유효하지 않습니다.', {'alias_key_sha256': sha256_json(canonical_key)})
        normalized = re.sub('\\s+', ' ', unicodedata.normalize('NFKC', text).strip())
        seen.add(normalized.casefold())
    append_as_strings = bool(merged['values']) and all((isinstance(value, str) for value in merged['values']))
...
def _resolve_natural_alias_conflicts(candidate_aliases, baseline_aliases):
    ...
    if not isinstance(candidate_aliases, dict) or not isinstance(baseline_aliases, dict):
        raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', 'Alias conflict inputs must be closed dictionaries.')

    def marker(value):
        text = _alias_value_text(value)
        if text is None:
            return ''
        return re.sub('\\s+', ' ', unicodedata.normalize('NFKC', text).strip()).casefold()
    baseline_targets = {}
    ...
    for canonical_key, card in sorted(baseline_aliases.items()):
        ...
        if not isinstance(values, list):
            raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', 'Approved baseline alias card is invalid.', {'alias_key_sha256': sha256_json(str(canonical_key))})
        markers = {marker(value) for value in values}
        if '' in markers:
            raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', 'Approved baseline alias value is invalid.', {'alias_key_sha256': sha256_json(str(canonical_key))})
    ...
    for canonical_key, card in sorted(candidate_aliases.items()):
        values = card.get('values') if isinstance(card, dict) else None
        if not isinstance(values, list):
            raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', 'Candidate alias card is invalid.', {'alias_key_sha256': sha256_json(str(canonical_key))})
        same_target_baseline = baseline_markers_by_key.get(canonical_key, set())
        for value in values:
            normalized = marker(value)
            if not normalized:
                raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', 'Candidate alias value is invalid.', {'alias_key_sha256': sha256_json(str(canonical_key))})
    ...
    seen_by_key = {}
    for canonical_key, normalized, value in sorted(delta_rows, key=lambda row: (row[0], row[1], sha256_json(row[2]))):
    ...
    for canonical_key, card in sorted(candidate_aliases.items()):
        ...
        if canonical_key in resolved:
            append_as_strings = bool(resolved[canonical_key]['values']) and all((isinstance(value, str) for value in resolved[canonical_key]['values']))
    ...
    evidence = {'contract_version': 'metadata.alias-conflict-resolution.v1', 'baseline_card_count': len(baseline_aliases), 'candidate_card_count': len(candidate_aliases), 'resolved_card_count': len(resolved), 'accepted_delta_count': sum((len(values) for values in accepted_by_key.values())), 'redundant_delta_count': redundant_count, 'discarded_baseline_conflict_count': discarded_baseline_conflict_count, 'discarded_cross_target_count': discarded_cross_target_count, 'duplicate_same_target_count': duplicate_same_target_count, 'resolved_aliases_sha256': sha256_json(resolved)}
    return (resolved, evidence)

def _expand_compact_main_filter_fragment(fragment, *, approved_semantic_vocabulary, allow_unresolved=False, reconciliation_out=None):
    """Compile typed natural-language alias additions into canonical cards."""
    vocabulary = None
    if approved_semantic_vocabulary:
        try:
            vocabulary = _validated_semantic_vocabulary(approved_semantic_vocabulary)
        except ValueError as exc:
            raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', '승인 축약 업무 어휘가 유효하지 않습니다.') from exc
    elif not allow_unresolved:
        raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', '등록된 테이블 카탈로그에서 주요 필터 대상을 확인할 수 없습니다.')
    additions = fragment.get('alias_additions') if isinstance(fragment, dict) else None
    if not isinstance(additions, list) or not additions:
        raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '주요 필터 별칭 추가 목록이 비어 있습니다.')
    section_by_type = {target_type: section for section, target_type in _SEMANTIC_ALIAS_TARGET_SECTIONS}
    allowed_ids = {target_type: {str(card.get('id') or '') for card in vocabulary.get(section) or [] if isinstance(card, dict)} for target_type, section in section_by_type.items()} if vocabulary is not None else {}
    aliases = {}
    normalized_targets = []
    for index, addition in enumerate(additions):
        if not isinstance(addition, dict) or set(addition) != {'target_type', 'target_id', 'expressions'}:
            raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '주요 필터 별칭 추가 항목의 계약이 닫혀 있지 않습니다.', {'addition_index': index})
        ...
        if target_type not in section_by_type or not target_id:
            raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '주요 필터 대상 유형 또는 식별자가 올바르지 않습니다.', {'addition_index': index, 'target_type': target_type})
        if allowed_ids and target_id not in allowed_ids.get(target_type, set()):
            raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', '주요 필터 별칭 대상이 승인 업무 어휘에 없습니다.', {'addition_index': index, 'target_type': target_type, 'target_id_sha256': sha256_json(target_id)})
        canonical_key = f'{target_type}:{target_id}'
        generated = {'target_type': target_type, 'target_key': target_id, 'values': [{'text': text, 'priority': 100} for text in _normalized_natural_alias_expressions(addition.get('expressions'), target_id)], **deepcopy(_NATURAL_ALIAS_POLICY)}
        if canonical_key in aliases:
            aliases[canonical_key] = _merge_natural_alias_card(aliases[canonical_key], generated, canonical_key)
        else:
            aliases[canonical_key] = generated
        normalized_targets.append(canonical_key)
    evidence = {'contract_version': 'metadata.main-filter-ir-expansion.v1', 'input_count': len(additions), 'canonical_alias_count': len(aliases), 'canonical_targets_sha256': sha256_json(sorted(set(normalized_targets)))}
    if isinstance(reconciliation_out, dict):
        reconciliation_out.clear()
        reconciliation_out.update(evidence)
...
def _merge_typed_main_filter_patch(base_draft, expanded_patch):
    ...
    if not isinstance(expanded_patch, dict) or set(expanded_patch) != {'aliases'}:
        raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', 'Expanded Main Filter IR must contain only aliases.')
    base_aliases = base_draft.get('aliases') if isinstance(base_draft, dict) else None
    patch_aliases = expanded_patch.get('aliases')
    if not isinstance(base_aliases, dict) or not isinstance(patch_aliases, dict):
        raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', 'Main Filter alias catalogs are unavailable.')
    ...
    for canonical_key in sorted(patch_aliases):
        ...
        if canonical_key in base_aliases:
            candidate_aliases[canonical_key] = _merge_natural_alias_card(base_aliases[canonical_key], generated, canonical_key)
        else:
            candidate_aliases[canonical_key] = deepcopy(generated)
    resolved_aliases, _ = _resolve_natural_alias_conflicts(candidate_aliases, base_aliases)
    merged_patch = {canonical_key: resolved_aliases[canonical_key] for canonical_key in sorted(patch_aliases) if canonical_key in resolved_aliases}
...
def _normalize_bootstrap_alias_shorthand(draft, approved_semantic_vocabulary):
    ...
    if aliases is None:
        aliases = {}
    if not isinstance(aliases, dict):
        raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', 'aliases는 object여야 합니다.')
    full_cards = {}
    shorthand = []
    for alias_key in sorted(aliases):
        ...
        if isinstance(card, dict) and 'expressions' in card:
            if set(card) != {'expressions'}:
                raise ContractError('metadata_schema_error', 'metadata_semantic_reference_normalization', '자연어 alias shorthand에 비허용 key가 있습니다.', {'target_id_sha256': sha256_json(str(alias_key))})
    ...
    for target_id, expressions in shorthand:
        candidates = _semantic_alias_target_candidates(current, approved_semantic_vocabulary, target_id)
        if len(candidates) != 1:
            raise ContractError('metadata_dependency_error', 'metadata_semantic_reference_normalization', '자연어 alias 대상이 승인된 draft 대상 하나로 정확히 결정되지 않습니다.', {'target_id_sha256': sha256_json(target_id), 'candidate_count': len(candidates)})
        target_type, target_key = candidates[0]
        canonical_key = f'{target_type}:{target_key}'
        values = [{'text': text, 'priority': 100} for text in _normalized_natural_alias_expressions(expressions, target_id)]
        generated = {'target_type': target_type, 'target_key': target_key, 'values': values, **deepcopy(_NATURAL_ALIAS_POLICY)}
        if canonical_key in full_cards:
            full_cards[canonical_key] = _merge_natural_alias_card(full_cards[canonical_key], generated, canonical_key)
        else:
    ...
    return (current, {'contract_version': 'metadata.alias-semantic-reconciliation.v1', 'resolution_mode': 'approved_semantic_vocabulary_exact', 'normalized_count': len(normalized_ids), 'normalized_alias_ids_sha256': sha256_json(sorted(normalized_ids))})

def _normalize_filter_operator_aliases(draft):
    """Canonicalize a small provider-independent filter vocabulary."""
    aliases = {'equals': 'eq', 'equal': 'eq', 'not_equals': 'ne', 'not_equal': 'ne', 'not_null': 'is_not_null', 'not_blank': 'is_not_blank', 'null': 'is_null', 'blank': 'is_blank', 'startswith': 'starts_with', 'endswith': 'ends_with', 'notin': 'not_in', 'greater_than': 'gt', 'greater_than_or_equal': 'gte', 'less_than': 'lt', 'less_than_or_equal': 'lte'}
    current = deepcopy(draft) if isinstance(draft, dict) else {}
    replacements = []
    ...
    def canonical(value):
        if not isinstance(value, str):
            return value
        token = re.sub('[\\s-]+', '_', value.strip().casefold())
    ...
    visit(current, [])
    return (current, {'contract_version': 'metadata.filter-operator-normalization.v1', 'replacement_count': len(replacements), 'replacements_sha256': sha256_json(replacements)})

def _validate_authoring_source_bindings(draft, *, source_sha256='', proposal_sha256='', approved_reference_context=None, domain_id='', require_registry_exact_set=False):
    ...
    if not registry:
        ...
        default_adapters = {'oracle': 'oracle_source_retriever', 'sql': 'oracle_source_retriever', 'http': 'h_api_source_retriever', 'datalake': 'datalake_source_retriever', 'goodocs': 'goodocs_source_retriever', 'mongodb': 'mongodb_source_retriever', 'file': 'file_source_retriever', 'dummy': 'dummy_source_retriever', 'previous_result': 'previous_result_retriever'}
        normalized = []
        for dataset_id in sorted(datasets):
            card = datasets.get(dataset_id)
            if not isinstance(card, dict):
                raise ContractError('metadata_schema_error', 'metadata_source_bindings', '테이블 카탈로그 데이터셋은 object여야 합니다.', {'dataset_id': str(dataset_id)})
            source_type = str(card.get('source_type') or '').strip().casefold()
            if source_type not in source_types:
                raise ContractError('metadata_schema_error', 'metadata_source_bindings', '지원하지 않는 데이터 원천 유형입니다.', {'dataset_id': str(dataset_id), 'source_type': source_type})
            card['source_type'] = source_type
            card['source_adapter'] = str(card.get('source_adapter') or default_adapters[source_type]).strip()
            card['config_ref'] = str(card.get('config_ref') or f'config:{source_type}:{dataset_id}@1').strip()
            card['query_ref'] = str(card.get('query_ref') or f'query:{dataset_id}@1').strip()
            fields = card.get('fields')
            if not isinstance(fields, dict) or not fields:
                raise ContractError('metadata_schema_error', 'metadata_source_bindings', '테이블 카탈로그에는 하나 이상의 컬럼 정의가 필요합니다.', {'dataset_id': str(dataset_id)})
            normalized.append({'dataset_id': str(dataset_id), 'source_type': source_type, 'source_adapter': card['source_adapter'], 'config_ref': card['config_ref'], 'query_ref': card['query_ref']})
        return {'contract_version': 'metadata.source-binding.validation.v1', 'status': 'complete', 'binding_authority': 'metadata_three_collections', 'dataset_count': len(normalized), 'bindings_sha256': sha256_json(normalized)}
    if set(registry) != {'contract_version', 'domain_id', 'bindings', 'dataset_descriptors', 'semantic_vocabulary', 'semantic_templates', 'semantic_templates_sha256', 'semantic_templates_blueprint_sha256', 'semantic_templates_executable_sha256', 'semantic_templates_projection_sha256', 'registry_sha256'}:
        raise ContractError('metadata_dependency_error', 'metadata_source_bindings', '승인 Source 레지스트리 컨텍스트 root 계약이 닫혀 있지 않습니다.')
    registry_material = {'contract_version': registry.get('contract_version'), 'domain_id': registry.get('domain_id'), 'bindings': registry.get('bindings'), 'dataset_descriptors': registry.get('dataset_descriptors'), 'semantic_vocabulary': registry.get('semantic_vocabulary'), 'semantic_templates': registry.get('semantic_templates'), 'semantic_templates_sha256': registry.get('semantic_templates_sha256'), 'semantic_templates_blueprint_sha256': registry.get('semantic_templates_blueprint_sha256'), 'semantic_templates_executable_sha256': registry.get('semantic_templates_executable_sha256'), 'semantic_templates_projection_sha256': registry.get('semantic_templates_projection_sha256')}
    try:
        dataset_families, field_families = _semantic_maps_from_descriptors(registry.get('dataset_descriptors'))
        semantic_vocabulary = _validated_semantic_vocabulary(registry.get('semantic_vocabulary'), expected_dataset_families=dataset_families, expected_field_families=field_families)
        semantic_templates = _validated_semantic_templates(registry.get('semantic_templates'), semantic_vocabulary)
    except ValueError as exc:
        raise ContractError('metadata_dependency_error', 'metadata_source_bindings', '승인 축약 업무 어휘 계약이 유효하지 않습니다.') from exc
    if registry.get('contract_version') != 'metadata.authoring.source-registry-context.v3' or registry.get('domain_id') != str(domain_id or '') or (not isinstance(registry.get('bindings'), dict)) or (not isinstance(registry.get('dataset_descriptors'), dict)) or (set(registry.get('bindings') or {}) != set(registry.get('dataset_descriptors') or {})) or (registry.get('semantic_vocabulary') != semantic_vocabulary) or (registry.get('semantic_templates') != semantic_templates) or (registry.get('semantic_templates_sha256') != sha256_json(semantic_templates)) or any((not re.fullmatch('[0-9a-f]{64}', str(registry.get(key) or '')) for key in ('semantic_templates_blueprint_sha256', 'semantic_templates_executable_sha256', 'semantic_templates_projection_sha256'))) or (registry.get('registry_sha256') != sha256_json(registry_material)):
    ...
    id_pattern = re.compile('^[A-Za-z][A-Za-z0-9_.-]{0,127}$')
    adapter_pattern = re.compile('^[A-Za-z][A-Za-z0-9_.:-]{0,127}$')
    ref_pattern = re.compile('^(?:config|query):[A-Za-z0-9_.:-]{1,220}@[1-9][0-9]{0,8}$')
    source_types = {'oracle', 'sql', 'mongodb', 'http', 'datalake', 'goodocs', 'file', 'dummy', 'previous_result'}
    forbidden = ('http://', 'https://', 'mongodb://', 'mongodb+srv://', 'select ', 'insert ', 'update ', 'delete ', 'password', 'secret', 'token=', 'api_key')
    for dataset_id, expected in registry_bindings.items():
        ...
        if not id_pattern.fullmatch(str(dataset_id)) or not isinstance(expected, dict) or set(expected) != required or (normalized.get('source_type') not in source_types) or (not adapter_pattern.fullmatch(normalized.get('source_adapter') or '')) or (not ref_pattern.fullmatch(normalized.get('config_ref') or '')) or (not normalized.get('config_ref', '').startswith('config:')) or (not ref_pattern.fullmatch(normalized.get('query_ref') or '')) or (not normalized.get('query_ref', '').startswith('query:')) or any((fragment in joined for fragment in forbidden)):
    ...
    if unresolved:
        clarification_material = {'questions': ['새 데이터셋의 승인 Source adapter/config/query binding을 운영자가 등록해 주세요.'], 'missing_fields': [f'approved_reference_context.bindings.{item}' for item in unresolved[:31]], 'source_sha256': str(source_sha256 or '')}
        raise ContractError('metadata_clarification_required', 'metadata_clarification', '승인 Source 레지스트리에 없는 dataset binding은 활성 후보로 만들 수 없습니다.', {'contract_version': 'metadata.authoring.clarification.v1', **clarification_material, 'proposal_sha256': str(proposal_sha256 or '') or sha256_json(clarification_material)})
    omitted_approved = [dataset_id for dataset_id in sorted(registry_bindings) if dataset_id not in datasets]
    if require_registry_exact_set and omitted_approved:
        raise ContractError('metadata_dependency_error', 'metadata_source_bindings', '초기 도메인 등록의 데이터셋 집합이 승인 Source 레지스트리와 정확히 일치하지 않습니다.', {'omitted_approved_dataset_ids': omitted_approved[:32]})
    ...
    for dataset_id in sorted(datasets):
        ...
        descriptor = _validated_dataset_descriptor(dataset_id, registry_descriptors.get(dataset_id))
        if str(card.get('family') or '') != descriptor['family']:
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'dataset family가 승인 source descriptor와 일치하지 않습니다.', {'dataset_id': dataset_id})
        card_fields = card.get('fields')
        if not isinstance(card_fields, dict) or set(card_fields) != set(descriptor['fields']):
            raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'dataset field 집합이 승인 source descriptor와 정확히 일치하지 않습니다.', {'dataset_id': dataset_id})
        technical_keys = {'physical_column', 'semantic_type', 'roles', 'physical_aliases', 'coercion', 'nullable', 'required_in_source', 'timezone'}
        for field_id, approved_field in descriptor['fields'].items():
            ...
            if not isinstance(actual_field, dict):
                raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'dataset field binding이 object가 아닙니다.', {'dataset_id': dataset_id, 'field_id': field_id})
            for key in technical_keys:
                if key not in actual_field and key in approved_field:
                    actual_field[key] = deepcopy(approved_field[key])
                    completed_registry_fields.append(f'datasets.{dataset_id}.fields.{field_id}.{key}')
                    continue
                actual_value = actual_field.get(key)
                approved_value = approved_field.get(key)
                if key in {'roles', 'physical_aliases'}:
                    values_match = isinstance(actual_value, list) and isinstance(approved_value, list) and (len(actual_value) == len(set(actual_value))) and (set(actual_value) == set(approved_value))
                else:
                    values_match = actual_value == approved_value
                if not values_match:
                    raise ContractError('metadata_dependency_error', 'metadata_source_descriptors', 'dataset 기술 field 속성이 승인 source descriptor와 일치하지 않습니다.', {'dataset_id': dataset_id, 'field_id': field_id, 'attribute': key})
                if key in {'roles', 'physical_aliases'}:
        ...
        descriptor_rows.append({'dataset_id': dataset_id, 'family': descriptor['family'], 'fields_sha256': sha256_json(descriptor['fields'])})
    return {'contract_version': 'metadata.source-binding.validation.v1', 'status': 'approved_registry_exact', 'binding_authority': 'approved_registry', 'dataset_count': len(bindings), 'bindings_sha256': sha256_json(bindings), 'registry_sha256': str(registry['registry_sha256']), 'dataset_descriptors_sha256': sha256_json(descriptor_rows), 'registry_resolution': 'passed', 'discarded_untrusted_fields': discarded_untrusted_fields[:128], 'completed_registry_field_count': len(completed_registry_fields), 'completed_registry_fields_sha256': sha256_json(sorted(completed_registry_fields))}
...
def _freeform_authoring_manifest(source_text):
    ...
    inventories = {'datasets': [], 'dataset_fields': {}, 'fields': [], 'field_roles': {}, 'metrics': [], 'grains': [], 'grain_keys': {}, 'grain_display_fields': {}, 'relations': [], 'relation_endpoints': {}, 'relation_keys': {}, 'relation_policies': {}, 'recipes': [], 'operations': [], 'aliases': [], 'alias_targets': [], 'alias_bindings': []}
    counts = {'datasets': 0, 'fields': 0, 'field_bindings': 0, 'field_roles': 0, 'metrics': 0, 'grains': 0, 'grain_keys': 0, 'grain_display_fields': 0, 'relations': 0, 'relation_endpoints': 0, 'relation_keys': 0, 'relation_policies': 0, 'recipes': 0, 'operations': 0, 'aliases': 0, 'alias_targets': 0, 'alias_bindings': 0}
    material = {'contract_version': 'metadata.authoring.source-manifest.v1', 'source_sha256': source_sha256, 'inventories': inventories, 'required_sections': [], 'counts': counts}
...
def _freeform_dataset_patch_authorization_manifest(source_manifest, base_draft):
    ...
    if not isinstance(source_manifest, dict) or not isinstance(base_draft, dict):
        raise ContractError('metadata_dependency_error', 'metadata_patch_authorization', '자유형 데이터셋 수정에 사용할 활성 메타데이터가 없습니다.')
    datasets = base_draft.get('datasets')
    if not isinstance(datasets, dict) or not datasets:
        raise ContractError('metadata_dependency_error', 'metadata_patch_authorization', '자유형 데이터셋 수정 대상 목록이 비어 있습니다.')
    ...
    for dataset_id in dataset_ids:
        ...
        fields = dataset.get('fields') if isinstance(dataset, dict) else None
        if not isinstance(fields, dict) or not fields:
            raise ContractError('metadata_dependency_error', 'metadata_patch_authorization', '활성 데이터셋의 field catalog가 비어 있습니다.', {'dataset_id_sha256': hashlib.sha256(dataset_id.encode('utf-8')).hexdigest()})
        ...
        for field_id in field_ids:
            ...
            if isinstance(roles, list) and roles:
                normalized_roles = [role for role in AUTHORING_FIELD_ROLE_ORDER if role in roles]
                if len(normalized_roles) != len(roles):
                    raise ContractError('metadata_dependency_error', 'metadata_patch_authorization', '활성 field role 계약이 유효하지 않습니다.', {'dataset_id_sha256': hashlib.sha256(dataset_id.encode('utf-8')).hexdigest(), 'field_id_sha256': hashlib.sha256(field_id.encode('utf-8')).hexdigest()})
    ...
    inventories['fields'] = sorted({field_id for values in dataset_fields.values() for field_id in values})
...
_AUTHORING_QUERY_MARKER = re.compile('(?im)^[ \\t]*(?:query_template|sql_template|oracle_sql|datalake_sql)[ \\t]*:[ \\t]*(?:\\n|$)')

_AUTHORING_QUERY_BLOCK_END = re.compile('(?im)^[ \\t]*(?:filter_mappings|required_params|required_param_mappings|standard_column_aliases|default_detail_columns|metric_semantics|selection_criteria)[ \\t]*(?:[:=]|(?:은|는)\\b)')
...
def _authoring_prompt_runtime_context_sha256(purpose, variables):
    ...
    return sha256_json({'authority': 'untrusted_data', 'purpose': purpose, 'variables': projected})

def _bootstrap_context_payload(component, *, input_name, kind, purpose, domain_id, environment, bootstrap_fragment=True, grounding_mode='freeform_llm', annotation_only=False, expected_invoke=True):
    ...
    if not isinstance(context, dict) or set(context) != expected_root_keys:
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록 runtime context root 계약이 정확히 일치하지 않습니다.', {'input_name': input_name, 'authoring_branch': kind})
    ...
    expected_registry_keys = {'contract_version', 'domain_id', 'bindings', 'dataset_descriptors', 'semantic_vocabulary', 'semantic_templates', 'semantic_templates_sha256', 'semantic_templates_blueprint_sha256', 'semantic_templates_executable_sha256', 'semantic_templates_projection_sha256', 'registry_sha256'}
    if set(registry) != expected_registry_keys:
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록의 승인 참조 컨텍스트가 유효하지 않습니다.', {'input_name': input_name, 'authoring_branch': kind})
    registry_material = {key: registry.get(key) for key in ('contract_version', 'domain_id', 'bindings', 'dataset_descriptors', 'semantic_vocabulary', 'semantic_templates', 'semantic_templates_sha256', 'semantic_templates_blueprint_sha256', 'semantic_templates_executable_sha256', 'semantic_templates_projection_sha256')}
    expected_registry_sha256 = hashlib.sha256(json.dumps(registry_material, ensure_ascii=False, sort_keys=True, separators=(',', ':'), allow_nan=False).encode('utf-8')).hexdigest()
    try:
        dataset_families, field_families = _semantic_maps_from_descriptors(registry.get('dataset_descriptors'))
        expected_semantic_vocabulary = _validated_semantic_vocabulary(registry.get('semantic_vocabulary'), expected_dataset_families=dataset_families, expected_field_families=field_families)
        expected_semantic_templates = _validated_semantic_templates(registry.get('semantic_templates'), expected_semantic_vocabulary)
    except ValueError as exc:
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록의 승인 축약 의미 어휘가 유효하지 않습니다.', {'input_name': input_name, 'authoring_branch': kind}) from exc
    if registry.get('contract_version') != 'metadata.authoring.source-registry-context.v3' or registry.get('domain_id') != domain_id or (not isinstance(registry.get('bindings'), dict)) or (not isinstance(registry.get('dataset_descriptors'), dict)) or (set(registry.get('bindings') or {}) != set(registry.get('dataset_descriptors') or {})) or (registry.get('semantic_templates') != expected_semantic_templates) or (registry.get('semantic_templates_sha256') != sha256_json(expected_semantic_templates)) or any((not re.fullmatch('[0-9a-f]{64}', str(registry.get(key) or '')) for key in ('semantic_templates_blueprint_sha256', 'semantic_templates_executable_sha256', 'semantic_templates_projection_sha256'))) or (registry.get('registry_sha256') != expected_registry_sha256):
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록의 승인 참조 컨텍스트 hash 또는 identity가 유효하지 않습니다.', {'input_name': input_name, 'authoring_branch': kind})
    expected_variable_keys = {'authoring_kind', 'domain_id', 'environment', 'source_grounding_mode', 'source_text', 'source_sha256', 'source_manifest', 'bootstrap_fragment', 'approved_semantic_vocabulary', 'source_registry_sha256'}
    ...
    if not isinstance(variables, dict) or set(variables) != expected_variable_keys:
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록 runtime context variables 계약이 정확히 일치하지 않습니다.', {'input_name': input_name, 'authoring_branch': kind})
    ...
    if not isinstance(source_text, str) or not source_text or source_text != source_text.strip():
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록 원문이 비어 있거나 정규화되지 않았습니다.', {'input_name': input_name, 'authoring_branch': kind})
    expected_source_sha256 = hashlib.sha256(source_text.encode('utf-8')).hexdigest()
    try:
        expected_manifest = _freeform_authoring_manifest(source_text) if grounding_mode == 'freeform_llm' and (not annotation_only) else extract_authoring_source_manifest(source_text)
    except AuthoringSourceManifestError as exc:
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '등록 runtime context의 source manifest가 유효하지 않습니다.', {'input_name': input_name, 'authoring_branch': kind}) from exc
    expected_dataset_ids = sorted((str(card['id']) for card in expected_semantic_vocabulary['datasets']))
    exact_field_allowlists = _dataset_field_allowlists_from_vocabulary(expected_semantic_vocabulary, registry.get('dataset_descriptors')) if kind == 'dataset' else None
    expected_output_schema = _bootstrap_output_schema(kind, expected_source_sha256, approved_dataset_ids=expected_dataset_ids, approved_dataset_field_ids=exact_field_allowlists, approved_semantic_vocabulary=expected_semantic_vocabulary) if bootstrap_fragment and context_invoke is True else _authoring_section_output_schema(kind, source_sha256=expected_source_sha256, grounding_mode=grounding_mode, annotation_only=annotation_only, approved_dataset_ids=expected_dataset_ids, approved_dataset_field_ids=exact_field_allowlists, approved_semantic_vocabulary=expected_semantic_vocabulary) if context_invoke is True else None
    registry_payload_valid = variables.get('approved_semantic_vocabulary') == expected_semantic_vocabulary and variables.get('source_registry_sha256') == expected_registry_sha256
    context_valid = context.get('contract_version') == 'prompt.runtime-context.v1' and context.get('purpose') == purpose and isinstance(context_invoke, bool) and (expected_invoke is None or context_invoke is expected_invoke) and (variables.get('authoring_kind') == kind) and (variables.get('domain_id') == domain_id) and (variables.get('environment') == environment) and (variables.get('source_grounding_mode') == grounding_mode) and (variables.get('bootstrap_fragment') is bool(bootstrap_fragment)) and (source_sha256 == expected_source_sha256) and (variables.get('source_manifest') == expected_manifest) and registry_payload_valid and (context_invoke is False or _bootstrap_schema_material(variables.get('output_schema')) == _bootstrap_schema_material(expected_output_schema))
    if not context_valid:
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록 runtime context의 목적, 해시 또는 bootstrap 결합이 유효하지 않습니다.', {'input_name': input_name, 'authoring_branch': kind, 'expected_purpose': purpose, 'expected_source_sha256': expected_source_sha256})
    return {'source_text': source_text, 'source_sha256': expected_source_sha256, 'invoke': context_invoke, 'output_schema': deepcopy(variables['output_schema']) if context_invoke is True else None, 'output_schema_sha256': sha256_json(variables['output_schema']) if context_invoke is True else '', 'runtime_context_sha256': _authoring_prompt_runtime_context_sha256(purpose, variables)}

def _collection_authoring_context_payload(component, *, input_name, kind, purpose, domain_id, environment, grounding_mode):
    ...
    expected_keys = {'authoring_kind', 'domain_id', 'environment', 'source_grounding_mode', 'source_text', 'source_sha256', 'source_manifest', 'bootstrap_fragment', 'output_schema'}
    if not isinstance(context, dict) or set(context) != {'contract_version', 'purpose', 'invoke', 'variables'} or context.get('contract_version') != 'prompt.runtime-context.v1' or (context.get('purpose') != purpose) or (context.get('invoke') is not True) or (not isinstance(variables, dict)) or (set(variables) != expected_keys) or (variables.get('authoring_kind') != kind) or (variables.get('domain_id') != domain_id) or (variables.get('environment') != environment) or (variables.get('source_grounding_mode') != grounding_mode) or (variables.get('bootstrap_fragment') is not True):
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '컬렉션 기반 등록 runtime context 계약이 올바르지 않습니다.', {'input_name': input_name, 'authoring_branch': kind})
    source_text = variables.get('source_text')
    if not isinstance(source_text, str) or not source_text or source_text != source_text.strip():
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '컬렉션 기반 등록 원문이 비어 있거나 정규화되지 않았습니다.', {'input_name': input_name, 'authoring_branch': kind})
    source_sha256 = hashlib.sha256(source_text.encode('utf-8')).hexdigest()
    expected_manifest = _freeform_authoring_manifest(source_text)
    expected_schema = _bootstrap_output_schema(kind, source_sha256, approved_semantic_vocabulary=None)
    if variables.get('source_sha256') != source_sha256 or variables.get('source_manifest') != expected_manifest or _bootstrap_schema_material(variables.get('output_schema')) != _bootstrap_schema_material(expected_schema):
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '컬렉션 기반 등록 context의 원문 또는 출력 스키마 결합이 올바르지 않습니다.', {'input_name': input_name, 'authoring_branch': kind})
    return {'source_text': source_text, 'source_sha256': source_sha256, 'invoke': True, 'output_schema': deepcopy(variables['output_schema']), 'output_schema_sha256': sha256_json(variables['output_schema']), 'runtime_context_sha256': _authoring_prompt_runtime_context_sha256(purpose, variables)}

def _split_bootstrap_source(component, *, domain_id, environment):
    specifications = {'domain': ('authoring_source_context', 'metadata_domain_draft', '도메인 정보'), 'dataset': ('bootstrap_dataset_source_context', 'metadata_dataset_draft', '데이터셋 정보'), 'main_filter': ('bootstrap_main_filter_source_context', 'metadata_main_filter_draft', '주요 필터 정보')}
    branches = {}
    sections = []
    for kind in BOOTSTRAP_BRANCH_ORDER:
        input_name, purpose, label = specifications[kind]
        branch = _bootstrap_context_payload(component, input_name=input_name, kind=kind, purpose=purpose, domain_id=domain_id, environment=environment)
        branches[kind] = branch
        sections.append(f"--- {label} 시작 ---\n{branch['source_text']}\n--- {label} 끝 ---")
    bundled_source = '\n\n'.join(sections)
    ...
    if not isinstance(input_message, str) or input_message != bundled_source:
        ...
        raise ContractError('metadata_dependency_error', 'metadata_source_context', '분할 초기 등록 input_message가 세 runtime context의 정확한 원문 묶음과 일치하지 않습니다.', {'expected_source_sha256': hashlib.sha256(bundled_source.encode('utf-8')).hexdigest(), 'actual_source_sha256': hashlib.sha256(actual_text.encode('utf-8')).hexdigest()})
...
def _authoring_source_text(component):
    ...
    if isinstance(context, dict) and context:
        if context.get('contract_version') != 'prompt.runtime-context.v1':
            raise ContractError('metadata_schema_error', 'metadata_source_context', 'Authoring source context contract is invalid.')
        variables = context.get('variables')
        ...
        if context_mode and context_mode != configured_mode:
            raise ContractError('metadata_policy_error', 'metadata_source_context', 'Prompt context and compiler grounding modes do not match.')
        if source_text:
            return source_text
...
def _v2_alias_only_manifest_patch(source_manifest):
    ...
    if not isinstance(counts, dict) or not isinstance(inventories, dict):
        return None
    non_alias_kinds = ('datasets', 'fields', 'field_bindings', 'field_roles', 'metrics', 'grains', 'grain_keys', 'grain_display_fields', 'relations', 'relation_endpoints', 'relation_keys', 'relation_policies', 'recipes', 'operations')
    if any((int(counts.get(kind) or 0) != 0 for kind in non_alias_kinds)):
        return None
    bindings = inventories.get('alias_bindings')
    if int(counts.get('aliases') or 0) < 1 or not isinstance(bindings, list) or len(bindings) != int(counts.get('alias_bindings') or 0):
        return None
    aliases = {}
    for raw_binding in bindings:
...
class MetadataAuthoringEngine(Component):
    ...
    icon = 'book-lock'
    inputs = [MessageInput(name='input_message', display_name='자연어 메타데이터 TXT', required=False, info='작업자가 자유 형식으로 작성한 원문입니다. 프롬프트 컨텍스트와 동일 원문인지 해시로 확인합니다.'), DataInput(name='authoring_source_context', display_name='등록 원문 컨텍스트', required=False, info='외부 프롬프트 컨텍스트 노드가 봉인한 원문·유형·해시 정보입니다.'), BoolInput(name='split_bootstrap', display_name='초기 등록 3분할 컴파일', value=False, advanced=True, info='도메인 최초 등록에서 도메인·데이터셋·주요 필터 제안 세 개를 결정론적으로 병합합니다.'), DataInput(name='bootstrap_dataset_source_context', display_name='초기 데이터셋 원문 컨텍스트', required=False, info='분할 초기 등록의 데이터셋 원문·해시·목적을 봉인한 런타임 컨텍스트입니다.'), DataInput(name='bootstrap_main_filter_source_context', display_name='초기 주요 필터 원문 컨텍스트', required=False, info='분할 초기 등록의 주요 필터 원문·해시·목적을 봉인한 런타임 컨텍스트입니다.'), DataInput(name='approved_reference_context', display_name='승인 원천 참조 컨텍스트', required=False, info='제안서의 원천 식별자가 운영자 승인 레지스트리와 정확히 일치하는지 검증하는 정보입니다.'), DataInput(name='authoring_invocation_result', display_name='등록 LLM 호출 결과', required=False, info='외부 LLM 노드가 반환한 단일 메타데이터 제안서 또는 추가 확인 요청입니다.'), DataInput(name='bootstrap_dataset_invocation_result', display_name='초기 데이터셋 LLM 호출 결과', required=False, info='분할 초기 등록의 데이터셋 제안서 또는 추가 확인 요청입니다.'), DataInput(name='bootstrap_main_filter_invocation_result', display_name='초기 주요 필터 LLM 호출 결과', required=False, info='분할 초기 등록의 주요 필터 제안서 또는 추가 확인 요청입니다.'), DropdownInput(name='authoring_kind', display_name='등록 유형', options=['domain', 'dataset', 'main_filter', 'domain_policy'], value='domain', info='도메인, 데이터셋, 주요 필터, 관리자 정책 중 처리할 등록 계약을 선택합니다.'), DropdownInput(name='source_grounding_mode', display_name='자연어 입력 해석 방식', options=['freeform_llm', 'explicit_inventory'], value='freeform_llm', advanced=True, info='일반 작업자 입력은 freeform_llm을 사용하며, explicit_inventory는 관리자 검증 경로 전용입니다.'), DropdownInput(name='metadata_contract_mode', display_name='메타데이터 계약', options=['domain_package_v2'], value='domain_package_v2', info='컴파일과 저장에 적용할 메타데이터 패키지 계약 버전입니다.'), StrInput(name='domain_id', display_name='도메인 ID', value='default', info='등록·패치할 업무 도메인의 고유 식별자입니다. 공유 시 대상 도메인 ID로 바꿉니다.'), StrInput(name='environment', display_name='운영 환경', value='production', info='세 current 메타데이터 문서와 리비전을 구분할 환경 이름입니다.'), DropdownInput(name='revision_policy', display_name='리비전 정책', options=['auto_next', 'explicit'], value='auto_next', info='다음 리비전을 자동 계산하거나 명시적 리비전 번호를 사용할지 선택합니다.'), IntInput(name='revision', display_name='명시적 도메인 리비전', value=1, info='리비전 정책이 explicit일 때 사용할 양의 정수 리비전입니다.'), DataInput(name='inline_base_domain_bundle', display_name='인라인 기준 도메인 번들(패치 Dry Run)', required=False, info='데이터셋·필터 패치를 저장 없이 검증할 때 사용할 기준 도메인 번들입니다.'), MultilineInput(name='trusted_blueprint_json', display_name='신뢰 실행 블루프린트 JSON(관리자 전용)', value='', required=False, advanced=True, info='관리자 전용 결정론적 등록 경로에서만 사용하는 검토 완료 블루프린트입니다.'), StrInput(name='trusted_blueprint_sha256', display_name='신뢰 블루프린트 SHA-256 핀(관리자 전용)', value='', required=False, advanced=True, info='신뢰 실행 블루프린트가 변조되지 않았는지 확인할 SHA-256 값입니다.'), MultilineInput(name='intent_prompt_extension', display_name='의도 분석 특화 프롬프트', value='', required=False, info='공통 의도 분석 프롬프트 뒤에 연결할 도메인별 용어·판단 규칙입니다.'), MultilineInput(name='answer_prompt_extension', display_name='결과 생성 특화 프롬프트', value='', required=False, info='공통 결과 생성 프롬프트 뒤에 연결할 도메인별 표현·주의 규칙입니다.'), MultilineInput(name='specialized_functions_json', display_name='등록 특화 함수 JSON', value='', required=False, info='Typed IR 기본 연산으로 표현하기 어려운 검토 완료 격리 함수 등록 정보입니다.'), MultilineInput(name='output_profile_json', display_name='출력 프로필 JSON', value='', required=False, info='표시 컬럼, 표 미리보기, 다운로드 등 도메인별 기본 출력 설정입니다.'), DropdownInput(name='mode', display_name='저장 모드', options=['save', 'replace', 'validate_only'], value='save', info='save는 신규 key만 허용하고 기존 key 변경을 차단합니다. replace는 동일 section+key 항목을 교체하며 validate_only는 저장하지 않습니다.'), SecretStrInput(name='mongo_uri', display_name='MongoDB 연결 URI', value='', required=False, info='도메인·테이블·메인필터 컬렉션을 저장할 MongoDB 연결 문자열입니다.'), StrInput(name='mongo_database', display_name='MongoDB 데이터베이스', value='', required=False, info='운영 메타데이터가 저장되는 데이터베이스 이름입니다.'), StrInput(name='domain_collection', display_name='도메인 메타데이터 컬렉션', value='agent_v6_domain_metadata', info='자연어 도메인 원문과 검증된 도메인 규칙을 저장합니다.'), StrInput(name='table_collection', display_name='테이블 카탈로그 컬렉션', value='agent_v6_table_catalog', info='자연어 테이블 원문과 검증된 dataset/field binding을 저장합니다.'), StrInput(name='main_filter_collection', display_name='메인필터 컬렉션', value='agent_v6_main_filter', info='자연어 메인필터 원문과 검증된 predicate/alias를 저장합니다.'), IntInput(name='mongo_timeout_ms', display_name='MongoDB 제한 시간(ms)', value=5000, info='MongoDB 연결과 등록 트랜잭션에 적용할 제한 시간(밀리초)입니다.'), BoolInput(name='dry_run', display_name='저장 없는 검증', value=False, info='활성화하면 MongoDB에 저장하지 않고 자연어 변환·컴파일·계약 검증만 수행합니다.')]
    ...
    def _collection_names(self):
        values = {role: str(getattr(self, role, expected) or '').strip() for role, expected in V6_AUTHORING_COLLECTIONS.items()}
        invalid = [role for role, name in values.items() if re.fullmatch('[A-Za-z][A-Za-z0-9_.-]{0,127}', name) is None or name.casefold().startswith('system.')]
        if invalid or len(set(values.values())) != len(values):
            raise ContractError('metadata_policy_error', 'metadata_store_config', 'Metadata collection names must be safe and distinct.', {'invalid_roles': invalid, 'actual': values})
        return values

    def _response(self, status, *, stage, **values):
        material = {'contract_version': 'metadata.authoring.response.v1', 'response_type': 'metadata_authoring', 'status': status, 'stage': stage, 'authoring_kind': str(getattr(self, 'authoring_kind', 'domain')), 'metadata_contract_mode': str(getattr(self, 'metadata_contract_mode', 'domain_package_v2')), 'domain_id': str(getattr(self, 'domain_id', 'default')), 'environment': str(getattr(self, 'environment', 'production')), **values}
        material['response_sha256'] = sha256_json(material)
        return Data(data=validate_authoring_response_hash(material))

    def _save_initial_section_items(self, *, kind, raw_patch, source_text, source_manifest, authoring_proposal_validation, current_documents, collections, write_mode, dry_run, draft_llm_calls):
        """Store one collection-owned section before or after package completion."""
        source_binding_validation = {'contract_version': 'metadata.source-binding.validation.v1', 'status': 'not_applicable', 'binding_authority': 'metadata_three_collections'}
        source_query_extraction = {'contract_version': 'metadata.source-query-extraction.v1', 'status': 'not_applicable', 'query_count': 0, 'dataset_keys': [], 'datasets': {}, 'missing_query_dataset_keys': []}
        normalized_patch = deepcopy(raw_patch)
        if kind == 'domain':
            profile_keys = {'display_name', 'description', 'locale', 'timezone'}
            domain_item_sections = {'entity_groups', 'metrics', 'grains', 'relations', 'orderings', 'predicates', 'recipes'}
            is_profile_only = bool(profile_keys & set(normalized_patch)) and (not any((isinstance(normalized_patch.get(section), dict) for section in domain_item_sections)))
            if is_profile_only:
                normalized_patch['domain_id'] = str(getattr(self, 'domain_id', 'default') or 'default').strip()
        elif kind == 'dataset':
            partial_draft = {'datasets': deepcopy(raw_patch.get('datasets') or {})}
            source_binding_validation = _validate_authoring_source_bindings(partial_draft, source_sha256=str(source_manifest.get('source_sha256') or ''), proposal_sha256=str((authoring_proposal_validation or {}).get('proposal_sha256') or ''), approved_reference_context=getattr(self, 'approved_reference_context', None), domain_id=str(getattr(self, 'domain_id', '') or ''))
            partial_draft, source_query_extraction = apply_dataset_source_configs_from_text(partial_draft, source_text, require_complete=False)
            normalized_patch = {'datasets': partial_draft['datasets']}
        now = _bson_millisecond_utc(datetime.now(timezone.utc))
        candidate_items = make_partial_metadata_item_documents(kind, normalized_patch, source_text, updated_at=now.isoformat())
        merged_items, write_operations = merge_metadata_items_for_write(current_documents, candidate_items, mode=write_mode)
        conflict_count = int(write_operations.get('conflict_count') or 0)
        conflicts = list(write_operations.pop('conflicts', []))
        if conflict_count:
            raise ContractError('metadata_policy_error', 'metadata_duplicate', '기존 메타데이터와 중복되거나 어느 항목을 바꿀지 모호합니다. 표시된 canonical key를 확인해 주세요.', {'conflicts': conflicts, 'conflict_count': conflict_count, 'conflicts_truncated': bool(write_operations.get('conflicts_truncated'))})
        ...
        package = None
        alias_activation = {'contract_version': 'metadata.alias-activation.v1', 'active_count': 0, 'deferred_count': 0, 'deferred': []}
        runtime_activation_issue = {}
        if not missing_sections:
            try:
                package = assemble_domain_package_from_items(merged_items, alias_activation_out=alias_activation)
                ...
                package = None
                runtime_activation_issue = {'code': str(error.code or 'metadata_dependency_error'), 'message': str(error.public_message or '전체 실행 메타데이터가 아직 준비되지 않았습니다.'), 'details': deepcopy(error.details or {})}
        validation = {'schema': 'passed', 'semantic_lint': 'passed' if package is not None else 'deferred', 'dependency_closure': 'passed' if package is not None else 'deferred', 'runtime_compile': 'passed' if package is not None else 'waiting_for_remaining_collections' if missing_sections else 'stored_runtime_pending', 'section_ownership': 'passed', 'three_collection_items': 'complete' if package is not None else 'partial', 'ready_sections': ready_sections, 'missing_sections': missing_sections, 'source_bindings': source_binding_validation, 'source_queries': source_query_extraction, 'runtime_alias_activation': alias_activation}
        ...
        validation['write_policy'] = {'mode': write_mode, 'identity': 'typed_item_identity', 'preserve_unmentioned': True, 'operations': deepcopy(write_operations)}
        candidate_hash = sha256_json({'contract_version': 'metadata.initial-section-candidate.v1', 'authoring_kind': kind, 'write_mode': write_mode, 'item_counts': item_counts, 'runtime_catalog_sha256': str(package['runtime_catalog'].get('catalog_sha256') or '') if package is not None else '', 'validation': validation})
            ...
            try:
                with client.start_session() as session:
                    with session.start_transaction():
                        transaction_documents = {'domain': _find_items(db[collections['domain_collection']], session=session), 'table_catalog': _find_items(db[collections['table_collection']], session=session), 'main_filter': _find_items(db[collections['main_filter_collection']], session=session)}
                        if metadata_item_set_projection(transaction_documents) != metadata_item_set_projection(current_documents):
                            raise ContractError('state_conflict', 'metadata_duplicate', '중복 확인 이후 기존 메타데이터가 변경되었습니다. 다시 실행해 주세요.', {'reason': 'concurrent_metadata_update'}, retryable=True)
                        if package is None:
                            upsert_partial_metadata_items(db, merged_items, session=session, domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection'])
                        else:
                            replace_metadata_items(db, merged_items, session=session, domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection'])
                            persisted = load_domain_package_from_three_collections(db, str(getattr(self, 'domain_id', '') or ''), str(getattr(self, 'environment', 'production') or 'production'), domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection'], session=session)
                            if persisted['runtime_catalog'] != package['runtime_catalog']:
                                raise ContractError('metadata_hash_conflict', 'metadata_three_collection', '저장 후 다시 결합한 메타데이터 package가 컴파일 결과와 다릅니다.')
            finally:
                client.close()
        response_values = {'candidate_id': candidate_id, 'candidate_sha256': candidate_hash, 'revision': int((package or {}).get('revision') or 0), 'persisted': not dry_run, 'idempotent_replay': False, 'diff': {'authoring_kind': kind, 'write_mode': write_mode, 'write_operations': write_operations, 'item_counts': item_counts, 'activation_status': 'ready' if package is not None else 'waiting_for_sections' if missing_sections else 'stored_runtime_pending', 'ready_sections': ready_sections, 'missing_sections': missing_sections}, 'unchanged_section_checks': {}, 'validation': validation, 'llm_usage': {'draft_llm_calls': draft_llm_calls, 'annotation_llm_calls': 0, 'repair_llm_calls': 0}}
        if package is not None:
            response_values.update(package_sha256=package['package_sha256'], bundle_sha256=package['bundle_sha256'], catalog_sha256=package['runtime_catalog']['catalog_sha256'])
        return self._response('ok', stage='validated' if dry_run else 'committed', **response_values)
    ...
    def _prepare_v2(self):
        ...
        if grounding_mode not in {'freeform_llm', 'explicit_inventory'}:
            raise ContractError('metadata_schema_error', 'metadata_source_grounding', 'source_grounding_mode must be freeform_llm or explicit_inventory.')
        ...
        if kind == 'domain' and bool(blueprint_text) != bool(trusted_blueprint_pin):
            raise ContractError('metadata_blueprint_invalid', 'metadata_blueprint', 'Trusted blueprint JSON and its external SHA-256 pin must be configured together.')
        ...
        if split_bootstrap and (kind != 'domain' or grounding_mode != 'freeform_llm' or bool(blueprint_text) or bool(trusted_blueprint_pin)):
            raise ContractError('metadata_policy_error', 'metadata_authoring', '분할 초기 등록은 blueprint가 없는 domain/freeform_llm prepare에서만 사용할 수 있습니다.')
        bootstrap_branches = None
        section_context = None
        single_domain_bootstrap = False
        simple_section_bootstrap = False
        expected_purpose = {'domain': 'metadata_domain_annotation' if annotation_only else 'metadata_domain_draft', 'dataset': 'metadata_dataset_draft', 'main_filter': 'metadata_main_filter_draft', 'domain_policy': 'metadata_domain_policy'}[kind]
        if split_bootstrap:
            bootstrap_branches, source_text = _split_bootstrap_source(self, domain_id=domain_id, environment=environment)
        elif kind != 'domain_policy':
            raw_section_context = getattr(self, 'authoring_source_context', None)
            section_context_payload = getattr(raw_section_context, 'data', raw_section_context)
            if isinstance(section_context_payload, dict) and section_context_payload:
                ...
                simple_section_bootstrap = bool(isinstance(raw_variables, dict) and raw_variables.get('bootstrap_fragment') is True)
                ...
                if simple_section_bootstrap and (not reference_payload):
                    section_context = _collection_authoring_context_payload(self, input_name='authoring_source_context', kind=kind, purpose=expected_purpose, domain_id=domain_id, environment=environment, grounding_mode=grounding_mode)
                else:
                    section_context = _bootstrap_context_payload(self, input_name='authoring_source_context', kind=kind, purpose=expected_purpose, domain_id=domain_id, environment=environment, bootstrap_fragment=simple_section_bootstrap, grounding_mode=grounding_mode, annotation_only=annotation_only, expected_invoke=None)
        ...
        if strict_inventory:
            try:
                ...
                raise ContractError('metadata_dependency_error', 'metadata_source_inventory', 'Explicit-inventory metadata is ambiguous or invalid.', deepcopy(exc.evidence)) from exc
        else:
        ...
        collection_owned_bootstrap = bool(simple_section_bootstrap and (not isinstance(reference_context_payload, dict))) or bool(simple_section_bootstrap and (not reference_context_payload))
        if kind == 'dataset' and strict_inventory:
            inventories = source_manifest.get('inventories')
            datasets = inventories.get('datasets') if isinstance(inventories, dict) else None
            dataset_fields = inventories.get('dataset_fields') if isinstance(inventories, dict) else None
            valid_dataset_inventory = isinstance(datasets, list) and bool(datasets) and isinstance(dataset_fields, dict) and (set(dataset_fields) == set(datasets)) and all((isinstance(dataset_fields.get(dataset_id), list) and bool(dataset_fields[dataset_id]) for dataset_id in datasets))
            if not valid_dataset_inventory:
                counts = source_manifest.get('counts')
                raise ContractError('metadata_dependency_error', 'metadata_source_inventory', 'Dataset authoring requires explicit dataset IDs and canonical field declarations.', {'source_manifest_sha256': str(source_manifest.get('manifest_sha256') or ''), 'dataset_count': int(counts.get('datasets') or 0) if isinstance(counts, dict) else 0, 'field_bindings': int(counts.get('field_bindings') or 0) if isinstance(counts, dict) else 0})
        trusted_blueprint = None
        if annotation_only:
            if len(blueprint_text.encode('utf-8')) > 1048576 or not re.fullmatch('[0-9a-f]{64}', trusted_blueprint_pin):
                raise ContractError('metadata_blueprint_invalid', 'metadata_blueprint', 'Trusted executable blueprint configuration is invalid.')
            try:
                parsed_blueprint = json.loads(blueprint_text)
            except json.JSONDecodeError as exc:
                raise ContractError('metadata_blueprint_invalid', 'metadata_blueprint', 'Trusted executable blueprint JSON is invalid.', {'line': exc.lineno}) from exc
            trusted_blueprint = validate_executable_blueprint(parsed_blueprint, expected_blueprint_sha256=trusted_blueprint_pin, expected_domain_id=domain_id, expected_environment=environment, source_manifest=source_manifest)
        deterministic_alias_patch = _v2_alias_only_manifest_patch(source_manifest) if kind == 'main_filter' and strict_inventory else None
        model_required = kind in {'domain', 'dataset'} or (kind == 'main_filter' and deterministic_alias_patch is None)
        if model_required and (not split_bootstrap) and (section_context is None):
            raise ContractError('metadata_dependency_error', 'metadata_source_context', 'A sealed authoring runtime context is required before any LLM invocation.', {'authoring_kind': kind, 'expected_purpose': expected_purpose})
        if section_context is not None and section_context['invoke'] is not model_required:
            raise ContractError('metadata_dependency_error', 'metadata_source_context', '등록 runtime context의 invoke 결정이 compiler 경로와 일치하지 않습니다.', {'authoring_kind': kind, 'expected_purpose': expected_purpose})
        ...
        if split_bootstrap:
            invocation_specs = {'domain': ('authoring_invocation_result', 'metadata_domain_draft'), 'dataset': ('bootstrap_dataset_invocation_result', 'metadata_dataset_draft'), 'main_filter': ('bootstrap_main_filter_invocation_result', 'metadata_main_filter_draft')}
            raw_proposals = {}
            invocation_error = None
            for branch_kind in BOOTSTRAP_BRANCH_ORDER:
                input_name, branch_purpose = invocation_specs[branch_kind]
                try:
                    raw_proposals[branch_kind] = _authoring_invocation_draft(self, input_name=input_name, expected_purpose=branch_purpose, required=True, expected_output_schema=bootstrap_branches[branch_kind]['output_schema'], expected_runtime_context_sha256=bootstrap_branches[branch_kind]['runtime_context_sha256'])
                except ContractError as exc:
            ...
            proposal_evidence = {}
            for branch_kind in BOOTSTRAP_BRANCH_ORDER:
                fragment, evidence = _unwrap_bootstrap_authoring_proposal(raw_proposals[branch_kind], kind=branch_kind, source_sha256=bootstrap_branches[branch_kind]['source_sha256'], composite_source_sha256=str(source_manifest.get('source_sha256') or ''))
                fragments[branch_kind] = fragment
                if branch_kind == 'dataset':
                    evidence = {**evidence, 'dataset_ir_contract_version': 'metadata.bootstrap.dataset-ir.v1', 'dataset_ir_expander_version': 'metadata.dataset-ir-expander.v1', 'dataset_ir_sha256': str(evidence.get('draft_sha256') or '')}
            ...
            approved_planner_policy = deepcopy((registry_context.get('semantic_templates') or {}).get('planner_policy') or {})
            fragments['domain'], domain_template_expansion = _expand_bootstrap_domain_annotation(fragments['domain'], semantic_templates=registry_context.get('semantic_templates'), semantic_vocabulary=registry_context.get('semantic_vocabulary'))
            proposal_evidence['domain'] = {**proposal_evidence['domain'], 'template_expansion': deepcopy(domain_template_expansion)}
            dataset_reconciliation = {}
            main_filter_reconciliation = {}
            invocation_draft = _merge_bootstrap_fragments(fragments, dataset_descriptors=registry_context.get('dataset_descriptors'), semantic_vocabulary=registry_context.get('semantic_vocabulary'), reconciliation_out=dataset_reconciliation, main_filter_reconciliation_out=main_filter_reconciliation, domain_already_expanded=True)
            invocation_draft, metric_binding_completion = _complete_unambiguous_metric_bindings(invocation_draft, registry_context.get('semantic_vocabulary'))
            invocation_draft, semantic_alias_normalization = _normalize_bootstrap_alias_shorthand(invocation_draft, registry_context.get('semantic_vocabulary'))
            proposal_evidence['dataset'] = {**proposal_evidence['dataset'], 'registry_reconciliation': dataset_reconciliation}
            proposal_evidence['main_filter'] = {**proposal_evidence['main_filter'], 'main_filter_ir_expansion': main_filter_reconciliation}
            bootstrap_raw_draft_sha256 = sha256_json(invocation_draft)
            proposal_hashes = {branch_kind: proposal_evidence[branch_kind]['proposal_sha256'] for branch_kind in BOOTSTRAP_BRANCH_ORDER}
            authoring_proposals_validation = {branch_kind: deepcopy(proposal_evidence[branch_kind]) for branch_kind in BOOTSTRAP_BRANCH_ORDER}
            authoring_proposal_validation = {'contract_version': 'metadata.authoring.proposal.validation.v1', 'proposal_contract_version': 'metadata.authoring.proposal.v1', 'status': 'complete', 'source_sha256': str(source_manifest.get('source_sha256') or ''), 'proposal_sha256': sha256_json(proposal_hashes), 'draft_sha256': bootstrap_raw_draft_sha256, 'expanded_draft_sha256': bootstrap_raw_draft_sha256, 'dataset_ir_sha256': str(proposal_evidence['dataset'].get('dataset_ir_sha256') or ''), 'dataset_ir_expander_version': 'metadata.dataset-ir-expander.v1', 'domain_template_expander_version': 'metadata.domain-template-expansion.v1', 'semantic_templates_sha256': str((domain_template_expansion or {}).get('semantic_templates_sha256') or '')}
        else:
            invocation_draft = _authoring_invocation_draft(self, input_name='authoring_invocation_result', expected_purpose=expected_purpose, required=model_required, expected_output_schema=section_context.get('output_schema') if section_context is not None and model_required else None, expected_runtime_context_sha256=section_context.get('runtime_context_sha256') if section_context is not None and model_required else '')
            if model_required and grounding_mode == 'freeform_llm' and (not annotation_only):
                invocation_draft, authoring_proposal_validation = _unwrap_freeform_authoring_proposal(invocation_draft, source_sha256=str(source_manifest.get('source_sha256') or ''))
            if model_required and single_domain_bootstrap and (not collection_owned_bootstrap):
                raw_reference = getattr(self, 'approved_reference_context', None)
                registry_context = getattr(raw_reference, 'data', raw_reference)
                registry_context = registry_context if isinstance(registry_context, dict) else {}
                approved_planner_policy = deepcopy((registry_context.get('semantic_templates') or {}).get('planner_policy') or {})
                domain_fragment, domain_template_expansion = _expand_bootstrap_domain_annotation(invocation_draft, semantic_templates=registry_context.get('semantic_templates'), semantic_vocabulary=registry_context.get('semantic_vocabulary'))
                dataset_fragment, main_filter_fragment = _deterministic_registry_bootstrap_fragments(registry_context)
                dataset_reconciliation = {}
                main_filter_reconciliation = {}
                invocation_draft = _merge_bootstrap_fragments({'domain': domain_fragment, 'dataset': dataset_fragment, 'main_filter': main_filter_fragment}, dataset_descriptors=registry_context.get('dataset_descriptors'), semantic_vocabulary=registry_context.get('semantic_vocabulary'), reconciliation_out=dataset_reconciliation, main_filter_reconciliation_out=main_filter_reconciliation, domain_already_expanded=True)
                invocation_draft, metric_binding_completion = _complete_unambiguous_metric_bindings(invocation_draft, registry_context.get('semantic_vocabulary'))
                invocation_draft, semantic_alias_normalization = _normalize_bootstrap_alias_shorthand(invocation_draft, registry_context.get('semantic_vocabulary'))
                if authoring_proposal_validation is not None:
                    authoring_proposal_validation = {**authoring_proposal_validation, 'expanded_draft_sha256': sha256_json(invocation_draft), 'domain_template_expansion': deepcopy(domain_template_expansion), 'dataset_registry_reconciliation': dataset_reconciliation, 'main_filter_registry_reconciliation': main_filter_reconciliation, 'single_llm_registry_bootstrap': True}
            if model_required and collection_owned_bootstrap and (kind == 'domain'):
                ...
                invocation_draft = _expand_freeform_domain_fragment(invocation_draft, reconciliation_out=section_ir_reconciliation, source_text=source_text)
                if authoring_proposal_validation is not None:
                    authoring_proposal_validation = {**(authoring_proposal_validation or {}), 'compact_ir_sha256': compact_ir_sha256, 'expanded_draft_sha256': sha256_json(invocation_draft), 'section_ir_expander_version': 'metadata.freeform-domain-expansion.v1', 'section_ir_reconciliation': section_ir_reconciliation}
            if model_required and kind in {'dataset', 'main_filter'}:
                ...
                if kind == 'dataset':
                    if registry_context:
                        invocation_draft = _expand_compact_dataset_fragment(invocation_draft, dataset_descriptors=registry_context.get('dataset_descriptors'), reconciliation_out=section_ir_reconciliation)
                        expander_version = 'metadata.dataset-ir-expander.v1'
                    else:
                        invocation_draft = _expand_freeform_dataset_fragment(invocation_draft, source_text, reconciliation_out=section_ir_reconciliation)
                        expander_version = 'metadata.freeform-dataset-expansion.v1'
                else:
                    if not registry_context:
                        invocation_draft = _expand_freeform_main_filter_fragment(invocation_draft)
                    invocation_draft = _expand_compact_main_filter_fragment(invocation_draft, approved_semantic_vocabulary=registry_context.get('semantic_vocabulary'), allow_unresolved=not bool(registry_context), reconciliation_out=section_ir_reconciliation)
                    expander_version = 'metadata.main-filter-ir-expansion.v1' if registry_context else 'metadata.collection-main-filter-items.v1'
                if authoring_proposal_validation is not None:
                    authoring_proposal_validation = {**(authoring_proposal_validation or {}), 'compact_ir_sha256': compact_ir_sha256, 'expanded_draft_sha256': sha256_json(invocation_draft), 'section_ir_expander_version': expander_version, 'section_ir_reconciliation': section_ir_reconciliation}
        ...
        if kind == 'domain' and (not annotation_only) and (not collection_owned_bootstrap):
            prevalidated_domain_draft = _enforce_domain_policy_boundary(invocation_draft, source_sha256=str(source_manifest.get('source_sha256') or ''), proposal_sha256=str((authoring_proposal_validation or {}).get('proposal_sha256') or ''), grounding_mode=grounding_mode, approved_planner_policy=approved_planner_policy)
            if split_bootstrap or single_domain_bootstrap:
                prevalidated_source_binding = _validate_authoring_source_bindings(prevalidated_domain_draft, source_sha256=str(source_manifest.get('source_sha256') or ''), proposal_sha256=str((authoring_proposal_validation or {}).get('proposal_sha256') or ''), approved_reference_context=getattr(self, 'approved_reference_context', None), domain_id=domain_id, require_registry_exact_set=True)
            if not split_bootstrap:
                raw_reference = getattr(self, 'approved_reference_context', None)
                registry_context = getattr(raw_reference, 'data', raw_reference)
                registry_context = registry_context if isinstance(registry_context, dict) else {}
                prevalidated_domain_draft, metric_binding_completion = _complete_unambiguous_metric_bindings(prevalidated_domain_draft, registry_context.get('semantic_vocabulary'))
            prevalidated_domain_draft, filter_operator_normalization = _normalize_filter_operator_aliases(prevalidated_domain_draft)
            prevalidated_domain_draft = validate_contract(prevalidated_domain_draft, 'metadata-authoring-draft.schema.json', stage='metadata_section_patch', error_code='metadata_schema_error')
        ...
        if not dry_run or requested_write_mode == 'validate_only' or (explicit_dry_run and requested_write_mode != 'prepare') or (kind != 'domain' and active_package is None):
            client, db = self._mongo()
            try:
                current_documents = {'domain': list(db[collections['domain_collection']].find({})), 'table_catalog': list(db[collections['table_collection']].find({})), 'main_filter': list(db[collections['main_filter_collection']].find({}))}
                ...
                if len(present) == 3:
                    ...
                    current_source_texts = {name: '\n\n'.join(dict.fromkeys((str(item.get('natural_text') or '').strip() for item in value if str(item.get('natural_text') or '').strip())))[:65536] for name, value in current_documents.items()}
                    loaded_package = load_domain_package_from_three_collections(db, domain_id, environment, domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection']) if active_package is not None else None
                ...
                client.close()
                client = db = None
        base_draft = runtime_catalog_v2_to_authoring_draft(active_package['runtime_catalog']) if kind != 'domain' and active_package is not None else None
        blueprint_validation = None
        if kind == 'domain':
            if annotation_only:
                raw_annotations = deepcopy(invocation_draft)
                annotations = validate_contract(raw_annotations, 'metadata-annotation-proposal.schema.json', stage='metadata_annotation_proposal', error_code='metadata_schema_error')
                raw_patch = merge_blueprint_annotations(trusted_blueprint, annotations, expected_blueprint_sha256=trusted_blueprint_pin, expected_domain_id=domain_id, expected_environment=environment, source_manifest=source_manifest)
                blueprint_validation = {'contract_version': 'metadata.blueprint.validation.v1', 'blueprint_sha256': trusted_blueprint['blueprint_sha256'], 'executable_sha256': trusted_blueprint['executable_sha256'], 'annotation_proposal_sha256': sha256_json(annotations), 'external_pin': 'passed', 'executable_immutable': 'passed'}
            else:
                raw_patch = deepcopy(invocation_draft if collection_owned_bootstrap else prevalidated_domain_draft)
            ...
            if kind == 'dataset' and grounding_mode == 'freeform_llm' and (base_draft is not None):
                normalization_manifest = _freeform_dataset_patch_authorization_manifest(source_manifest, base_draft)
            if kind == 'main_filter' and model_required:
                if base_draft is not None:
                    alias_base = base_draft
                else:
                    alias_base = {'aliases': {str(item.get('key') or ''): deepcopy(item.get('payload') or {}) for items in current_documents.values() for item in items if isinstance(item, dict) and item.get('section') == 'aliases' and str(item.get('key') or '')}}
                ...
                if not isinstance(raw_patch, dict) or set(raw_patch) != {'datasets'} or (not isinstance(raw_patch.get('datasets'), dict)) or (not raw_patch['datasets']):
                    raise ContractError('metadata_schema_error', 'metadata_section_patch', '최초 데이터셋 등록 제안은 비어 있지 않은 datasets patch여야 합니다.')
            else:
                try:
                    raw_patch = normalize_authoring_section_patch_shorthand(normalization_manifest, raw_patch, kind, base_draft=base_draft)
                except AuthoringSourceManifestError as exc:
                    raise ContractError('metadata_dependency_error', 'metadata_alias_normalization', 'Compiled metadata contains an alias shorthand that is not bound by the source inventory.', {**deepcopy(exc.evidence), 'manifest_error_code': exc.code}) from exc
        else:
            raw_patch = {}
        policy_input_values = {'intent': str(getattr(self, 'intent_prompt_extension', '') or '').strip(), 'answer': str(getattr(self, 'answer_prompt_extension', '') or '').strip(), 'specialized_functions': str(getattr(self, 'specialized_functions_json', '') or '').strip(), 'output_profile': str(getattr(self, 'output_profile_json', '') or '').strip()}
        if kind != 'domain_policy' and any(policy_input_values.values()):
            raise ContractError('metadata_schema_error', 'metadata_policy_input', 'Domain policy inputs may only be used when Authoring Kind is domain_policy.')
        if kind == 'domain_policy':
            if not any(policy_input_values.values()):
                raise ContractError('metadata_schema_error', 'metadata_policy_input', 'Domain policy prepare requires at least one explicit operator input.')
            prompt_patch = raw_patch.get('prompt_extensions')
            if prompt_patch is None:
                prompt_patch = {}
            if not isinstance(prompt_patch, dict):
                raise ContractError('metadata_schema_error', 'metadata_policy_input', 'prompt_extensions must be an object.')
            ...
            functions_value = _strict_json_value(policy_input_values['specialized_functions'], label='Specialized Functions JSON', expected_type=list)
            if functions_value is not None:
                raw_patch['specialized_functions'] = functions_value
            output_value = _strict_json_value(policy_input_values['output_profile'], label='Output Profile JSON', expected_type=dict)
            if output_value is not None:
                reserved_planner_keys = {'planner_profile', 'legacy_catalog_sha256'}
                if reserved_planner_keys.intersection(output_value):
                    raise ContractError('metadata_policy_error', 'metadata_policy_input', '실행 planner 호환 정책은 승인 템플릿이 소유하며 출력 프로필 입력으로 변경할 수 없습니다.')
                raw_patch['output_profile'] = output_value
        if collection_owned_bootstrap or (active_package is None and kind in {'dataset', 'main_filter'}):
            return self._save_initial_section_items(kind=kind, raw_patch=raw_patch, source_text=source_text, source_manifest=source_manifest, authoring_proposal_validation=authoring_proposal_validation, current_documents=current_documents, collections=collections, write_mode=write_mode, dry_run=dry_run, draft_llm_calls=draft_llm_calls)
        if kind == 'domain':
            if not annotation_only and prevalidated_domain_draft is None:
                raw_patch = _enforce_domain_policy_boundary(raw_patch, source_sha256=str(source_manifest.get('source_sha256') or ''), proposal_sha256=str((authoring_proposal_validation or {}).get('proposal_sha256') or ''), grounding_mode=grounding_mode, approved_planner_policy=approved_planner_policy)
            draft = validate_contract(raw_patch, 'metadata-authoring-draft.schema.json', stage='metadata_section_patch', error_code='metadata_schema_error')
            unchanged_sections = {}
        else:
            if not isinstance(base_draft, dict):
                raise ContractError('metadata_dependency_error', 'metadata_section_patch', 'Validated active authoring draft is unavailable.')
            draft = apply_authoring_section_patch(base_draft, raw_patch, kind)
            owned = {'datasets'} if kind == 'dataset' else {'aliases', 'entity_groups', 'grains', 'orderings', 'predicates', 'recipes'} if kind == 'main_filter' else {'prompt_extensions', 'specialized_functions', 'output_profile'}
            unchanged_sections = {section: sha256_json(base_draft.get(section)) == sha256_json(draft.get(section)) for section in base_draft if section not in owned and section not in {'source_provenance'}}
        ...
        source_binding_validation = prevalidated_source_binding or _validate_authoring_source_bindings(draft, source_sha256=str(source_manifest.get('source_sha256') or ''), proposal_sha256=str((authoring_proposal_validation or {}).get('proposal_sha256') or ''), approved_reference_context=getattr(self, 'approved_reference_context', None), domain_id=domain_id)
        query_source_text = source_text
        if split_bootstrap:
            query_source_text = str(bootstrap_branches['dataset']['source_text'])
        draft, source_query_extraction = apply_dataset_source_configs_from_text(draft, query_source_text, require_complete=False)
        if strict_inventory:
            try:
                coverage = validate_draft_inventory_coverage(source_manifest, draft, supported_operations=AUTHORING_SUPPORTED_OPERATIONS)
            except AuthoringSourceManifestError as exc:
                raise ContractError('metadata_dependency_error', 'metadata_source_coverage', 'Compiled metadata does not cover the explicit source inventory.', deepcopy(exc.evidence)) from exc
        else:
            proposal_grounding_sha256 = bootstrap_raw_draft_sha256 if split_bootstrap else str((authoring_proposal_validation or {}).get('expanded_draft_sha256') or (authoring_proposal_validation or {}).get('draft_sha256') or sha256_json(draft))
            coverage = {'contract_version': 'source.grounding.evidence.v1', 'mode': 'freeform_llm', 'source_sha256': str(source_manifest.get('source_sha256') or ''), 'structured_proposal_sha256': proposal_grounding_sha256, 'explicit_inventory_coverage': 'not_requested', 'schema_validation': 'passed', 'dependency_closure': 'passed', 'human_approval_required': True}
        ...
        if revision <= active_revision:
            if client is not None:
                client.close()
            raise ContractError('metadata_revision_conflict', 'metadata_prepare', 'Prepared revision must be greater than the active revision.', {'active_revision': active_revision, 'requested_revision': revision})
        ...
        if strict_inventory:
            try:
                coverage = validate_draft_inventory_coverage(source_manifest, package['runtime_catalog'], supported_operations=AUTHORING_SUPPORTED_OPERATIONS)
            except AuthoringSourceManifestError as exc:
                raise ContractError('metadata_dependency_error', 'metadata_source_coverage', 'Compiled metadata does not cover the explicit source inventory.', deepcopy(exc.evidence)) from exc
        else:
            coverage = {**coverage, 'compiled_catalog_sha256': str(package['runtime_catalog'].get('catalog_sha256') or '')}
        coverage = {**coverage, 'coverage_sha256': sha256_json(coverage)}
        now = _bson_millisecond_utc(datetime.now(timezone.utc))
        validation = {'schema': 'passed', 'semantic_lint': 'passed', 'dependency_closure': 'passed', 'runtime_compile': 'passed', 'section_ownership': 'passed', 'three_collection_items': 'passed', 'source_coverage': coverage, 'source_bindings': source_binding_validation, 'source_queries': source_query_extraction}
        observed_schema_bindings = deepcopy(getattr(self, '_observed_authoring_schema_bindings', None) or {})
        if observed_schema_bindings:
            validation['llm_schema_bindings'] = {'contract_version': 'metadata.llm-schema-bindings.v1', 'bindings': [observed_schema_bindings[purpose] for purpose in sorted(observed_schema_bindings)]}
        if split_bootstrap and authoring_proposal_validation is not None:
            authoring_proposal_validation = {**authoring_proposal_validation, 'sealed_authoring_sha256': sha256_json(draft)}
        if authoring_proposal_validation is not None:
            validation['authoring_proposal'] = authoring_proposal_validation
        ...
        if filter_operator_normalization is not None:
            validation['filter_operator_normalization'] = filter_operator_normalization
        ...
        if split_bootstrap:
            source_texts = {'domain': str(bootstrap_branches['domain']['source_text']), 'table_catalog': str(bootstrap_branches['dataset']['source_text']), 'main_filter': str(bootstrap_branches['main_filter']['source_text'])}
        elif kind == 'domain':
        ...
        candidate_item_documents = make_metadata_item_documents(package, source_texts, updated_at=now.isoformat())
        if kind == 'domain' and active_package is None and (requested_write_mode != 'prepare'):
            candidate_item_documents = {'domain': candidate_item_documents['domain'], 'table_catalog': [], 'main_filter': []}
        merged_item_documents, write_operations = merge_metadata_items_for_write(current_documents, candidate_item_documents, mode=write_mode)
        conflict_count = int(write_operations.get('conflict_count') or 0)
        conflicts = list(write_operations.pop('conflicts', []))
        if conflict_count:
            raise ContractError('metadata_policy_error', 'metadata_duplicate', '기존 메타데이터와 중복되거나 어느 항목을 바꿀지 모호합니다. 표시된 canonical key를 확인해 주세요.', {'conflicts': conflicts, 'conflict_count': conflict_count, 'conflicts_truncated': bool(write_operations.get('conflicts_truncated'))})
        missing_sections = sorted((name for name, items in merged_item_documents.items() if not items))
        if any(current_documents.values()) and (not missing_sections):
            ...
            merged_draft = runtime_catalog_v2_to_authoring_draft(merged_projection['runtime_catalog'])
            package = compile_domain_package(merged_draft, domain_id, environment, revision=revision, lifecycle_status='validated')
            package = validate_domain_package(package)
            validate_runtime_catalog_v2(package['runtime_catalog'])
        ...
        if package is None:
            validation.update(semantic_lint='deferred', dependency_closure='deferred', runtime_compile='waiting_for_remaining_collections', three_collection_items='partial', ready_sections=sorted((name for name, items in item_documents.items() if items)), missing_sections=missing_sections)
        validation['write_policy'] = {'mode': write_mode, 'identity': 'typed_item_identity', 'preserve_unmentioned': True, 'operations': deepcopy(write_operations)}
        candidate_hash = sha256_json({'contract_version': 'metadata.save-candidate.v1', 'write_mode': write_mode, 'item_counts': item_counts, 'runtime_catalog_sha256': package['runtime_catalog']['catalog_sha256'] if package is not None else '', 'validation': validation})
        candidate_id = f'candidate:{candidate_hash}'
        if not dry_run:
            client, db = self._mongo()
            try:
                with client.start_session() as session:
                    with session.start_transaction():
                        ...
                        if metadata_item_set_projection(transaction_documents) != metadata_item_set_projection(current_documents):
                            raise ContractError('state_conflict', 'metadata_duplicate', '중복 확인 이후 기존 메타데이터가 변경되었습니다. 다시 실행해 주세요.', {'reason': 'concurrent_metadata_update'}, retryable=True)
                        if package is None:
                            upsert_partial_metadata_items(db, item_documents, session=session, domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection'])
                        else:
                            replace_metadata_items(db, item_documents, session=session, domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection'])
                            persisted = load_domain_package_from_three_collections(db, domain_id, environment, domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection'], session=session)
                            if persisted['runtime_catalog'] != package['runtime_catalog']:
                                raise ContractError('metadata_hash_conflict', 'metadata_three_collection', '저장 후 다시 결합한 메타데이터 package가 컴파일 결과와 다릅니다.')
            except ContractError:
                raise
                ...
                transient = bool(callable(has_error_label) and (has_error_label('TransientTransactionError') or has_error_label('UnknownTransactionCommitResult')))
                if transient or error_code in {112, 251} or 'WriteConflict' in exc.__class__.__name__:
                    raise ContractError('state_conflict', 'metadata_duplicate', '다른 작업자의 메타데이터 저장과 충돌했습니다. 다시 실행해 주세요.', {'reason': 'concurrent_metadata_write'}, retryable=True) from exc
                raise
            finally:
                client.close()
        catalog = package['runtime_catalog'] if package is not None else {}
        response_values = {'candidate_id': candidate_id, 'candidate_sha256': candidate_hash, 'revision': int((package or {}).get('revision') or 0), 'persisted': not dry_run, 'idempotent_replay': False, 'diff': {'authoring_kind': kind, 'write_mode': write_mode, 'write_operations': write_operations, 'item_counts': item_counts, 'activation_status': 'ready' if package is not None else 'waiting_for_sections', 'ready_sections': sorted((name for name, items in item_documents.items() if items)), 'missing_sections': missing_sections, 'datasets': len(catalog.get('datasets') or {}), 'fields': len(catalog.get('fields') or {}), 'metrics': len(catalog.get('metrics') or {}), 'relations': len(catalog.get('relations') or {}), 'recipes': len(catalog.get('recipes') or {})}, 'unchanged_section_checks': unchanged_sections, 'validation': validation, 'llm_usage': {'draft_llm_calls': draft_llm_calls, 'annotation_llm_calls': 1 if annotation_only else 0, 'repair_llm_calls': 0}}
        if package is not None:
            response_values.update(package_sha256=package['package_sha256'], bundle_sha256=package['bundle_sha256'], catalog_sha256=catalog['catalog_sha256'])
        return self._response('ok', stage='validated' if dry_run else 'committed', **response_values)

    def _prepare(self):
        contract_mode = str(getattr(self, 'metadata_contract_mode', 'domain_package_v2'))
        if contract_mode != 'domain_package_v2':
            raise ContractError('metadata_policy_error', 'metadata_contract_mode', 'Only domain_package_v2 authoring is executable; legacy projection is migration-tool only.')
        return self._prepare_v2()

    def run_authoring(self) -> Data:
        self._observed_authoring_llm_usage = {'draft_llm_calls': 0, 'annotation_llm_calls': 0, 'repair_llm_calls': 0}
        ...
        try:
            ...
            if exc.code == 'metadata_clarification_required' and isinstance(exc.details, dict):
                safe_questions, safe_missing_fields = _worker_safe_clarification(list(exc.details.get('questions') or [])[:3], list(exc.details.get('missing_fields') or [])[:32])
                clarification = {'contract_version': 'metadata.authoring.clarification.v1', 'questions': safe_questions, 'missing_fields': safe_missing_fields, 'source_sha256': str(exc.details.get('source_sha256') or ''), 'proposal_sha256': str(exc.details.get('proposal_sha256') or '')}
                return self._response('needs_clarification', stage='metadata_clarification', clarification=clarification, llm_usage=usage)
            return self._response('error', stage=exc.stage, error=exc.as_dict(), llm_usage=usage)
        except Exception as exc:
            error = ContractError('metadata_authoring_failed', 'metadata_runtime', 'metadata authoring 실행 중 계약 오류가 발생했습니다.', {'error_type': type(exc).__name__, 'error_location': _safe_exception_location(exc)})
            usage = deepcopy(getattr(self, '_observed_authoring_llm_usage', None) or {})
            usage.setdefault('draft_llm_calls', 0)
            usage.setdefault('annotation_llm_calls', 0)
            usage['repair_llm_calls'] = 0
