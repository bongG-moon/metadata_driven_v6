...
class MetadataAuthoringEngine(Component):
    ...
    def _prepare_v2(self):
        ...
        item_documents = make_metadata_item_documents(package, source_texts, updated_at=now.isoformat())
        item_counts = {name: len(items) for name, items in item_documents.items()}
        candidate_hash = sha256_json({'contract_version': 'metadata.save-candidate.v1', 'domain_id': domain_id, 'item_counts': item_counts, 'runtime_catalog_sha256': package['runtime_catalog']['catalog_sha256'], 'validation': validation})
