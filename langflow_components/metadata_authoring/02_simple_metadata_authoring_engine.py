...
class MetadataAuthoringEngine(Component):
    ...
    def _prepare_v2(self):
        ...
        if not dry_run or requested_write_mode == 'validate_only' or (explicit_dry_run and requested_write_mode != 'prepare') or (kind != 'domain' and active_package is None):
            ...
            try:
                ...
                if present and len(present) != 3:
                    raise ContractError('metadata_dependency_error', 'metadata_three_collection', '기존 메타데이터 release가 세 컬렉션에 완전하게 존재하지 않습니다.', {'present_sections': present})
                if len(present) == 3:
        ...
        if kind != 'domain' and active_package is None:
            raise ContractError('metadata_dependency_error', 'metadata_prepare', 'Section patches require an exact active v2 domain package or inline dry-run base.')
        base_draft = runtime_catalog_v2_to_authoring_draft(active_package['runtime_catalog']) if kind != 'domain' else None
        ...
        candidate_hash = sha256_json({'contract_version': 'metadata.save-candidate.v1', 'write_mode': write_mode, 'item_counts': item_counts, 'runtime_catalog_sha256': package['runtime_catalog']['catalog_sha256'], 'validation': validation})
