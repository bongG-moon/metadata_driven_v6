...
def _authoring_invocation_draft(component, *, input_name, expected_purpose, required, expected_output_schema=None, expected_runtime_context_sha256=''):
    ...
    if expected_output_schema is not None:
        ...
        if schema_errors:
            exc = schema_errors[0]
            raise ContractError('metadata_schema_error', 'metadata_llm_schema_validation', 'LLM output does not satisfy the authoritative metadata schema.', {'input_name': input_name, 'expected_purpose': expected_purpose, 'path': list(exc.absolute_path), 'validator': str(exc.validator or '')[:80], 'response_bytes': response_bytes, 'response_sha256': observed_response_sha256})
...
def _expand_compact_main_filter_fragment(fragment, *, approved_semantic_vocabulary, reconciliation_out=None):
    ...
    allowed_ids = {target_type: {str(card.get('id') or '') for card in vocabulary.get(section) or [] if isinstance(card, dict)} for target_type, section in section_by_type.items()}
    aliases = {}
    normalized_targets = []
    for index, addition in enumerate(additions):
        ...
        if target_type not in allowed_ids or target_id not in allowed_ids[target_type]:
...
def _validate_authoring_source_bindings(draft, *, source_sha256='', proposal_sha256='', approved_reference_context=None, domain_id='', require_registry_exact_set=False):
    ...
    if not registry:
        clarification_material = {'questions': ['이 도메인에서 사용할 승인 Source 레지스트리를 운영자가 먼저 연결해 주세요.'], 'missing_fields': ['approved_reference_context'], 'source_sha256': str(source_sha256 or '')}
        raise ContractError('metadata_clarification_required', 'metadata_clarification', '작업자 자연어에서 source registry ID를 추측하지 않도록 운영자 레지스트리 연결이 필요합니다.', {'contract_version': 'metadata.authoring.clarification.v1', **clarification_material, 'proposal_sha256': str(proposal_sha256 or '') or sha256_json(clarification_material)})
...
class MetadataAuthoringEngine(Component):
    ...
    def _save_initial_section_items(self, *, kind, raw_patch, source_text, source_manifest, authoring_proposal_validation, current_documents, collections, write_mode, dry_run, draft_llm_calls):
        ...
        source_binding_validation = {'contract_version': 'metadata.source-binding.validation.v1', 'status': 'not_applicable', 'binding_authority': 'approved_semantic_vocabulary'}
        ...
        validation = {'schema': 'passed', 'semantic_lint': 'passed' if package is not None else 'deferred', 'dependency_closure': 'passed' if package is not None else 'deferred', 'runtime_compile': 'passed' if package is not None else 'waiting_for_remaining_collections', 'section_ownership': 'passed', 'three_collection_items': 'complete' if package is not None else 'partial', 'ready_sections': ready_sections, 'missing_sections': missing_sections, 'source_bindings': source_binding_validation, 'source_queries': source_query_extraction}
    ...
    def _prepare_v2(self):
        ...
        if split_bootstrap:
            ...
            if isinstance(section_context_payload, dict) and section_context_payload:
                ...
                single_domain_bootstrap = bool(kind == 'domain' and isinstance(raw_variables, dict) and (raw_variables.get('bootstrap_fragment') is True))
                section_context = _bootstrap_context_payload(self, input_name='authoring_source_context', kind=kind, purpose=expected_purpose, domain_id=domain_id, environment=environment, bootstrap_fragment=single_domain_bootstrap, grounding_mode=grounding_mode, annotation_only=annotation_only, expected_invoke=None)
        ...
        if split_bootstrap:
            ...
            if model_required and kind in {'dataset', 'main_filter'}:
                ...
                if kind == 'dataset':
                    ...
                    invocation_draft = _expand_compact_main_filter_fragment(invocation_draft, approved_semantic_vocabulary=registry_context.get('semantic_vocabulary'), reconciliation_out=section_ir_reconciliation)
        ...
        if not dry_run or requested_write_mode == 'validate_only' or (explicit_dry_run and requested_write_mode != 'prepare') or (kind != 'domain' and active_package is None):
            ...
            try:
                ...
                if len(present) == 3:
                    ...
                    loaded_package = load_domain_package_from_three_collections(db, domain_id, environment, domain_collection=collections['domain_collection'], table_collection=collections['table_collection'], main_filter_collection=collections['main_filter_collection'])
