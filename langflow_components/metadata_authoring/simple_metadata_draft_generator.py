...
class AuthoringPromptContextBuilder(Component):
    ...
    def build_context(self) -> Data:
        ...
        if bootstrap_fragment and (not reference_context):
            raise ContractError('metadata_dependency_error', 'metadata_prompt_context', '분할 초기 등록에는 승인 축약 의미 어휘가 필요합니다.')
...
def _pipeline_error(current, exc, stage):
    ...
    if isinstance(exc, ContractError):
        ...
        error = ContractError('plan_contract_error', stage, '분석 파이프라인 계약 오류가 발생했습니다.', {'error_type': type(exc).__name__}).as_dict(trace_id)
    context.update({'contract_version': PIPELINE_VERSION, 'ok': False, 'stage': stage, 'request': request, 'error': error})
    return context
...
def _planner_profile(catalog):
    if not isinstance(catalog, dict) or catalog.get('contract_version') != 'metadata.runtime.catalog.v2':
        return 'legacy_v1'
    allowed = {'filter', 'project', 'aggregate', 'join', 'derive', 'compare_fields', 'sort', 'rank', 'transform_previous_result', 'registered_call'}
    ...
    if profile.get('planner_profile') == 'legacy_v1_compat':
        ...
        compiler_ok = str(catalog.get('compiler_version') or '') in {'metadata-domain-compiler.v6.2', 'metadata-domain-compiler.v6.3'}
        if expected and expected == actual and identity_ok and compiler_ok:
            return 'legacy_v1_compat'
    raise ContractError('unsupported_operation', 'planner_profile', 'The active domain requires typed operators that are not supported by the generic v2 planner.', {'registered_operations': sorted(operations), 'profile': profile.get('planner_profile')})
...
class AuthoringReferenceRegistry(Component):
    ...
    inputs = [MultilineInput(name='registry_json', display_name='승인 업무 어휘·원천 레지스트리 JSON', value='', required=True, info='운영자가 검토한 축약 업무 어휘와 데이터셋별 어댑터·설정·조회 식별자를 포함합니다. 비밀값이나 실행 쿼리는 입력하지 않습니다.'), StrInput(name='domain_id', display_name='도메인 ID', value='default', required=True, info='레지스트리가 적용되는 업무 도메인의 고유 식별자입니다. 공유 시 대상 도메인 ID로 바꿉니다.')]
    outputs = [Output(name='reference_context', display_name='검증된 업무 어휘·원천 컨텍스트', method='load_registry', types=['Data'])]
    ...
    def load_registry(self) -> Data:
        ...
        if not isinstance(registry, dict) or set(registry) != {'contract_version', 'domain_id', 'datasets', 'semantic_vocabulary', 'semantic_templates', 'semantic_templates_sha256', 'semantic_templates_blueprint_sha256', 'semantic_templates_executable_sha256', 'semantic_templates_projection_sha256'}:
            raise ValueError('승인 Source 레지스트리 root 계약이 닫혀 있지 않습니다.')
        domain_id = str(getattr(self, 'domain_id', '') or '').strip()
        if registry.get('contract_version') != 'metadata.authoring.source-registry.v3' or registry.get('domain_id') != domain_id or (not re.fullmatch('[a-z][a-z0-9_-]{1,63}', domain_id)):
        ...
        for dataset_id in sorted(datasets):
            ...
            required = {'source_type', 'source_adapter', 'config_ref', 'query_ref', 'family', 'field_descriptors', 'proposal_exclusions', 'dataset_template', 'dataset_template_sha256'}
            if set(card) != required:
                raise ValueError('승인 Source binding은 실행 참조, family, field_descriptors만 포함해야 합니다.')
            normalized = {key: str(card.get(key) or '').strip() for key in ('source_type', 'source_adapter', 'config_ref', 'query_ref')}
            joined = json.dumps(normalized, ensure_ascii=False).casefold()
            if normalized['source_type'] not in source_types or not adapter_pattern.fullmatch(normalized['source_adapter']) or (not ref_pattern.fullmatch(normalized['config_ref'])) or (not normalized['config_ref'].startswith('config:')) or (not ref_pattern.fullmatch(normalized['query_ref'])) or (not normalized['query_ref'].startswith('query:')) or any((fragment in joined for fragment in forbidden)):
            ...
            for field_id in sorted(raw_descriptors):
                ...
                if not id_pattern.fullmatch(str(field_id)) or not isinstance(descriptor, dict) or (not descriptor_required <= set(descriptor) <= descriptor_required | descriptor_optional):
                ...
                if not physical_column or len(physical_column) > 256 or (not semantic_type_pattern.fullmatch(semantic_type)) or (not isinstance(roles, list)) or (not roles) or (len(roles) != len(set(roles))) or (not set(roles) <= field_roles) or (not isinstance(aliases, list)) or (len(aliases) != len(set(aliases))) or any((not isinstance(value, str) or not value.strip() or len(value) > 256 for value in aliases)):
                    raise ValueError('승인 field descriptor의 물리 컬럼, 타입, 역할 또는 alias가 유효하지 않습니다.')
                normalized_descriptor = {'physical_column': physical_column, 'semantic_type': semantic_type, 'roles': list(roles)}
            ...
            try:
                dataset_template = _validated_dataset_template(dataset_id, card.get('dataset_template'), card.get('dataset_template_sha256'), descriptors)
            except ValueError as exc:
                raise ValueError('승인 dataset template 또는 hash가 유효하지 않습니다.') from exc
            approved_names = {re.sub('\\s+', ' ', str(_name).strip()).casefold() for field_id, descriptor in descriptors.items() for _name in (field_id, descriptor['physical_column'], *(descriptor.get('physical_aliases') or []))}
            ...
            for exclusion_name in sorted(raw_exclusions):
                ...
                folded = re.sub('\\s+', ' ', str(exclusion_name).strip()).casefold()
                if not isinstance(exclusion_name, str) or not exclusion_name.strip() or len(exclusion_name) > 256 or any((ord(character) < 32 for character in exclusion_name)) or (reason not in exclusion_reasons) or (set(policy) != expected_policy_keys) or (folded in folded_exclusions) or (folded in approved_names):
                ...
                proposal_exclusions[exclusion_name] = {key: policy[key] for key in sorted(policy)}
            bindings[str(dataset_id)] = normalized
            dataset_descriptors[str(dataset_id)] = {'family': family, 'fields': descriptors, 'proposal_exclusions': proposal_exclusions, 'dataset_template': dataset_template, 'dataset_template_sha256': str(card.get('dataset_template_sha256') or '')}
        dataset_families, field_families = _semantic_maps_from_descriptors(dataset_descriptors)
        ...
        template_provenance = {key: str(registry.get(key) or '') for key in ('semantic_templates_sha256', 'semantic_templates_blueprint_sha256', 'semantic_templates_executable_sha256', 'semantic_templates_projection_sha256')}
        if template_provenance['semantic_templates_sha256'] != semantic_templates_sha256 or any((not re.fullmatch('[0-9a-f]{64}', value) for value in template_provenance.values())):
            raise ValueError('승인 의미 템플릿 provenance hash가 유효하지 않습니다.')
        material = {'contract_version': 'metadata.authoring.source-registry-context.v3', 'domain_id': domain_id, 'bindings': bindings, 'dataset_descriptors': dataset_descriptors, 'semantic_vocabulary': semantic_vocabulary, 'semantic_templates': semantic_templates, **template_provenance}
