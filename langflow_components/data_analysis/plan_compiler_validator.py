...
def _v2p_validate_catalog_pin(catalog: dict[str, Any]) -> None:
    ...
    for section in ('datasets', 'fields', 'metrics'):
        if not isinstance(catalog.get(section), dict):
            _v2p_fail('Runtime catalog registry is invalid.', {'section': section})
...
def _v2p_validate_plan_hashes(plan: dict[str, Any]) -> None:
    ...
    semantic = {key: material[key] for key in ('input_refs', 'retrieval_jobs', 'operations', 'result_operation_id', 'result_contract', 'lineage') if key in material}
...
def _v2p_source_constraints(catalog: dict[str, Any], dataset_key: str, base_metrics: Iterable[str], semantics: dict[str, Any], *, apply_date: bool) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if apply_date and (bool(semantics.get('date_explicit')) or implicit_current_day):
        ...
        parameter_specs = dataset.get('parameters') if isinstance(dataset.get('parameters'), dict) else {}
        local_date_parameters = [str(name) for name, spec in parameter_specs.items() if isinstance(spec, dict) and (str(spec.get('type') or '').casefold() in {'localdate', 'date'} or _v2p_semantic_type(catalog, str(name)).casefold() in {'localdate', 'date'})]
...
def _v2p_finalize(intent: dict[str, Any], bundle: dict[str, Any], catalog: dict[str, Any], jobs: list[dict[str, Any]], operations: list[dict[str, Any]], result_operation_id: str, columns: list[str], grain: list[str], *, input_refs: list[str] | None=None) -> dict[str, Any]:
    lineage = {field: {'field': field, 'dataset_keys': _v2p_stable(((catalog.get('fields') or {}).get(field) or {}).get('dataset_keys') or [])} for field in columns}
    material = {'contract_version': 'analysis.plan.v1', 'intent_sha256': intent.get('intent_sha256'), 'candidate_bundle_sha256': bundle.get('bundle_sha256'), 'input_refs': list(input_refs or []), 'retrieval_jobs': jobs, 'operations': operations, 'result_operation_id': result_operation_id, 'result_contract': {'columns': columns, 'ordering': [], 'grain': grain}, 'lineage': lineage}
    normalized = deepcopy(material)
    normalized['retrieval_jobs'] = sorted(normalized['retrieval_jobs'], key=lambda item: item['job_id'])
    plan_hash = sha256_json(normalized)
    semantic = {key: normalized[key] for key in ('input_refs', 'retrieval_jobs', 'operations', 'result_operation_id', 'result_contract', 'lineage')}
